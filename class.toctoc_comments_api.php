 ...
 *
 *
 *   84: class toctoc_comments_api
 *  119:     public function __construct()
 *  139:     public function comments_getComments_fe_user($params, $conf)
 *  156:     public function getwebpagepreview($cmd, $cid, $data, $previewid, $conf)
 *  171:     public function cleanupfup($cmd, $cid, $data, $previewid, $conf, $originalfilename)
 *  184:     public function handleCommentatorNotifications($ref, $conf = null, $notfromeID =FALSE, $pid)
 *  200:     public function handleeID($ref, $conf, $messagetodisplay, $returnurl)
 *  221:     public function getAjaxRatingDisplay($ref, $conf = null, $fromAjax = FALSE, $pid=0, $returnasarray = FALSE, $feuserid = 0, $cmd, $cid, $fromcomments=FALSE, $commentspics, $scopeid=0)
 *  236:     public function getUserCard($basedimgstr, $basedtoctocuid, $conf, $commentid)
 *  254:     public function getAjaxCommentDisplay($ref, $conf = null, $fromAjax, $pid=0, $returnasarray = FALSE,
 ...
 *  299:     public function updateComment($conf, $ctid, $content, $pid, $plugincacheid)
 *  309:     public function previewcomment($data, $conf)
 *  321:     public function isVoted($ref, $conf, $scopeid, $feuser)
 *  329:     public function initCaches()
 *  338:     public function enableFields($table)
 *  347:     public function setPluginCacheControlTstamp ($external_ref_uid_list)
 *
 * TOTAL FUNCTIONS: 15
...
class toctoc_comments_api {
	...
	*/
	 ...
	 * @return	string		Generated HTML
	 */
	public function cleanupfup($cmd, $cid, $data, $previewid, $conf, $originalfilename) {
		$html = $this->lib->cleanupfup($cmd, $cid, $data, $this,TRUE, $previewid, $conf, $originalfilename);
		return $html;
	}
	/**
	 ...
	 * @return	string		Generated HTML
	 */
	public function getAjaxRatingDisplay($ref, $conf = null, $fromAjax = FALSE, $pid=0, $returnasarray = FALSE, $feuserid = 0, $cmd, $cid, $fromcomments=FALSE, $commentspics, $scopeid=0) {
 ...
 */
		public function getAjaxCommentDisplay($ref, $conf = null, $fromAjax, $pid=0, $returnasarray = FALSE,
			...
			if ($fromAjax) {
				...
				$_SESSION['lastTotalrows']= $this->totalrows;
				$_SESSION['lastStartpoint']= $this->startpoint;
				...
				$html = $this->lib->maincomments($ref, $conf, $fromAjax, $pid, $feuserid, $cmd, $this, $piVars, $cid, $datathis['sessionCaptchaData'], $userpic, $commentspics, $check, $AjaxData, $extref, $tctreestate, $commentreplyid, $confSess);
				if ($isrefresh==1) {
					$html .= '<div id="dummy"></div>' . $this->lib->maincomments($ref, $conf, $fromAjax, $pid, $feuserid, 'showform', $this, $piVars, $cid, $datathis['sessionCaptchaData'], $userpic, $commentspics, $check, $AjaxData, $extref, $tctreestate, $commentreplyid);
					$html .= '<div id="dummy"></div><div>' . $this->lib->maincomments($ref, $conf, $fromAjax, $pid, $feuserid, 'updateAJAXdata', $this, $piVars, $cid, $datathis['sessionCaptchaData'], $userpic, $commentspics, $check, $AjaxData, $extref, $tctreestate, $commentreplyid) . '</div>';
				}
	...
	public function updateComment($conf, $ctid, $content, $pid, $plugincacheid) {
		return $this->lib->updateComment($conf, $this, $ctid, $content, $pid, $plugincacheid);
	}
	/**
	...
	public function previewcomment($data, $conf) {
		return $this->lib->previewcomment($data, $this, $conf, TRUE);
	}
	/**
	 ...
	 * @return	boolean		TRUE if item was voted
	 */
	public function isVoted($ref, $conf, $scopeid, $feuser) {
		return $this->lib->isVoted($ref, $conf, $this, $scopeid, $feuser);
	}
	/**
	...
	public function enableFields($table) {
		return $this->lib->enableFields($table, $this);
	}
	/**
