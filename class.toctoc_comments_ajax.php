 ...
 *
 *
 *   87: class toctoc_comments_ajax
 *  133:     public function __construct()
 *  514:     public function main()
 *  551:     public function handleCommentatorNotifications()
 *  562:     protected function updateCommentDisplay()
 *  576:     protected function updateComment()
 *  590:     protected function webpagepreview()
 *  601:     protected function previewcomment()
 *  612:     protected function cleanupfup()
 *  626:     protected function getCaptcha($captchatype, $cid)
 *  755:     protected function chkcaptcha($cid, $code)
 *  777:     protected function getUserCard()
 *  789:     protected function updateCommentsView()
 *  891:     protected function updateRating()
 * 1194:     protected function processDeleteSubmission()
 * 1262:     protected function processDenotifycommentSubmission()
 * 1307:     protected function recentCommentsClearCache()
 *
 * TOTAL FUNCTIONS: 16
...
class toctoc_comments_ajax {
	...
	protected function updateCommentDisplay() {
		...
		$piVars = $this->piVars;
		echo $apiObj->getAjaxCommentDisplay($this->ref, $this->conf, TRUE, $this->pid, FALSE, $this->feuser, $this->cmd, $this->piVars, $this->cid, $this->datathis, $this->AjaxData, $this->userpic, $this->commentspics, $this->check, $this->extref,$this->tctreestate, $this->commentreplyid, $this->isrefresh, $this->confSess);
	}
	/**
	 * Updates comment
	 *
 ...
 */
	protected function updateCommentsView() {
		...
		$dataWhere = 'deleted=0 AND pid=' . intval($this->conf['storagePid']) . ' AND reference="' . $pluginid . '" AND toctoc_comments_user=' . $fetoctocusertoquery .'' ;
		...
		echo '$action: '. $action .', feusertoinsert: '. $feusertoinsert . ', $pageid: '. $pageid . ', $setPluginCache...: '.  $setPluginCacheControlTstamppluginid . ', $pluginid: '. $pluginid . ', storagePid: '. $this->conf['storagePid'];
	}
	/**
	 * Updates rating data and outputs new result
	 *
