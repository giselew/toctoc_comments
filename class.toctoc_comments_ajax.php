 ...
 *  335:     protected function updateComment()
 *  353:     function getCaptcha($captchatype, $cid)
 *  482:     function chkcaptcha($cid, $code)
 *  503:     protected function getUserCard()
 *  515:     protected function updateRating()
 *  714:     protected function processDeleteSubmission()
 *
 * TOTAL FUNCTIONS: 8
...
class toctoc_comments_ajax {
	...
	public function __construct() {
		...
		if($this->cmd == 'getcap') {
			...
			if((strpos($this->cmd, 'ote') === false) &&(strpos($this->cmd, 'like') === false) &&($this->cmd !== 'deletecomment') &&(strpos($this->cmd, 'browse') === false)) {
	...
	protected function updateRating() {
		...
		if($this->conf['ratings.']['disableIpCheck'] || !$apiObj->isVoted($this->ref, $this->conf) ||!(($this->cmd == 'vote') ||($this->cmd == 'votearticle'))) {
			...
			if(($this->conf['advanced.']['enableUrlLog']) || (!($this->conf['ratings.']['disableIpCheck']))) {
				if((strpos($this->cmd, 'top') !== false) ||(strpos($this->cmd, 'ote') !== false)) {
