 ...
 *
 *
 *   88: class toctoc_comments_ajax
 *  134:     public function __construct()
 *  535:     public function main()
 *  574:     public function handleCommentatorNotifications()
 *  585:     protected function updateCommentDisplay()
 *  603:     protected function updateComment()
 *  618:     protected function webpagepreview()
 *  630:     protected function previewcomment()
 *  642:     protected function cleanupfup()
 *  656:     protected function getCaptcha($captchatype, $cid)
 *  788:     protected function chkcaptcha($cid, $code)
 *  812:     protected function getUserCard()
 *  825:     protected function updateCommentsView()
 *  945:     protected function updateRating()
 * 1299:     protected function processDeleteSubmission()
 * 1378:     protected function processDenotifycommentSubmission()
 * 1428:     protected function recentCommentsClearCache()
 *
 * TOTAL FUNCTIONS: 16
...
class toctoc_comments_ajax {
	...
	private $capchafreecapbackgoundcolor = '255, 255, 255'; //valid rgb
	private $capchafreecaptextcolor = '95, 117, 200'; //valid rgb
	private $capchafreecapnumberchars = 5;    //max is 10, min is 3
	private $capchafreecapheight = 23;    //max is 50, min is 23
	private $softcheck = 0;
 ...
 */
	protected function updateCommentsView() {
		...
		echo '$action: '. $action .', feusertoinsert: '. $feusertoinsert . ', $pageid: '. $pageid . ', $setPluginCache...: '.  $setPluginCacheControlTstamppluginid .
				', $pluginid: '. $pluginid . ', storagePid: '. $this->conf['storagePid'];
	}
	...
	protected function updateRating() {
		...
		if($this->conf['ratings.']['disableIpCheck'] || (!$apiObj->isVoted($this->ref, $scopeid, $this->feuser) || ($ratingsmode=='autostatic')) ||
			...
			$sqlstr = 'SELECT COUNT(uid) AS nbrentries, SUM(ilike) AS sumilike, SUM(idislike) AS sumidislike, SUM(myrating) AS summyrating,
