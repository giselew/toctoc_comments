...
class toctoc_comments_be_comments {
	...
	public function beComments(&$pObj, $pid = 0) {
		...
		while($row=$GLOBALS['TYPO3_DB']->sql_fetch_assoc($res)) {
			...
			$editTable = 'tx_toctoc_comments_comments';
			$editUid = $row['uid'];
			$cacheId = $row['external_ref_uid'] . 'external_ref' . $row['external_ref'];
			$pid_record = $row['pid'];
			$hiddenField = 'hidden';
			...
			*/
			...
			if ($row[$approvedField])	{
				...
				if ($fromAjax == TRUE) {
					$content .= '
						...
						$GLOBALS['LANG']->getLL('disapprove').'" align="top" alt="" /></span></td>';
				} else {
					$content .='
						<td class="img tx-tc-wm30 tx-tc-flop2-col"><a href="'.$newlink.'">
						<img ' . $pObj->iconWidthHeight . ' src="'.$GLOBALS['BACK_PATH'].'../typo3conf/ext/toctoc_comments' . $pObj->picpathtoctoc . '/' . $pObj->iconApproved . '" border="0" title="'.
						$GLOBALS['LANG']->getLL('disapprove').'" align="top" alt="" /></a></td>';
				 }
			...
			if ($row[$hiddenField])	{
				...
				if ($fromAjax == TRUE) {
					$content .= '
						...
						$GLOBALS['LANG']->getLL('show').'" align="top" alt="" /></span></td>';
				} else {
					$content .='
						<td class="img tx-tc-wm30 tx-tc-flip2-col"><a href="'.$newlink.'">
						<img ' . $pObj->iconWidthHeight . ' src="'.$GLOBALS['BACK_PATH'] . $pObj->picpathsysext . $pObj->iconUnhide . '" border="0" title="'.
						$GLOBALS['LANG']->getLL('show').'" align="top" alt="" /></a></td>';
				}
			...
			$_SESSION['toctoccomments_cacheid'] = $cacheId;
			...
			$content .= '
				...
				$GLOBALS['LANG']->getLL('edit').'" border="0" alt="" /></a></td>';
			...
			if ($fromAjax == TRUE) {
				...
				$GLOBALS['LANG']->getLL('delete').'" alt="" /></span></td>';
			} else {
				...
				<img src="' . $GLOBALS['BACK_PATH'] . $pObj->picpathgfx . $pObj->iconDelete . '" ' . $pObj->iconWidthHeight . 'title="'.
				$GLOBALS['LANG']->getLL('delete').'" alt="" /></a></td>';
			}

			$content .= '
				<td class="tx-tc-wm20"><input class="tx-tc-wm20" type="checkbox" name="fields[]" value="'.$editUid.'" /></td>';
			$content .= '
				</tr>
				';
		}
