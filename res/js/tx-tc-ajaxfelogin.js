...
function tt_logout(idnr, ttrid) {
	...
	var inith =0;
	if (standalone===0) {
		inith = eval(document.getElementById('cf'+commentid).offsetHeight) - eval(document.getElementById('textarea-'+commentid).offsetHeight);
		jQuery('#formhider-'+commentid).html(loadinghtml);
	...
	jQuery.ajax({
		...
		success: function(data) {
			...
			if (response.redirect){
				...
				if (standalone===0) {
					inith = eval(document.getElementById('formhider-'+commentid).offsetHeight);
					jQuery('#'+obj.form.id).css('display','none');
