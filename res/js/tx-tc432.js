...
function toctoc_comments_refresh_single(id, ttcontentid, commentidin, islogout,tctreestateenc) {
	...
	if (islogout === 1) {
		commentsAjaxDataLogin = eval("commentsAjaxDataLogout" + idnr);
	} else {
		commentsAjaxDataLogin = eval("commentsAjaxDataLogin" + idnr);
	}
...
function toctoc_comments_submit(id, rating, ajaxData, check, action, cssident, datac, thisdata, capsess, cid, caperr,commentsImgs, loggedon, extid, webpagepreviewheight,windowpreviewhtml) {
	if ((cssident.indexOf("cts")>=0) || (cssident.indexOf("form")>=0)) {
		...
		// var elem = document.getElementById('tx-tc-cts-dp-' + thisdata);
...
function commentreply(uid, previewselcommentcid, cid, ref, triggeredzone, triggereduid){
	...
	if (triggeredzone === 'triggerform') {
		...
		var elemcid =document.getElementById('toctoc_comments_pi1_contenttextbox_'+cid);
		comment_formhider(cid, 1, '', 0, 0,elemcid);
		window['previewselcomment' + cid] = uid;
...
function check_dates() {
	...
	if (tcservertime !== 0) {
		...
		for (i= tccommnetidstart ; i < tccommnetidto ; i++) {
			var elem = document.getElementById('tx-tc-ctdatetime-' + i);
			if (elem) {
