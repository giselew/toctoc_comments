...
function comment_formhider(cidcomments, showhider, textaddcomment, loggedon, makewindowresize , thisin) {
    ...
    //onFocus
            thistextarea.style.margin = tamargin;
       	 	var loginformaddhiderheight=0;
       	 	loginformaddhiderdisplay = 'block';
       	 	if ((loginRequiredIdLoginForm=='') || (loggedon==1)) {
...
function toctoc_comments_browse(id, rating, ajaxData, check, action, cssident, datac, thisdata, startpoint, cid, totalrows,commentsImgs) {
	//toctoc_comments_browse('tt_content_199', 1, commentsAjaxData199, '5dc82b9bf7eb3d2dce933ed7d4a7c26a', 'browse', 'cts', commentsAjaxDataC199, commentsAjaxThisData199,25,'199',28, commentsAjaxDataImg199
	var str1=this.toctoc_comments_pi1_serialize(tctreestate);
    var tctreestateenc=this.toctoc_comments_pi1_base64_encode(str1);
    
    setopacity('tx-tc-cts-dp-' + id,'0.4','toctoc_comments_browse 1');
    jQuery.ajax({
        type: 'POST',
        url: 'index.php?eID=toctoc_comments_ajax',
        async: false,
        ...
        success: function(html){
    		if (html.indexOf('<div class="dummy"></div><div class="dummy2">')!=-1) {
    			var htarr=html.split('<div class="dummy"></div><div class="dummy2">');
    			var html=htarr[0];
        		var htmlsessionvar=htarr[1];
        		htmlsessionvar=htmlsessionvar.replace('</div>','');
         		eval("commentsAjaxDataLoginSess" + cid + "=\'" + htmlsessionvar + "\';");
    		}
        	if (html.indexOf('<div id="dummy"></div>')>0) {
        		var htarr=html.split('<div id="dummy"></div>');
        		tophtml=htarr[0];
    ...
    setopacity('tx-tc-cts-dp-' + id,'1','toctoc_comments_browse 2');
}
...
function toctoc_comments_refresh_single(id, ttcontentid, commentidin, islogout,tctreestateenc) {
    ...
    idnr = idnr.replace("\'","");
     var commentsAjaxData = eval("commentsAjaxData" + idnr);
    var commentsAjaxDataImg = eval("commentsAjaxDataImg" + idnr);
    var commentsAjaxThisData = eval("commentsAjaxThisData" + idnr);
    var commentsAjaxDataC = eval("commentsAjaxDataC" + idnr);
    if (islogout==1) {
        var commentsAjaxDataLogin = eval("commentsAjaxDataLogout" + idnr);    	
    } else {
        var commentsAjaxDataLogin = eval("commentsAjaxDataLogin" + idnr);    	
    }
    var commentsAjaxDataLoginSess = eval("commentsAjaxDataLoginSess" + idnr);
    var rating = 1;
	    ...
	    jQuery.ajax({
	        ...
	        url: 'index.php?eID=toctoc_comments_ajax',
	        async: true,
	        data: 'ref=' + id + '&rating=' + rating+ '&islogout=' + islogout + '&data=' + commentsAjaxData + '&dataLogin=' + commentsAjaxDataLogin + '&dataLoginSess=' + commentsAjaxDataLoginSess + '&isrefresh=1&datac=' + commentsAjaxDataC + '&datathis=' + commentsAjaxThisData + '&capsess=0&check=&cmd=showcomments&userpic=&commentsimgs=' + commentsAjaxDataImg + '&tctreestateenc=' + tctreestateenc + '&extref='  + extidroot,
	        success: function(html){
	        	if (html.indexOf('<div id="dummy"></div>')>0){
	        		var htarr=html.split('<div id="dummy"></div>');
	        		tophtml=htarr[0];
	        		...
	        		htarr[3]=htarr[3].replace('<\/div>','');
	        		eval("commentsAjaxData" + idnr + "=\'" + htarr[3] + "\';");
	        		if (islogout==1) {
	        			global_loggedon=0;
	        		} else {
	        			global_loggedon=1;
	        		...
	        		if (commentidin!= 0) {
		        		var elemta=document.getElementById('toctoc_comments_pi1_contenttextbox_' + commentidin);
		        		if (elemta) {
		        			...
		        			if (commentidarr.length>1) {
		        				...
		        				if (confreplyModeInline==1) {
		        					if (confreplyModeInlineOpenForm==0) {
		        						...
		        						jQuery('#tx-tc-cts-rply-' + uid).css('display', 'block');
		       							elemta.focus();
		       							elemta.value=currentcommenttext;
		       							window['previewselcomment' + idnr] = uid;
			            			} else {
			            				elemta.value=currentcommenttext;
			            				elemta.focus();
				            			elemta.value=currentcommenttext;	
		        				...
		        				commentidin=window['previewselcomment' + idnr];	
		        				elemta.value=currentcommenttext;
		        				elemta.focus();
		            			elemta.value=currentcommenttext;
		            			window['previewselcomment' + idnr] = commentidin;
		        			}
		        			if (currentcommenttext!='') {
		        				eval("previewstarted" + idnr + "= toctoc_checkurl(0, document.getElementById('toctoc_comments_pi1_contenttextbox_" + commentidin +"') ,\'" + commentidin + "\', previewstarted" + idnr + ", 'jedi', 0, '', '', " + maxCommentLength + ");");
		        			}
...
function toctoc_comments_submit(id, rating, ajaxData, check, action, cssident, datac, thisdata, capsess, cid, caperr,commentsImgs, loggedon, extid, webpagepreviewheight,windowpreviewhtml) {
    if ((cssident.indexOf("cts")>=0) || (cssident.indexOf("form")>=0)) {
        ...
        jQuery.ajax({
            ...
            success: function(html){
            	...
            	if (elemdisp2) {
            		...
            		htmout =htmout.substr(0,htmout.length-20);
        ...
        if ((idwithnoscope != id) && (idwithnoscope != 0)) {
            ...
            tttip('t201-5',"#tx-tc-rts-dp-" + idwithnoscope + " .tx-tc-rts-vote-bar div div[title]");
        ...
        tttip('t201-5',"#tx-tc-rts-dp-" + id + " .tx-tc-rts-vote-bar div div[title]");
        tttip('t201-5',"#tx-tc-rts-dp-" + id + " .tx-tc-rts-vote-bar div a[title]");
        tttip('t201-5',"#tx-tc-rts-disp2-" + id + " .tx-tc-rts-vote-bar div span[title]");
        tttip('t201-5',"#tx-tc-rts-disp2-" + id + " .tx-tc-rts-vote-bar div div[title]");
        tttip('t201-5',"#tx-tc-rts-disp2-" + id + " .tx-tc-rts-vote-bar div a[title]");
...
function savect(id, icid,pid){
   ...
   jQuery.ajax({
        ...
        success: function(html){
			...
			if (elemd2vilikeids) {
				jQuery('#tx-tc-ct-box-cttxt-start-' + id).html(html);
					if (elemd2vilikeids.style.display!='inline') {
						elemd2vilikeids.style.display='inline';
					}
		...
		if (editon == false) {
			...
			if (elemdnf) {
			 ...
			 if (edithtmlarr.length >1) {
				 ...
				 if (edithtmlkitarr[1]) {
					 edithtml='<b>Firstname</b>&nbsp;<b>Lastname</b> - ' + edithtmlkitarr[1];//.substr(0,edithtmlkitarr[1].length-8);
				 }
			 } 
			...
			if (edithtmloutemoarr.length > 1) {
				 for (var i = 0; i < edithtmloutemoarr.length; i++) {
					 edithtmloutemoarr2=new Array();
					 edithtmloutemoarr2= edithtmloutemoarr[i].split('"></span>');
					 if (edithtmloutemoarr2.length > 1) {
						 edithtmloutemoarr3=new Array();
...
//				outhtml= outhtml.replace('class=\"tx-tc-form-for-newcomment\"','class=\"tx-tc-form-for-newcomment\" style=\"margin: ' + (2+boxmodelSpacing+boxmodelLineHeight) + 'px 0 0 0\"');
				outhtml= outhtml.replace('class=\"tx-tc-form-for-newcomment\"','class=\"tx-tc-form-for-newcomment\" style=\"margin: 0\"');
        	}
			if (elemd2vilikeids) {
				outhtml= outhtml.replace('class=\"tx-tc-form-for-newcomment\"','class=\"tx-tc-form-for-newcomment\" style=\"margin: 0\"');
				//older outhtml= outhtml.replace('class=\"tx-tc-form-for-newcomment\"','class=\"tx-tc-form-for-newcomment\" style=\"margin: ' + (2+boxmodelSpacing+boxmodelLineHeight) + 'px 0 ' + (-boxmodelLineHeight) + 'px 0\"');
...
function tctrvw (expand, uid, children, allchildren){
	...
	//expand
	    	...
	    	if  (tcisInt(childrenArray[i])){
				...
				if (elemicon) {
	    			...
	    			if (tcelemstate[uid][childrenArray[i]]['expanded']==1) {
	    				...
	    				document.getElementById('tx-tc-cts-explink-' + childrenArray[i]).innerHTML=ihtml.replace('tctrvw(0,','tctrvw(1,');
	    			} else {
	    				...
	    				document.getElementById('tx-tc-cts-explink-' + childrenArray[i]).innerHTML=ihtml.replace('tctrvw(1,','tctrvw(0,');
	    			}
				} else{ 
	    	...
	    	if  (tcisInt(tctrim(allchildrenArray[i]))){
				...
				if (elemicon) {
	    			...
	    			if (tcelemstate[uid][allchildrenArray[i]]['expanded']==1) {
	    				...
	    				ihtml = ihtml.replace('"tx-tc-lnkexp-2"','"tx-tc-lnkexp"');
	    				document.getElementById('tx-tc-cts-explink-' + allchildrenArray[i]).innerHTML=ihtml.replace('tctrvw(0,','tctrvw(1,');
	    			} else {
	    				...
	    				ihtml = ihtml.replace('"tx-tc-lnkexp"','"tx-tc-lnkexp-2"');
	    				document.getElementById('tx-tc-cts-explink-' + allchildrenArray[i]).innerHTML=ihtml.replace('tctrvw(1,','tctrvw(0,');
	    			}
				} else{ 
	    ...
	    ihtml= document.getElementById('tx-tc-cts-explink-' + uid).innerHTML;
	    ...
	    ihtml = ihtml.replace('"tx-tc-lnkexp-2"','"tx-tc-lnkexp"');
	    document.getElementById('tx-tc-cts-explink-' + uid).innerHTML=ihtml.replace('tctrvw(0,','tctrvw(1,');
	} else {
		//collapse
		for (var i = 0; i < childrenArray.length; i++) {
	    	childrenArray[i]=tctrim(childrenArray[i]);
	    ...
	    ihtml = ihtml.replace('"tx-tc-lnkexp"','"tx-tc-lnkexp-2"');
	    document.getElementById('tx-tc-cts-explink-' + uid).innerHTML=ihtml.replace('tctrvw(1,','tctrvw(0,');
	}
}
function tcsroc (uid){
	...
	if (elem) {
		if (elem.innerHTML.indexOf('icon_comments')>0) {
			...
			if (elem.innerHTML.indexOf('nbrofcomments srt">')>0) {
				var elemarriconized= elem.innerHTML.split('nbrofcomments srt">');
				var countval=tctrim(elemarriconized[1].substr(0, elemarriconized[1].indexOf('<')));
				var newval = eval(countval+addval);
				elemarriconized[1]=newval+'&nbsp;'+elemarriconized[1].substr(elemarriconized[1].indexOf('<'));
				var eleminnerHTML = elemarriconized.join('nbrofcomments srt">');

			} else {
				
				if (elem.innerHTML.indexOf('nbrofcomments-1">')>0) {
					///nbrofcomments-1">
					var elemarriconized= elem.innerHTML.split('nbrofcomments-1">');
					var countval=tctrim(elemarriconized[1].substr(0, elemarriconized[1].indexOf('<')));
					var newval = eval(countval+addval);
					elemarriconized[1]=newval+elemarriconized[1].substr(elemarriconized[1].indexOf('<'));
					var eleminnerHTML = elemarriconized.join('nbrofcomments-1">');

				} else {
					var elemarriconized= elem.innerHTML.split('nbrofcomments">');
					var countval=tctrim(elemarriconized[1].substr(0, elemarriconized[1].indexOf('<')));
					var newval = eval(countval+addval);
					elemarriconized[1]=newval+elemarriconized[1].substr(elemarriconized[1].indexOf('<'));
					var eleminnerHTML = elemarriconized.join('nbrofcomments">');
				}			
			}
		}
			...
			elem= document.getElementById('tx-tc-ctdatetime-' + i);
		 	if (elem) {
			 	...
			 	if (elemdisp) {
			 		...
			 		if (elemdisp.innerHTML.indexOf('&middot;')>1) {
			 			addmiddot ='&nbsp;&middot;&nbsp;';	
			 		}
...
function comments_view(icid, iusr, ajaxData, isComment, storagepid) {
	window.setTimeout("comments_view_delayed('"+icid+"', '"+iusr+"', '"+ajaxData+"', '"+isComment+"', '"+storagepid+"')", 3000);
}
function comments_view_delayed(icid, iusr, ajaxData, isComment, storagepid) {
	jQuery.ajax({
        ...
        url: 'index.php?eID=toctoc_comments_ajax',
        async: true,
        data: 'data=' + ajaxData + '&pageid=' + pageid + '&usr=' + iusr + '&ref=' + icid + '&cmd=commentsview&storagepid=' + storagepid,
        success: function(html){
        }
    });
}
...
function tt_showlogin(idtoshowlogin, onoff) {
	...
	if (htmlloginform=='') {
		taelem=document.getElementById(loginRequiredIdLoginForm);
		if (taelem) {
			htmlloginform=taelem.innerHTML;
