...
function scalecomment(IpushmainH) {
	
}
function pushmain(IpushmainH,makewindowresize) {
	//placeholder function for resize of dependent controls
	//IpushmainH :  offsetHeight-Difference as px
...
function comment_formhider(cidcomments, showhider, textaddcomment, loggedon, makewindowresize , thisin) {
	...
	thishider = document.getElementById('formhider-' + cidcomments);
	thisuserimg = document.getElementById('tx-toctoc-comments-uimg-' + cidcomments);
	thisinput = thisin; 
	...
	if (showhider==2) {
		//onblur
			document.getElementById('textarea-' + cidcomments).style.margin = tainitmargin;
			//document.getElementById('toctoc_comments_pi1_contenttextbox_' + cidcomments).rows = 1;
			var	field = document.getElementById('toctoc_comments_pi1_contenttextbox_' + cidcomments);
			if (field) {
				checklen = this.tctrim(textaddcomment).length;
				checkfield = this.tctrim(field.value);
				checkfield2 = checkfield.substr(0,checklen);
				
				if ((checkfield2 == this.tctrim(textaddcomment)) || (checkfield=='')) {
					//see also in elastic.js, line 3, pos 178
					document.getElementById('toctoc_comments_pi1_contenttextbox_' + cidcomments).style.height = '20px';
				}
			}

			document.getElementById('toctoc_comments_pi1_submit_' + cidcomments).style.visibility = 'hidden';
			pushmainH = eval(-1 * thishider.offsetHeight);
			thishider.style.visibility = 'hidden';
			document.getElementById('tx-toctoc-comments-formsqueezer-' + cidcomments).style.height = '32px';
			...
			document.getElementById('toctoc_comments_pi1_submit_' + cidcomments).style.height = 0; 
			document.getElementById('toctoc_comments_pi1_submit_' + cidcomments).style.margin = 0;
			pushmain(pushmainH,makewindowresize);

	};
	if (showhider==3) {
	//onFocus=
			document.getElementById('textarea-' + cidcomments).style.margin = tamargin;
			document.getElementById('toctoc_comments_pi1_submit_' + cidcomments).style.visibility = 'visible';
			...
			document.getElementById('toctoc_comments_pi1_submit_' + cidcomments).style.height = 'auto'; 
			document.getElementById('toctoc_comments_pi1_submit_' + cidcomments).style.margin = submitmargin;
			document.getElementById('tx-toctoc-comments-formsqueezer-' + cidcomments).style.height = '';
			minheight=document.getElementById('textarea-' + cidcomments).style.minHeight;
			minheightval=minheight.substr(0,minheight.indexOf("p"));
			taoffsetheight=document.getElementById('textarea-' + cidcomments).offsetHeight
...
function commentform_validate(cidcomments, textemptyerror, texttoshorterror) {
...
function toctoc_comments_submit(id, rating, ajaxData, check, action, cssident, datac, thisdata, capsess, cid, caperr,commentsImgs, loggedon) {
	if ((cssident.indexOf("comment")>=0) || (cssident.indexOf("form")>=0)) {
		...
		if (responsed==1) {
			...
			jQuery.ajax({
		        ...
		        url: 'index.php?eID=toctoc_comments_ajax',
		        async: false,
		        data: 'ref=' + id + '&rating=' + rating + '&data=' + ajaxData + '&datac=' + datac + '&datathis=' + thisdata + '&capsess=' + capsess + '&check=' + check + '&cmd=' + action + '&userpic=' + picress,
		        success: function(html){
			...
			jQuery('#tx-toctoc-comments-' + cssident + '-display-' + id).css('opacity', '1');
			...
			if(htmlretcode != 0)	{
				...
				jQuery('#tx-toctoc-comments-comments-display-' + id).css('opacity', '0.6');
				jQuery.ajax({
			        type: 'POST',
			        url: 'index.php?eID=toctoc_comments_ajax',
			        async: false,
			        ...
			        success: function(html){
			        	jQuery('#tx-toctoc-comments-comments-' + id).html(html);
			        }
			    });
				jQuery('#tx-toctoc-comments-comments-display-' + id).css('opacity', '1');
			}
