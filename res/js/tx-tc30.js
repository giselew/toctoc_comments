    ...
    if ((cssident.indexOf("cts")>=0) || (cssident.indexOf("form")>=0)) {
        ...
        if (responsed==1) {
        	...
        	setopacity('tx-tc-' + cssident + '-dp-' + extid,'0.4','toctoc_comments_submit 5');
             ...
             setopacity('tx-tc-' + cssident + '-dp-' + extid,'1','toctoc_comments_submit 6');
            ...
            if(htmlretcode != 0)    {
                ...
                setopacity('tx-tc-cts-dp-' + extid,'0.4','toctoc_comments_submit 7');
                jQuery.ajax({
                    type: 'POST',
                    url: 'index.php?eID=toctoc_comments_ajax',
                    async: false,
                    ...
                    success: function(html){
                    	if (html.indexOf('<div id="dummy"></div>')>0){
                    		...
                    		tophtml=htarr[0];
                    		html=htarr[1];
                    		jQuery('#tx-tc-cts-dp-split-' + extid).html(tophtml);
                    	}
                ...
                setopacity('tx-tc-cts-dp-' + extid,'1','toctoc_comments_submit 8');
                jQuery.ajax({
                    type: 'POST',
                    url: 'index.php?eID=toctoc_comments_ajax',
                    async: true,
...
function getopacity(elemid) {
	if (document.getElementById(elemid).style.opacity >= 0) {
		...
		if (document.getElementById(elemid).style.filter.indexOf('opacity') >= 0) {
			...
			opacityval='1';
			alert('no opacity found for element: ' + elemid);
		}
	}
	return opacityval;
}
...
function tcfilechange(id, lang, fuppreviewheight,ajaxData,ajaxDataAtt){
    ...
    if( inthis.files ){
	  ...
	  //console.log( inthis.files[0] );
		...
		jQuery.ajax({
		  	  ...
		  	  success: function(data){
		  			...
		  			//console.log('previewidfup: ' + previewidfup);
		  			//console.log('previewfupdir: ' + previewfupdir);
		  			//console.log('previewfupheight: ' + previewfupheight);	      			
		  	  },
	  ...
	  //.files not supported
    	//console.log( inthis.value );
    	var file = inthis;
    	var myFSO = new ActiveXObject("Scripting.FileSystemObject");
    	if (myFSO) {
		  var filepath = inthis.value;
		  var thefile = myFSO.getFile(filepath);
		  var fsize = thefile.size;
		  
		  var fname = thefile.name;
		  var ftype = thefile.type;
		  //console.log('ftype: ' + ftype + 'fname: ' + fname + 'fsize: ' + fsize);
		  
		  if(fname.length < 1) {
			  return false;
		  }
		  else if (uploadtype=='pic') {
			  if(ftype.indexOf("JPEG") == -1 && ftype.indexOf("GIF") == -1 && ftype.indexOf("PNG") == -1 && ftype != 'image/jpeg') {
				  textmsg = ftype + ': ' + utf8_decode(tcb64_dec(textPicFiletypeErr));
				  information(textmsg,id, function () {});
				  return false;
			  }
			  if(fsize > picUploadMaxfilesize) {
				  textmsg = utf8_decode(tcb64_dec(textPicFileToBig));
				  information(textmsg,id, function () {});
				  return false;
			  }
			  ...
			  if(ftype != 'Adobe Acrobat Document' && ftype != 'application/x-pdf' ) {
				  textmsg = ftype + ': ' + utf8_decode(tcb64_dec(textpdfFiletypeErr));    		    	
				  information(textmsg,id, function () {});
				  return false;
			  }
			  if(fsize > pdfUploadMaxfilesize) {
				  textmsg = utf8_decode(tcb64_dec(textPdfFileToBig));
				  information(textmsg,id, function () {});
			      return false;
			  }
    		...
    		alert ('For file-uploads you need change your security settings to allow the activex (if your using internet explorer)');
    		return false;
    	}
    	//var oData = toctoc_comments_pi1_getUploadData(id);
    	//console.log('oData: ' + oData);
    	window['previewstartedfp' + id] = 1;
    	setopacity('toctoc_comments_pi1_submit_' + id,'0.4','tcfilechange');	 
    	jQuery('#' + inupldiv).css('display', 'none');
    	jQuery('#tx-tc-form-fup-working' + id).css('display', 'block');
    	//var oData= jQuery("form").serialize();
    	var form= document.forms.namedItem('n' + informid);
    	//iframeobj= document.getElementById('txtc-iframe- ' + id);
    	action_url='/typo3conf/ext/toctoc_comments/pi1/class.toctoc_comments_attachmentupload.php';
     ...
     //console.log('content: ' +contentout);
       
//    	
//    	 
//    	 
//    	 jQuery('form cf' . id).ajaxSubmit({
//    	    type: 'POST',
//            url: '/typo3conf/ext/toctoc_comments/pi1/class.toctoc_comments_attachmentupload.php',
//            async: false,
//    	 	success: function(data) {
...
function remove_fup_pic(id,previewopenedprogress,fuppreviewheight,ajaxData,ajaxDataAtt) {
	...
	var ininputpicidvalelemh = document.getElementById(ininputpicidh);
	var ininputpicnameelem = document.getElementById(ininputpicname);
	//console.log(ininputpicnameelem.value);
    //var str1=this.toctoc_comments_pi1_serialize(ininputpicnameelem.value);
