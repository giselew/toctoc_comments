...
function toctoc_comments_pi1_getUserData(icid) {
    ...
    var reresultucd = /(%u)+/g;
    var resultucd =  escape(tmpcontent);
    tmpcontent =  resultucd.replace(reresultucd, "\\u");
    tmpcontent =  unescape(tmpcontent);
    ...
    if (document.getElementById('tx-tc-cts-uc-inner-' + commentid).innerHTML.length < 50) {
        ...
        jQuery.ajax({
            ...
            success: function(html){
                ...
                jQuery(".tx-tc-ct-uc-pic img[title]").tooltip({offset: [-12, 0],effect: 'fade',opacity: 0.8}); 
            }
        });
    } 
             ...
             jQuery.ajax({
                ...
                success: function(html){
                    ...
                    if (!tcisInt(htmlretcode)) {
                        ...
                        var htmlretcodeapproval=tctrim(htmlretcode.substr(0,1));
...
function toctoc_previewurl_fetch (id, purl,pip, iterations, lang,webpagepreviewheight , ajaxData, ajaxDataAtt) {
    ...
    if (previewopenedprogress != 2) {
    	if (iterations<30) {
	            ...
	            success: function(html){
			    	...
			    	if ((htmlretcode >3) ) {
		    			...
		    			if (document.getElementById('tx-tc-form-dp-wpp-' + id).style.maxHeight != '0px') {
			    	    	...
			    	    	document.getElementById('formhider-' + id).style.height = "auto";
			    	    	jQuery('#tx-tc-form-wpp-' + id).html('<img id="tx-tc-form-wpp-working' + id + '" width="16" height="11" align="right" alt="'+ utf8_decode(tcb64_dec(textLoading)) + '" title="' + utf8_decode(tcb64_dec(textLoading)) +'" src="/typo3conf/ext/toctoc_comments/res/css/themes/' + selectedTheme + '/img/workingslides.gif">');
		    			 ...
		    			 console.log('wpptoctoc_previewurl_fetch' + '1');
		    			setopacity('toctoc_comments_pi1_submit_' + id,'1','toctoc_previewurl_fetch 1');
		    			return true;
	    			}
			    	 if (previewopenedprogress==2) {
			    		 console.log('wpptoctoc_previewurl_fetch' + '2');
 	                    	...
 	                    	jQuery.ajax({
	                            ...
	                            success: function(html){
	                            	if (html != '') {
	                            		if (html > 0) {
	                            			window['previewselpreviewid' + id] =html;
	                            			console.log('savepreview: htmlretcode ' + htmlretcode);
	                            		}
	                            	}
	                            }
	                        });
                    	} else {
                    		console.log('wpp-working ' + 'none');
                    		jQuery('#tx-tc-form-wpp-working' + id).css('display', 'none');
                    	}
    		...
    		console.log('no reply at all: iterations ' + iterations + ' ,previewopenedprogress ' + previewopenedprogress + ' ,(windowpreviewhtml' + id + ' :  ' + window['previewhtml' + id]);
    		iterations = 0;
    		previewopenedprogress = 2;
    		window['previewstarted' + id] = previewopenedprogress;
    		
      		if (window['previewhtml' + id] =='<img align="right" id="tx-tc-form-wpp-working' + id +'" src="/typo3conf/ext/toctoc_comments/res/css/themes/'+selectedTheme+'/img/workingslides.gif" width="16" height="11" />') {
    	    	//document.getElementById('formhider-' + id).style.height = eval(document.getElementById('formhider-' + id).offsetHeight - formhidermoreheightsecondlap - 15) + "px";
    	    	document.getElementById('formhider-' + id).style.height = "auto";
    	    	jQuery('#tx-tc-form-wpp-' + id).html('<img id="tx-tc-form-wpp-working' + id + '" width="16" height="11" align="right" alt="'+ tcb64_dec(textLoading) + '" title="' + tcb64_dec(textLoading) +'" src="/typo3conf/ext/toctoc_comments/res/css/themes/' + selectedTheme + '/img/workingslides.gif">');
    	    	jQuery('#tx-tc-form-dp-wpp-' + id).css('display', 'none');
    	    	jQuery('#tx-tc-form-dp-wpp-' + id).css('min-height', '0px');
    	    	jQuery('#tx-tc-form-dp-wpp-' + id).css('max-height', '0px');
...
function savect(id, icid,pid){
	...
	str1=tctrim(str1);
    var reresultucd = /(%u)+/g;
    var resultucd =  escape(str1);
    str1 =  resultucd.replace(reresultucd, "\\u");
    str1 =  unescape(str1);
	...
	if (onoff==0) {
		...
		if (editon == false) {
	    	...
	    	var dnArray = edithtml.split('-');
			 ...
			 var edithtmloutemoarr= edithtmloutbb.split('<span class="emoji emoji');
				if (edithtmloutemoarr.length > 1) {
					 for (var i = 0; i < edithtmloutemoarr.length; i++) {
						 ...
						 if (edithtmloutemoarr2.length > 1) {
							 //for (var j = 0; j < edithtmloutemoarr2.length; j++) {
								 edithtmloutemoarr3= edithtmloutemoarr2[0].split(' ');
								 if (edithtmloutemoarr3.length > 1) {
									 Emojis[emoidx]=edithtmloutemoarr3[0];
									 emoidx++;
									 edithtmloutemouc = '%' + edithtmloutemoarr3[1].replace('-','%');
			...
			jQuery('#tx-tc-ct-box-cttxt-' + uid).bind('keyup', function(e) {
				var dummyret=99999;
	        	 toctoc_checkurl(e.keyCode,document.getElementById('toctoc_comments_pi1_contenttextboxc_' + uid).value,uid, dummyret, 'en', 0, '', '',0);
	          });
