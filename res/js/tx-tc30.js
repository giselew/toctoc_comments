...
function comment_formhider(cidcomments, showhider, textaddcomment, loggedon, makewindowresize , thisin) {
    ...
    //onFocus=
            ...
            if (thiscontenttextbox.value=='') {
            	//console.log(thiscontenttextbox.value);
...
//                    		// does not return from invalid insert
//                    		//htmlretcode=0;
//                    	}
//                    }
...
function recentct(rcid, pid, url) {
...
function check_dates() {
	...
	if (tcservertime != 0) {
		...
		for (var i = tccommnetidstart ; i < tccommnetidto ; i++) {
			elem= document.getElementById('tx-tc-ctdatetime-' + i);
		 	if (elem) {
			 	...
			 	if (elemdisp) {
			 		elemdisp.innerHTML=newformatdate;
			 		j++;
			 	}
			}
		}
		for (var i = tccommnetidstart ; i < tccommnetidto ; i++) {
			elem= document.getElementById('tx-tc-rctdatetime-' + i);
		 	if (elem) {
		 		tstampdate=elem.innerHTML;
		 		newformatdate=tcformatnewdate(tstampdate);
		 		//console.log('rc newformatdate: ' + newformatdate);
		...
		//console.log('elapsed: ' + cdtime + 'ms, found elems: ' + j + ' in uid: ' + tccommnetidstart + ' to ' + tccommnetidto);
	}
	...
	if (checkdatedelay==200) {
		if (tcservertime != 0) {
			...
			//console.log('waiting for AJAX reply ' + noajaxreply);
			noajaxreply++;
			if (noajaxreply>100) {
...
if (tcdateformat==0) {
	jQuery.ajax({
        ...
        success: function(html){
        	...
        	//console.log('tcservertime: ' + tcservertime + ', tclocaltimediff: ' + tclocaltimediff);
        }
    });
	check_dates();
}
