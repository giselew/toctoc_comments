...
function comment_formhider(cidcomments, showhider, textaddcomment, loggedon, makewindowresize , thisin) {
	...
	var tamargin = '0';
	var submitmargin='4px 0 5px 134px';
	var tainitmargin = '0';
	var hidermargin= '0';
...
function commentform_validate(cidcomments, cidnropt) {
	...
	//console.log(requiredcommentlength);
...
function show_uc(commentid, cid, commentsAjaxData,toctocuid,imgstr,timeoutms) {    
    jQuery('#tx-tc-cts-uc-' + commentid).css('opacity', '0.9');
    ...
    if (document.getElementById('tx-tc-cts-uc-inner-' + commentid).innerHTML.length < 50) {
        document.getElementById('tx-tc-cts-uc-inner-' + commentid).innerHTML='';
        jQuery.ajax({
            type: 'POST',
            url: 'index.php',
            async: false,
    ...
    setopacity('tx-tc-cts-uc-' + commentid,'1');
    //jQuery('#tx-tc-cts-uc-' + commentid).css('opacity', '1');
    jQuery('#tx-tc-cts-uc-' + commentid).css('display', 'block');
    jQuery('#tx-tc-cts-uc-' + commentid).css('height', 'auto');
    toctoc_comments_uc_show(timeoutms, 'tx-tc-cts-uc-' + commentid);
 }

function toctoc_comments_delete(id, rating, ajaxData, check, action, cssident, datac, thisdata, commentid, cid, parentid,commentsImgs,refshow) {
    ...
    tcconfirm(txtDeleteConfirm,cid, function () {
		...
		if (elem) {
	   	...
	   	//jQuery('#tx-tc-' + cssident + '-dp-' + id).css('opacity', '0.6');
	   	setopacity('tx-tc-' + cssident + '-dp-' + id,'0.4');
	    jQuery.ajax({
	        type: 'POST',
	        ...
	        success: function(html){
	            ...
	            if (parentid != 0) {
	                //console.log('firing ref=' + refshow + '&rating=' + rating +'&capsess=0&check=' + check + '&cmd=showcomments'  + '&userpic=' + picressshow );
	                //jQuery('#tx-tc-cts-dp-' + refshow).css('opacity', '0.6');
	                setopacity('tx-tc-cts-dp-' + refshow,'0.4');
	                jQuery.ajax({
	                    type: 'POST',
	                    url: 'index.php?eID=toctoc_comments_ajax',
	                    async: false,
	                    data: 'ref=' + refshow + '&rating=' + rating + '&data=' + ajaxData + '&datac=' + datac + '&datathis=' + thisdata + '&capsess=0&check=' + check + '&cmd=showcomments'  + '&userpic=' + picressshow + '&commentsimgs=' + commentsImgs + '&tctreestateenc=' + tctreestateenc+ '&softcheck=1',
	                    success: function(html){
...
function toctoc_comments_browse(id, rating, ajaxData, check, action, cssident, datac, thisdata, startpoint, cid, totalrows,commentsImgs) {
    ...
    //jQuery('#tx-tc-cts-dp-' + id).css('opacity', '0.7');
    setopacity('tx-tc-cts-dp-' + id,'0.4');
    jQuery.ajax({
        type: 'POST',
        url: 'index.php?eID=toctoc_comments_ajax',
        async: false,
    ...
    setopacity('tx-tc-cts-dp-' + id,'1');
    //jQuery('#tx-tc-cts-dp-' + id).css('opacity', '1');
}
function toctoc_comments_submit(id, rating, ajaxData, check, action, cssident, datac, thisdata, capsess, cid, caperr,commentsImgs, loggedon, extid, webpagepreviewheight,windowpreviewhtml) {
    if ((cssident.indexOf("cts")>=0) || (cssident.indexOf("form")>=0)) {
    	...
    	//if (document.getElementById('toctoc_comments_pi1_submit_' + cid).style.opacity != '1') {
    	if (getopacity('toctoc_comments_pi1_submit_' + cid) != '1') {
    		return false;
    	}
     	var thumbstylemaxheight=webpagepreviewheight + 25;
        ...
        if (capsess=='1') {
        	...
        	setopacity('toctoc_comments_cap_' + cid,'0.4');
            //jQuery('#toctoc_comments_cap_' + cid).css('opacity', '0.6');
            responsed=0;
            jQuery.ajax({
                type: 'POST',
                url: 'index.php?eID=toctoc_comments_ajax',
                ...
                success: function(response){
                    if(response==1)    {
                        //jQuery('#toctoc_comments_cap_' + cid).css('opacity', '1');
                        responsed=1;    
                    }
                    else
                    {
                    ...
                    setopacity('toctoc_comments_cap_' + cid,'1');
                }
            });
        ...
        if (responsed==1) {
        	var htmlretcode=0;
        	setopacity('tx-tc-' + cssident + '-dp-' + extid,'0.4');
        	//jQuery('#tx-tc-' + cssident + '-dp-' + extid).css('opacity', '0.6');
            jQuery.ajax({
                type: 'POST',
                url: 'index.php?eID=toctoc_comments_ajax',
                async: false,
            ...
            //jQuery('#tx-tc-' + cssident + '-dp-' + extid).css('opacity', '1');
            setopacity('tx-tc-' + cssident + '-dp-' + extid,'1');
            // check if the insert wwas omk and only if...
            if(htmlretcode != 0)    {
            	tctreestate[htmlretcode]=0;
            	//console.log('htmlretcode=' + htmlretcode);
             	var str1=this.toctoc_comments_pi1_serialize(tctreestate);
                var tctreestateenc=this.toctoc_comments_pi1_base64_encode(str1);
                ...
                //jQuery('#tx-tc-cts-dp-' + extid).css('opacity', '0.6');
                setopacity('tx-tc-cts-dp-' + extid,'0.4');
                //console.log('i-firing extid=' + extid + 'ref=' + id + '&rating=' + rating +'&capsess=' + capsess + '&check=' + check + '&cmd=showcomments'  + '&userpic=' + picressshow );
                jQuery.ajax({
                    type: 'POST',
                    url: 'index.php?eID=toctoc_comments_ajax',
                    async: false,
                    data: 'ref=' + id + '&rating=' + rating + '&data=' + ajaxData + '&datac=' + datac + '&datathis=' + thisdata + '&capsess=' + capsess + '&check=' + check + '&cmd=showcomments'  + '&userpic=' + picressshow + '&commentsimgs=' + commentsImgs + '&tctreestateenc=' + tctreestateenc,
                    success: function(html){
                        jQuery('#tx-tc-cts-' + extid).html(html);
                        window['previewselcomment' + cid] = 0;
                        reset_previewvars(cid);
                    }
                });
                //jQuery('#tx-tc-cts-dp-' + extid).css('opacity', '1');
                setopacity('tx-tc-cts-dp-' + extid,'1');
                jQuery.ajax({
                    type: 'POST',
                    url: 'index.php?eID=toctoc_comments_ajax',
                    async: true,
        ...
        //jQuery('#tx-tc-myrtstop-dp-' + id).css('opacity', '0.8');
        setopacity('tx-tc-myrtstop-dp-' + id,'0.8');
        //jQuery('#tx-tc-myrts-dp-' + id).css('opacity', '0.6');
        setopacity('tx-tc-myrts-dp-' + id,'0.4');
        jQuery.ajax({
            type: 'POST',
            url: 'index.php?eID=toctoc_comments_ajax',
            async: false,
        ...
        //jQuery('#tx-tc-myrtstop-dp-' + id).css('opacity', '0.6');
        setopacity('tx-tc-myrtstop-dp-' + id,'0.4');
        jQuery.ajax({
            type: 'POST',
            url: 'index.php?eID=toctoc_comments_ajax',
            async: true,
...
function toctoc_previewurl(id, previewopened, previewopenedprogress, lang, webpagepreviewheight,ajaxData,ajaxDataAtt) {
	...
	setopacity('toctoc_comments_pi1_submit_' + id,'0.4');
	//jQuery('#toctoc_comments_pi1_submit_' + id).css('opacity', '0.6');
...
function toctoc_previewurl_fetch (id, purl,pip, iterations, lang,webpagepreviewheight , ajaxData, ajaxDataAtt) {
	...
	var thumbstylemaxheight=tmpthumbheight + 25;
    ...
    if (previewopenedprogress != 2) {
    	if (iterations<30) {
	    	...
	    	jQuery.ajax({
	            ...
	            success: function(html){
			    	...
			    	if ((iterations>=1) && (html.length > 150)) {
			    		if ((htmlretcode < 4) && (posworking>2)) {
				    		if (document.getElementById('tx-tc-form-dp-wpp-' + id).style.maxHeight != (thumbstylemaxheight+'px')) {
						    	document.getElementById('formhider-' + id).style.height = eval(document.getElementById('formhider-' + id).offsetHeight + formhidermoreheight) + "px";
						    	jQuery('#tx-tc-form-dp-wpp-' + id).css('display', 'block');
			    	...
			    	if ((htmlretcode >3) ) {
			    		//console.log('1. htmlretcode: ' + htmlretcode + ', html: ' + html + ', iterations: ' + iterations);
		    			...
		    			setopacity('toctoc_comments_pi1_submit_' + id,'1');
		    			//jQuery('#toctoc_comments_pi1_submit_' + id).css('opacity', '1');
                    ...
                    if ((tctrim(htmlretcode)=='2') || (tctrim(htmlretcode)=='3') || (window['previewstarted' + id] == 2)) {
                    	...
                    	//console.log("saving ... hret: " + htmlretcode + ", iterations: " + iterations + ", previewopenedprogress: " + previewopenedprogress);
                    	...
                    	//jQuery('#toctoc_comments_pi1_submit_' + id).css('opacity', '1');
                    	setopacity('toctoc_comments_pi1_submit_' + id,'1');
                    	//console.log('2. htmlretcode: ' + htmlretcode + ', html: ' + html.substr(0,60));
                    	return true;
                    }
	            }
	        });
	    	//console.log('0. setTimeout, htmlretcode: ' + htmlretcode + ', previewopenurl: ' + previewopenurl + ', iterations: ' + iterations);
    		...
    		// console.log('3. htmlretcode: ' + htmlretcode + ', iterations: ' + iterations.substr(0,60));
    		iterations = 0;
    		previewopenedprogress = 2;
    	...
    	//console.log('4. iterations: ' + iterations + ', htmlretcodesave: ' + htmlretcodesave);
...
function toctoc_previewurl_close (id,previewopenedprogress,webpagepreviewheight,ajaxData,ajaxDataAtt) {
	...
	setopacity('tx-tc-form-dp-wpp-' + id,'1');
	//jQuery('#tx-tc-form-dp-wpp-' + id).css('opacity', '1');
...
function remove_pvs_pic(cid) {
		...
		//jQuery('#pvsimgp' + cid + 'index' + window['previewselpic' + cid]).css('display', 'none');
		...
		setopacity('toctoc_comments-pvs-image-box-p' + cid,'1');
		//jQuery('#toctoc_comments-pvs-image-box-p' + cid).css('opacity', '1');
		return 888;
}
function setopacity(elemid, opacityval) {
	document.getElementById(elemid).style.opacity=opacityval;
	document.getElementById(elemid).style.filter='alpha(opacity='+ eval(100*opacityval) +')';
}
function getopacity(elemid) {
	if (document.getElementById(elemid).style.opacity >= 0) {
		...
		if (document.getElementById(elemid).style.filter.indexOf('opacity') >= 0) {
			...
			opacityval='1';
			alert('no opacity found for ' + elemid);
		}
	}
	return opacityval;
}
function commentreply(uid, previewselcommentcid, cid){
	 
	window['previewselcomment' + cid] = uid;
	setopacity('tx-tc-ct-ry-frame-' + cid,'0');
	...
	comment_formhider(cid, 1, '', 0, 0, document.getElementById('toctoc_comments_pi1_contenttextbox_'+cid));
...
function changeavatar(gender, cid){
	if (gender==0) {
		...
		setopacity('tx-toctoc-comments-comments-img-gender-0-' + cid,'1');
		setopacity('tx-toctoc-comments-comments-img-gender-1-' + cid,'0.4');
	} else {
		document.getElementById('toctoc_comments_pi1_' + cid + 'gender').value= '1';
		setopacity('tx-toctoc-comments-comments-img-gender-0-' + cid,'0.4');
		setopacity('tx-toctoc-comments-comments-img-gender-1-' + cid,'1');
	}
}
function savect(id, icid,pid){
	var strcontent=this.toctoc_comments_pi1_base64_encode(document.getElementById('toctoc_comments_pi1_contenttextboxc_' + id).value);
	//console.log(document.getElementById('toctoc_comments_pi1_contenttextboxc_' + id).value);
	...
	var cssident = 'cts-ct';
	setopacity('tx-tc-' + cssident + '-dp-tx-tc-cts_' + id,'0.4');
   jQuery.ajax({
        type: 'POST',
        url: 'index.php?eID=toctoc_comments_ajax',
        async: false,
	...
	setopacity('tx-tc-' + cssident + '-dp-tx-tc-cts_' + id,'1');
}
...
function editct(uid, icid, pid, onoff){
	...
	if (onoff==0) {
		...
		if (editon == false) {
			...
			outhtml+='" style="overflow: hidden; height: 20px; font-size: 100%;" cols="42" autocomplete="off">' + edithtmlouturl + '</textarea></fieldset></form>';
