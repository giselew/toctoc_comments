 ...
 *   73:     function init()
 *  130:     function main()
 *  263:     protected function ipBlock()
 *
 * TOTAL FUNCTIONS: 3
...
class toctoc_comments_eID {
	...
	function init() {
		...
		if (!t3lib_div::inList('approve,approvecoi,delete,kill', $this->command)) {
					 ...
					 switch ($this->command) {
						case 'approvecoi':
							...
							if ($rowsa[0]['t'] == 0) {
								...
								$rowsa = $GLOBALS['TYPO3_DB']->exec_SELECTgetRows('COUNT(*) AS t', 'tx_toctoc_comments_comments', 'uid=' . $this->uid . ' AND approved = 1');
