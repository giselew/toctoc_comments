 ...
 *
 *
 *   53: class user_toctoc_comments_ttnews
 *   63:     function extraItemMarkerProcessor($markerArray, $row, $lConf, &$pObj)
 *  100:     function getNumberOfComments($newsUid, &$pObj)
 *  119:     function getTemplate($section, $conf, &$pObj)
 *  149:     function getItemLink($marker, $itemUid, &$pObj)
 *
 * TOTAL FUNCTIONS: 4
 ...
 */
	function extraItemMarkerProcessor($markerArray, $row, $lConf, &$pObj) {
		...
		if ($pObj->theCode == 'LIST') {
			...
			$poscommentscount= strpos($pObj->templateCode, '###TX_COMMENTS_COUNT###');
			$poscommentsplugin = strpos($pObj->templateCode, '###TX_TOCTOCCOMMENTS###');
		}
						...
						if (($template = $this->getTemplate($templateName, $lConf, $pObj))) {
							...
							$markerArray['###TX_COMMENTS_COUNT###'] = $pObj->cObj->substituteMarkerArray(
								$template, array(
									...
									'###COMMENTS_COUNT###' => sprintf($lang->sL('LLL:EXT:toctoc_comments/locallang_hooks.xml:comments_number'), $commentCount),
			...
			if (($poscommentsplugin > $beginlist) && ($poscommentsplugin < $endlist)) {
				...
				switch ($pObj->theCode) {
					...
					case 'SEARCH':
						...
						//$markerArray['###TX_TOCTOCCOMMENTS###'] =  $GLOBALS['TSFE']->tmpl->setup['plugin.']['tx_toctoccomments_pi1.']['templateFile'];
						/*  exit;
						$conftc['advanced.']= $GLOBALS['TSFE']->tmpl->setup['plugin.']['toctoc_comments_pi1.']['advanced.'];
						 */
