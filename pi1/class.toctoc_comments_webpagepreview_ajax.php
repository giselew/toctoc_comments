...
var $extKey = 'toctoc_comments';
	...
	function main() {
		...
		$outhtml .= '<div id="toctoc-comments-pvs-formtext-' . $cid . '" style="margin: 0 0 0 ' . $textdivleft . 'px; ">';
			...
			$picfoundinfo .= '</span>';
			$outhtml .= '<div class="tx-tc-pvs-title"><a href="' . $_SESSION[$cid][$commentid]['urlfound'] . '">' . $_SESSION[$cid][$commentid]['title'] . '</a>' . $picfoundinfo  . '</div>' ;
		...
		if ($_SESSION[$cid][$commentid]['description'] != '') {
			$outhtml .= '<div class="tx-tc-pvs-desc">' . $_SESSION[$cid][$commentid]['description'] . '</div>' ;
		}
		if ($_SESSION[$cid][$commentid]['title'] != '') {
			$outhtml .='</div>';
		}
