...
var $extKey = 'toctoc_comments';
	...
	function main() {
		...
		$outhtml='';
		$divlogoset=false;
		$wrkimg = '<img align="right" id="tx-tc-form-wpp-working' . trim($_POST['ref']) . '" src="/typo3conf/ext/toctoc_comments/res/css/themes/' . $conf['theme.']['selectedTheme'] . '/img/workingslides.gif" width="16" height="11" />';
		//$_SESSION['pvscont'] +=1;
				...
				//video found
					...
					if ($_SESSION[$cid][$commentid]['logo'] != '') {
						...
						if ($_SESSION[$cid][$commentid]['logo']!=$_SESSION[$cid][$commentid]['embedUrl']){
							//flash
							$videohtml .= '				<div class="tx-tc-vid"><img style="display:' . $displaystyle . '; max-height: ' . $conf['attachments.']['webpagePreviewHeight'] . 'px;" id="vidimg' . $cid . 'index1" src="' . $_SESSION[$cid][$commentid]['logo'] . '" class="tx-tc-pvs-vid-img" /></div>' ;
