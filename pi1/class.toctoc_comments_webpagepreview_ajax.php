...
private $extKey = 'toctoc_comments';
	...
	public function main() {
		...
		$wrkimg = '<img align="right" id="tx-tc-form-wpp-working' . trim($_POST['ref']) . '" src="'.$data['configBaseURL'].
					'typo3conf/ext/toctoc_comments/res/css/themes/' . $conf['theme.']['selectedTheme'] . '/img/workingslides.gif" width="16" height="11" />';
				...
				//video found
					...
					if ($_SESSION[$cid][$commentid]['logo'] != '') {
						...
						if ($_SESSION[$cid][$commentid]['logo']!=$_SESSION[$cid][$commentid]['embedUrl']){
							...
							$videohtml .= '				<div class="tx-tc-vid"><img style="display:' . $displaystyle . '; max-width: ' . $vidmaxwidth .
											'px; max-height: ' . $conf['attachments.']['webpagePreviewHeight'] . 'px;" id="vidimg' . $cid . 'index1" src="' .
							...
							$videohtml .= '				<video width="140" style="display:' . $displaystyle . '; margin: 0 -4px 0 -8px; max-height: ' .
		...
		if (!array_key_exists('embedUrl', $_SESSION[$cid][$commentid])) {
			...
			if (count($_SESSION[$cid][$commentid]['images']) > 0) {
				...
				$outhtml .= '<div class="tx-tc-pvs-images"  id="toctoc_comments-pvs-images-' . $cid . '" style="border-right: 1px solid #D4D4D4; float: left; width: ' .
								$websitepreviewareaimagewidth . 'px; height: ' . $conf['attachments.']['webpagePreviewHeight'] .
								'px;"><div id="toctoc_comments-pvs-image-box-' .
								$cid . '"  style="filter: alpha(opacity=100); opacity:1;">';
				$outhtmlpic ='';
				...
				for ($i = 0; $i < $cntimg; $i++) {
					...
					$outhtmlpic .= '<img style="display:' . $displaystyle . ';" id="pvsimg' . $cid . 'index' . $i . '"src="' .
									$_SESSION[$cid][$commentid]['images'][$i]['locallink'] . '" class="tx-tc-pvs-img" />';
				}
		...
		if ($_SESSION[$cid][$commentid]['title'] != '') {
			if (!array_key_exists('embedUrl', $_SESSION[$cid][$commentid])) {
				...
				If ($_SESSION[$cid][$commentid]['totalcounter']>0) {
					...
					$picfoundinfo = '<span style="display:block;" id="toctoc-picfoundinfo-' . $cid .'"> (' . $_SESSION[$cid][$commentid]['totalcounter'] .' ' .
									$txtimage . ')';
				}
		...
		if (!array_key_exists('embedUrl', $_SESSION[$cid][$commentid])) {
			if ($divlogoset) {
				$outhtml .= '</div>';
				$outhtml .= '<div class="tx-tc-pvs-logobg"  id="tx-tc-pvs-logobg-' . $cid .'" style="margin-top: -' . (7+$conf['attachments.']['webpagePreviewHeight']) .
							 	'px; " >';
				$outhtml .= '<img src="' . $_SESSION[$cid][$commentid]['logo'] . '" style="max-height: ' . $conf['attachments.']['webpagePreviewHeight'] .
								'px; max-width:100px; float:right; margin-top: 7px; " />';
				$outhtml .= '</div>';
			}
