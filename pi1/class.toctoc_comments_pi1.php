 ...
 *
 *
 *   89: class tx_toctoccomments_pi1 extends tslib_pibase
 *  153:     public function main($content, $conf, $hookTablePrefix = '', $hookId = 0, $hookcObj = NULL)
 * 1980:     protected function checkJSLoc()
 * 2079:     protected function checkCSSTheme()
 * 2194:     protected function checkCSSLoc()
 * 2327:     protected function initprefixToTableMap()
 * 2364:     protected function init($newconfigcsswaswritten = 0)
 * 2851:     protected function mergeConfiguration()
 * 3109:     protected function fetchConfigValue($param)
 * 3138:     private function createLinks($text)
 * 3160:     private function applyStdWrap($text, $stdWrapName)
 * 3173:     protected function ae_detect_ie()
 * 3197:     protected function boxmodel($newconfigcsswaswritten = 0)
 * 3617:     protected function calculate_string( $mathString )
 * 3639:     protected function locationHeaderUrlsubDir()
 * 3658:     protected function currentPageName()
 * 3686:     protected function ttclearcache ($pid, $withplugin=TRUE, $withcache = FALSE, $debugstr = '')
 * 3717:     protected function doClearCache ($forceclear=FALSE)
 * 3751:     protected function getPluginCacheControlTstamp ($external_ref_uid)
 * 3775:     protected function getLastUserAdditionTstamp ()
 * 3798:     protected function initLegacyCache ()
 * 3812:     protected function check_scopes()
 *
 * TOTAL FUNCTIONS: 21
...
class tx_toctoccomments_pi1 extends tslib_pibase {
	...
	protected function checkJSLoc() {
		...
		$jscontent .= 'var confloginRequireduseFancyBox = ' . intval($this->conf['theme.']['loginRequireduseFancyBox']) . ';' . "\n";
	...
	protected function checkCSSTheme() {
			...
			$txdirname= str_replace('/', DIRECTORY_SEPARATOR, str_replace($repstr, '', dirname(__FILE__)) . $dirsep . t3lib_extMgm::siteRelPath('toctoc_comments') .
						'res/css/themes/' . $this->conf['theme.']['selectedTheme'] . '/css/' );
			...
			if (file_exists($filenamecss)) {
				...
				$retstr =$this->pi_wrapInBaseClass($this->lib->pi_getLLWrap($this, 'error', FALSE) . ': ' .
						$this->lib->pi_getLLWrap($this, 'error.no.css.theme', FALSE)) . ': ' . $filenamecss;
				return $retstr;
			}
	...
	protected function checkCSSLoc() {
		...
		$txdirname= str_replace('/', DIRECTORY_SEPARATOR, str_replace($repstr, '', dirname(__FILE__)) . $dirsep .
					t3lib_extMgm::siteRelPath('toctoc_comments') . 'res/css/' );
...
.tx-tc-pi1 .tx-tc-cts-form-fieldtextarea-1 {
	 ...
	 */
	protected function init($newconfigcsswaswritten = 0) {
		if ($this->showsdebugprint==TRUE) {
		...
		if (!isset($GLOBALS['TSFE']->additionalHeaderData[$key])) {
			...
			if ($headerParts) {
				...
				$tcsmiliesenchtml='';
				$tcsmiliesenchtml='<script type="text/javascript">var tcsmiliecard ="";
									</script>';
				...
				if (($this->conf['advanced.']['loginRequired'] == 1)) {
	 ...
	 */
	 private function createLinks($text) {
		if ($this->conf['advanced.']['autoConvertLinks']) {
			$textout= preg_replace('/((https?:\/\/)?((?(2)([^\s]+)|(www\.[^\s]+))))/',
						'<a href="http://\3" rel="nofollow" class="tx-toctoc-comments-external-autolink">\1</a>', $text);
	 ...
	 */
	private function applyStdWrap($text, $stdWrapName) {
		...
		if (file_exists($filenameboxmodelsystem)) {
			...
			if (file_exists($filenamedefaultcss)) {
				...
				if (($forceregenerate==TRUE) || ($bmcsslastmodif<$bmtxtlastmodif) || ($bmcsslastmodif<$basecsslastmodif) ||
						($bmcsslastmodif<$newconfigcsswaswritten)) {
