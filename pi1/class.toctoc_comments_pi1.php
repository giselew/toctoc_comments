 ...
 *  155:     public function main($content, $conf, $hookTablePrefix = '', $hookId = 0, $hookcObj = null)
 * 1744:     protected function checkJSLoc()
 * 1837:     protected function checkCSSTheme()
 * 1928:     protected function checkCSSLoc()
 * 2053:     protected function initprefixToTableMap()
 * 2085:     function init($newconfigcsswaswritten = 0)
 * 2514:     function mergeConfiguration()
 * 2758:     function fetchConfigValue($param)
 * 2781:     function createLinks($text)
 * 2800:     function applyStdWrap($text, $stdWrapName)
 * 2812:     protected function ae_detect_ie()
 * 2833:     protected function boxmodel($newconfigcsswaswritten = 0)
 * 3188:     protected function calculate_string( $mathString )
 * 3209:     protected function locationHeaderUrlsubDir()
 * 3225:     protected function currentPageName()
 * 3251:     protected function ttclearcache ($pid, $withplugin=TRUE, $withcache = FALSE, $debugstr = '')
 * 3278:     protected function doClearCache ($forceclear=FALSE)
 * 3307:     protected function getPluginCacheControlTstamp ($external_ref_uid)
 * 3329:     protected function getLastUserAdditionTstamp ()
 * 3350:     protected function initLegacyCache ()
 * 3362:     protected function check_scopes()
 *
 * TOTAL FUNCTIONS: 21
...
class tx_toctoccomments_pi1 extends tslib_pibase {
	...
	var $maxtimeafterinsert = 1400;			   // time in milliseconds the system waits until considering a submit as new total transaction
...
// 		if (isset($_SESSION['maxtimeafterinsert'])) {
// 			$this->maxtimeafterinsert=$_SESSION['maxtimeafterinsert'];
// 		}
 ...
 */
		...
		if ($_SESSION['commentsPageId'] != $GLOBALS['TSFE']->id) {
			...
			/* if ($loginreset==TRUE) {
				...
				$loginreset=FALSE;
				$this->doClearCache();
				$this->sdebugprint .= 'Init Sessionvariables because of logout/in on page id, (feuserid case) ' .$GLOBALS['TSFE']->id. '<br>';
		...
		while ($wrki==0) {
			...
			if ($_SESSION['indexOfSortedCommentsCidList']>count($wrkrowscidflex)) {
				...
				if ($optionalRecordIdlhookTablePrefix!='') {
					...
					if ($this->showsdebugprint==TRUE) {
						$debugnocidfoundtext='<p><b>Additional information on detected content element IDs:</b><br>All content elements: ' . json_encode($wrkrowscidflex) .
