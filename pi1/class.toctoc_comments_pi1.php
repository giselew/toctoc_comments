 ...
 *
 *
 *   68: class tx_toctoccomments_pi1 extends tslib_pibase
 *  107:     function main($content, $conf)
 *  643:     function init()
 *  714:     function mergeConfiguration()
 *  818:     function fetchConfigValue($param)
 *  841:     function createLinks($text)
 *  860:     function applyStdWrap($text, $stdWrapName)
 *
 * TOTAL FUNCTIONS: 6
 ...
 */
		...
		if (t3lib_extMgm::isLoaded('comments_report')) {
			$conftx_commentsreport = &$GLOBALS['TSFE']->tmpl->setup['plugin.']['tx_commentsreport_pi1.'];
			$this->conf['tx_commentsreport_pi1.']['templateFile']=$conftx_commentsreport['templateFile'];
			...
			$_SESSION['commentsPageIds'][$this->conf['tx_commentsreport_pi1.']['reportPid']] = $reportpage;
		}
		...
		if ($_SESSION['commentsPageId'] != $GLOBALS['TSFE']->id){
			...
			$_SESSION['activelangid'] =$GLOBALS['TSFE']->sys_language_uid;
		} elseif ($_SESSION['feuserid'] != $TSFE->fe_user->user['uid']) {
		...
		if (!$this->foreignTableName) {
			return sprintf($this->lib->pi_getLLWrap('error.undefined.foreign.table', false), $this->prefixId, $this->conf['externalPrefix']);
		}
