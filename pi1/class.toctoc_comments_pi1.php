 ...
 *  997:     protected function checkJSLoc()
 * 1062:     protected function checkCSSTheme()
 * 1150:     protected function checkCSSLoc()
 * 1230:     protected function initprefixToTableMap()
 * 1262:     function init()
 * 1523:     function mergeConfiguration()
 * 1680:     function fetchConfigValue($param)
 * 1703:     function createLinks($text)
 * 1722:     function applyStdWrap($text, $stdWrapName)
 * 1734:     protected function ae_detect_ie()
 * 1753:     protected function boxmodel()
 * 2089:     protected function calculate_string( $mathString )
 *
 * TOTAL FUNCTIONS: 13
 ...
 */
		...
		if (($this->conf['externalPrefix'] != 'pages') && ($this->lhookTablePrefix == '')) {
			//print $this->conf['externalPrefix'];exit;
		...
		if ($hookTablePrefix != '') {
			...
			$this->lhookId=$hookId;
			$hookTablePrefix = '';//print $isPlugin . ' - ' . $this->lhookId . ' - ' . $this->lhookTablePrefix;exit;
		}
...
// 		if (($this->conf['externalPrefix'] != 'pages') && ($this->lhookTablePrefix == '')) {
// 			$_SESSION['commentListCount']=$_SESSION['commentListCount'].$contentelementMultiReference;
// 		}
		...
		if ($this->conf['pluginmode'] == '') {
			if ((t3lib_div::_GP('typo3_user_int')==1) || ($_GET['toctoc_comments_pi1']['anchor'])) {
		...
		if (!isset($GLOBALS['TSFE']->additionalHeaderData[$key])) {
			...
			if ($headerParts) {
				...
				if ($this->conf['advanced.']['useEmoji']>0) {
					$emojijs='<script  type="text/javascript" src="'. t3lib_extMgm::siteRelPath('toctoc_comments') .'res/js/tx-tc-premojify.js"></script>';
				}
				if ($this->conf['advanced.']['useEmoji']==1) {
					$emojicss='<link href="'. t3lib_extMgm::siteRelPath('toctoc_comments') .'res/css/emoji/emoji16.css" rel="stylesheet" type="text/css"/>';
				} elseif ($this->conf['advanced.']['useEmoji']==2) {
					$emojicss='<link href="'. t3lib_extMgm::siteRelPath('toctoc_comments') .'res/css/emoji/emoji20.css" rel="stylesheet" type="text/css"/>';
				}
