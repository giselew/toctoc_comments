 ...
 *
 *
 *   66: class tx_toctoccomments_pi1 extends tslib_pibase
 *  104:     function main($content, $conf)
 *  616:     function init()
 *  687:     function mergeConfiguration()
 *  791:     function fetchConfigValue($param)
 *
 * TOTAL FUNCTIONS: 4
 ...
 */
			...
			if  ($this->extConf['updateMode']) {
				...
				if (($checktxratings) && ($checktxcomments)) {
					...
					foreach ($rows as $row) {
						...
						$commentsuid=trim(substr($row['reference'],12,30));
		...
		$content = '';
		return $this->lib->maincomments($this->ref, $this->conf, false, $_SESSION['commentsPageId'], $_SESSION['feuserid'] , 'commentdisplay', $this, $this->piVars, $this->cObj);
