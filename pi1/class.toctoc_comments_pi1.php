 ...
 *
 *
 *   81: class tx_toctoccomments_pi1 extends tslib_pibase
 *  142:     public function main($content, $conf, $hookTablePrefix = '', $hookId = 0, $hookcObj = null)
 * 1502:     protected function checkJSLoc()
 * 1581:     protected function checkCSSTheme()
 * 1672:     protected function checkCSSLoc()
 * 1754:     protected function initprefixToTableMap()
 * 1786:     function init($newconfigcsswaswritten = 0)
 * 2109:     function mergeConfiguration()
 * 2267:     function fetchConfigValue($param)
 * 2290:     function createLinks($text)
 * 2309:     function applyStdWrap($text, $stdWrapName)
 * 2321:     protected function ae_detect_ie()
 * 2341:     protected function boxmodel($newconfigcsswaswritten = 0)
 * 2649:     protected function calculate_string( $mathString )
 * 2670:     protected function locationHeaderUrlsubDir()
 * 2686:     protected function currentPageName()
 * 2706:     protected function ttclearcache ($pid, $withplugin=true, $withcache = false, $debugstr = '')
 * 2731:     protected function doClearCache ($forceclear=false)
 * 2757:     protected function getPluginCacheControlTstamp ($external_ref_uid)
 * 2778:     protected function initLegacyCache ()
 *
 * TOTAL FUNCTIONS: 19
	...
	public function main($content, $conf, $hookTablePrefix = '', $hookId = 0, $hookcObj = null) {
		...
		if ($this->showsdebugprint==true) {
			...
			if ($this->sdebugprintuser==intval($GLOBALS['TSFE']->fe_user->user['uid'])) {
				...
				if ($tdifftolastrun<=200000) {
					//Not if _SESSION['edgeTime'] is 0
					$timereportlast='<div class="tx-tc-debug">Time since last rendering: ' . round($tdifftolastrun,0) .  'ms</div>';
				}
				$tdifftolastrun = 1000*(microtime(true) - $_SESSION['edgeTime']);
			}
		}
 ...
 */
		...
		if (intval($this->conf['advanced.']['activateClearPageCache'])==1) {
			...
			//$this->conf['advanced.']['dateFormatOldStyle'] = 1;
		}
		...
		if (intval($tdiff) > intval($this->maxtimeafterinsert)) {
			...
			//$printrendering .= 'Index of sorted Pluginslist is at ' . $_SESSION['indexOfSortedCommentsCidList'] . ', Plugin count: ' . (count($_SESSION['rowscidflex'])-1);
		...
		if (intval($this->conf['pluginmode'])==0) {
			...
			if (($this->lhookTablePrefix == '') && ($errornocidfound==true) && (intval($this->conf['pluginmode'])==0)) {
				...
				return sprintf('<div class="tx-tc-form-top-message tx-tc-required-error"><p>' . $echodate . '<br />' . $this->lib->pi_getLLWrap($this, 'error.automaticcidfail', false)  . '</p></div>', $this->conf['advanced.']['UseMainColPos'], $this->conf['advanced.']['UseTemplavoilaField']);
			}
		...
		if (intval($this->conf['pluginmode'])==0) {
			if (isset($_GET['toctoc_comments_pi1']['anchor'])) {
				...
				if ($_SESSION['findanchor'] != '0') {
					//$this->ttclearcache($GLOBALS['TSFE']->id,true, true);
				...
				if ($this->showsdebugprint) {
					if ($this->sdebugprintuser==intval($GLOBALS['TSFE']->fe_user->user['uid'])) {
						if ((intval($this->conf['advanced.']['wallExtension']) == 0)) {
							$this->sdebugprint .= '<b>Cache dropped</b> for ' . $_SESSION['commentListRecord'] . ', L: ' . $_SESSION['activelang'] . ', Userid: ' . intval($_SESSION['feuserid']) .'<br>';
						} else {
							$this->sdebugprint .= '<b>No Cache Access on Wall</b> for ' . $_SESSION['commentListRecord'] . ', L: ' . $_SESSION['activelang'] . ', Userid: ' . intval($_SESSION['feuserid']) .'<br>';
						}
					}
				}
			}
			if ($domemcache == true) {
				if ($this->conf['theme.']['selectedBoxmodel'] != $_SESSION['activeBoxmodel']) {
					...
					if ($this->showsdebugprint) {
						if ($this->sdebugprintuser==intval($GLOBALS['TSFE']->fe_user->user['uid'])) {
							$this->sdebugprint .= '<b>Cache dropped and recacheing</b> (new boxmodel ' . $this->conf['theme.']['selectedBoxmodel'] . ') for ' . $_SESSION['commentListRecord'] . ', L: ' . $_SESSION['activelang'] . ', Userid: ' . intval($_SESSION['feuserid']) .'<br>';
						}
					}
				}
				if ($_SESSION['renderingdone']==true) {
			...
			if ($domemcache == false) {
				// access to lib
				$outml=$this->lib->maincomments($this->ref, $this->conf, false, $_SESSION['commentsPageId'], $_SESSION['feuserid'] , 'commentdisplay', $this, $this->piVars);
			...
			$timereport='';
			$starttimedebuglib=microtime(true);
			$outml=$timereportlast .$this->lib->getRecentComments($this, $this->conf,$_SESSION['feuserid']);
			if ($this->showsdebugprint) {
				if ($this->sdebugprintuser==intval($GLOBALS['TSFE']->fe_user->user['uid'])) {
	...
	function init($newconfigcsswaswritten = 0) {
		...
		if (!isset($GLOBALS['TSFE']->additionalHeaderData[$key])) {
			...
			if ($headerParts) {
				...
				list($row) = $GLOBALS['TYPO3_DB']->exec_SELECTgetRows('MIN(uid) AS minuid, MAX(uid) AS maxuid',
						'tx_toctoc_comments_comments', $rangenewcommentswhere);
				$mincommentid=intval($row['minuid']);
				$maxcommentid=intval($row['maxuid'])+50; // allow 50 new comments on cached pages by same user ... should do it...
				...
				if ($this->showsdebugprint==true) {
					$endtimedebug2=microtime(true);
					$this->sdebuginitprint.='Header: ' . round(1000*($endtimedebug2 - $starttimedebug2),1) .', ' ;
