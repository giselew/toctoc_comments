 ...
 *   70:     public function main($files,$post)
 *  429:     protected function create_thumbnail_frompdf($pdffile,$pdffilejpg, $impath)
 *
 * TOTAL FUNCTIONS: 2
...
class toctoc_comments_attachmentupload {
	...
	public function main($files,$post) {
		...
		if (isset($_POST['data'])) {
			$dataarr = unserialize(base64_decode($_POST['data']));
			print '$file: ' . $_POST['file']['type'] . ' $dataarr: ';print_r($dataarr);exit;
		} else
		{
			//print_r($files);print_r($_POST);exit;
		}
	...
	// Logo won't be resized'
		$arrnew_width=array();
		$arrnew_height=array();
		$savepath = 'uploads/tx_toctoccomments/';//return print_r ($files);
		$s=$files['myfile']['name'];
		...
		if ($files['myfile']['type'] =='application/pdf') {
			...
			if (intval($width)>$this->conf['attachments.']['picUploadMaxDimX']){
				...
				if (intval($arrnew_height[1])>$this->conf['attachments.']['picUploadMaxDimY']){
					...
					$arrnew_width[1]=intval($width*($this->conf['attachments.']['picUploadMaxDimY']/$arrnew_height[1]));
		...
		$arrnew_height[3]=0;
			$arrnew_width[3]=0;
		/* 		$arrnew_height[2]=$arrnew_height3;
			$arrnew_width[2]=$arrnew_width3; */
		...
		if ($files['myfile']['type'] =='application/pdf') {
			...
			if (!copy($copyfromfile, $copytofile)) {
				return print("failed imagescopy for $copyfromfile to $copytofile\n");
			}
			$pdffilejpg=$savepathfilename;
			//return print $post['toctoc_comments_pi1']['pathim'];
			$pdffilejpg = $this->create_thumbnail_frompdf($copytofile,$pdffilejpg,$impath);
			if ($pdffilejpg==''){
				$pathirname='/typo3conf/ext/toctoc_comments/res/img/';
...
//			$idstr = $idstr . 'IMGcid' . $post['toctoc_comments_pi1']['cid'] . 'v' . $_SESSION['uploadstep'] . '.' .$ext;
			...
			if ((strtoupper($ext) =='JPG') || (strtoupper($ext) =='JPEG')) {
				...
				return '$strtoupper($ext)==' .strtoupper($ext) ;
			}
					...
					if ($i<3){
						...
						switch(strtolower($ext)) {
							...
							case "jpeg":
								...
								break;
							default:
								return 'strtolower($ext)' . strtolower($ext);
