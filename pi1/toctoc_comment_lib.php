 ...
 *
 *
 *  224: class toctoc_comment_lib extends tslib_pibase
 *  293:     public function maincomments($ref, $conf = NULL, $fromAjax = FALSE, $pid=0, $feuserid = 0, $cmd = '', &$pObj, $piVars = array(),
 ...
 *  593:     protected function checkExternalUid($conf, $pObj)
 *
 *              SECTION: Commenting functions
 *  621:     public function comments($conf,&$pObj, $fromAjax, $feuserid, $pid)
 * 2223:     protected function makeemoji($text, $conf, $debug)
 * 2371:     public function comments_getComments(&$rows, $conf, $pObj, $feuserid, $fromAjax, $pid, $levelhlt=0)
 * 3385:     protected function comments_getComments_getRatings(&$row, $conf, $pObj, $feuserid, $fromAjax)
 * 3406:     protected function comments_getComments_getEmail($email)
 * 3422:     protected function isCommentingClosed($conf, $pObj)
 * 3475:     protected function commentingClosed($pObj, $fromAjax, $conf)
 * 3498:     protected function comments_getCommentsBrowser($rpp, $startpoint, $totalrows, $pObj, $fromAjax, $conf)
 * 3626:     protected function build_AJAXImages ($conf, $pObj, $usergenderexistsstr='', $fromAjax=FALSE)
 * 3824:     public function comments_getComments_fe_user($params, $conf, $pObj, $commentid, $fromAjax, $commentusername)
 * 3936:     protected function fixFeUserPic ($pic)
 * 3959:     protected function addleadingspace($text)
 * 3988:     protected function check_comment_ownership ($rowexternal_ref, $feuserid)
 * 4010:     public function previewcomment($data, $conf)
 *
 *              SECTION: functions for images cache (AJAX)
 * 4039:     protected function setAJAXimage($image, $feuserid)
 * 4050:     protected function getAJAXimage($feuserid, $commentid)
 * 4084:     protected function setAJAXimageCache($image, $imageoriginal, $feuserid)
 * 4095:     protected function getAJAXimageCache($commentuserimageout)
 * 4110:     protected function makeImageSprite($conf)
 *
 *              SECTION: Form functions
 * 4305:     private function form($conf, &$pObj, $piVars, $fromAjax, $pid, $ifeuserid=0, $userpic, $commentid=0, $replylevel=0,
			$fromcomments = FALSE)
 * 5195:     protected function form_updatePostVarsWithFeUserData($conf, $piVars, $feuserid, $fromAjax, $userpic, $cid, $output_cid)
 * 5538:     protected function form_getCaptcha($pObj, $conf, $fromAjax)
 * 5596:     protected function form_wrapError($field, $pObj, $conf)
 *
 *              SECTION: Comments page-browser functions
 * 5633:     protected function processBrowserSubmission($conf, $pObj, $cmd, $ref)
 *
 *              SECTION: Inserting and Updating Comments functions
 * 5702:     public function processSubmission($conf, $pObj, $piVars, $fromAjax, $feuserid, $pid, $lang)
 * 6180:     public function updateComment($conf, $pObj, $ctid, $content, $pid, $plugincacheid)
 * 6257:     protected function processSubmission_checkTypicalSpam($pObj, $conf, $piVars, $lang)
 * 6330:     protected function processSubmission_validate($piVars, $conf, $pObj, $fromAjax)
 * 6405:     public function sendNotificationEmail($uid, $plugincacheid, $points, $action, $conf, $pObj, $fromAjax, $piVars, $pid, $fetoctocusertoinsert,
			$attachment_id=0, $attachment_subid=0, $optinemail='', $ratingpageid=0)
 * 6915:     protected function emailOptIn($conf, $optinemail, $optin_ip)
 * 6992:     protected function checkCOI($conf, $email, $checkip=TRUE)
 *
 *              SECTION: language handling functions
 * 7028:     public function fixLL(&$conf)
 * 7053:     protected function fixLL_internal($LL, &$ll, $prefix = '')
 *
 *              SECTION: mostly formatting functions
 * 7078:     protected function createLinks($text, $conf)
 * 7102:     protected function applyStdWrap($text, $stdWrapName, $conf = NULL)
 * 7125:     protected function checkCustomFunctionCodes($code, $pObj)
 * 7150:     protected function isNoCacheUrl($url)
 * 7172:     protected function substituteMarkersAndSubparts($template, array $markers, array $subparts, $pObj)
 *
 *              SECTION: session and cache functions
 * 7199:     public function resetSessionVars($resetcontext, $alsoajaxvar = TRUE)
 * 7250:     public function getClearCacheIds($conf, $pid = 0, $repectsession = TRUE)
 * 7317:     protected function getClearCacheExternal_ref_uids($external_ref_uid = '')
 * 7343:     public function setPluginCacheControlTstamp ($external_ref_uid_list)
 * 7420:     public function initCaches()
 * 7476:     protected function clearPagesCaches($conf, $pid, $plugincacheid)
 *
 *              SECTION: Smilie functions
 * 7514:     public function parseSmilieArray($data)
 * 7531:     public function replaceSmilies($content, $conf)
 * 7551:     protected function checkbbcrop($content, $commentCropLength)
 * 7582:     protected function replaceBBs($content, $purge=FALSE)
 *
 *              SECTION: functions for the AJAx interface with jQuery/JavaScript
 * 7627:     protected function getAjaxJSData($feuserid, $pid, $languagecode, $conf, $pObj, $cid, $fromAjax, $dataonly=FALSE, $externalref)
 * 7696:     protected function getAjaxData($feuserid, $pid, $languagecode, $conf, $cid, $ref)
 * 7737:     protected function updateAjaxData($feuserid)
 * 7756:     protected function getAjaxLoggedInData($forjsvariable, $outputloginstate)
 * 7817:     protected function getAjaxDataAttachments($conf, $fromAjax, $pObj)
 * 7839:     protected function getAjaxJSDataCommentImgs($cid, $fromAjax)
 * 7879:     protected function getAjaxJSDataComments($cid, $pObj, $fromAjax)
 * 7921:     protected function getAjaxDataComments($pObj)
 * 7938:     protected function getAjaxDataImgs()
 *
 *              SECTION: URL- and IP-related functions
 * 7955:     public function getCurrentIp()
 * 7971:     public function hasValidItemUrl($piVars)
 * 7995:     public function getDefaultConfig($pluginkey='')
 * 8010:     protected function ae_detect_ie()
 *
 *              SECTION: Rating functions
 * 8041:     public function getRatingDisplay($ref, $conf = NULL, $fromAjax = 0, $pid=0, $returnasarray = FALSE, $feuserid = 0, $cmd = 'vote',
			$pObj = NULL, $cid, $fromcomments, $commentspics = array(), $scopeid=0)
 * 8099:     public function isVoted($ref, $pObj, $scopeid=0, $feuserid=0)
 * 8137:     protected function getBarWidth($rating, $conf)
 * 8152:     protected function getRatingInfo($ref, $pObj, $feuserid=-1, $conf, $scopeid=0)
 * 8296:     protected function generateRatingContent($ref, $template, $conf, $fromAjax, $pid, $returnasarray, $feuserid, $pObj, $cmd, $cid, $fromcomments,
	$commentspics, $scopeid = 0)
 * 9260:     private function makeiLikeText($conf, $pObj, $cmd, $ref, $fromAjax, $myrating = array(), $mylikeval, $mydis='', $template, $cid, $commentspics, $extpreffortext)
 *
 *              SECTION: TYPO3 workarounds
 * 9697:     public function enableFields($tableName, $pObj)
 * 9717:     public function pi_getLLWrap($pObj, $llkey, $fromAjax)
 * 9756:     protected function t3getSubpart ($pObj, $templateCode, $templateMarker)
 * 9780:     protected function t3substituteMarkerArray ($content, $markContentArray)
 * 9802:     protected function t3substituteMarker ($template, $marker, $markContent)
 * 9816:     protected function t3fileResource ($pObj, $usetemplateFile)
 * 9833:     protected function getcheck ($ref, $i, $ratingscheck)
 * 9857:     protected function formatDate($date, $pObj, $fromAjax, $conf)
 * 9993:     protected function getPageURL($fromAjax = FALSE, $pid = 0)
 * 10019:     protected function locationHeaderUrlsubDir($withleadingslash = TRUE)
 * 10044:     protected function getCleanHTML ($html)
 *
 *              SECTION: Usercards
 * 10073:     public function getSmiliesCard($conf)
 * 10384:     public function getUserCard($basedimgstr, $basedtoctocuid, $conf, $pObj, $commentid)
 *
 *              SECTION: Mail functions
 * 10748:     protected function getMailTo($mailAddress, $linktxt = '', $initP = '?')
 * 10795:     protected function encryptEmail($string, $back=0)
 * 10837:     protected function encryptCharcode($n, $start, $end, $offset)
 * 10861:     private function send_mail ($toEMail, $subject, $message, $html, $fromEMail, $fromName, $attachment='')
 * 11032:     protected function slashName ($name, $apostrophe='"')
 *
 *              SECTION: Commentator-Notification functions
 * 11056:     public function handleCommentatorNotifications($uid, $conf, $pObj, $fromeID = FALSE, $pid=0, $fromAjax=1)
 *
 *              SECTION: Administrator comment response functions
 * 11263:     protected function getCommentsReponse(&$params, $conf)
 *
 *              SECTION: Website preview functions
 * 11306:     public function cleanupfup($uploadedfile, $conf, $originalfilename)
 * 11325:     public function getwebpagepreview($cmd, $cid, $data, $pObj, $conf)
 * 11367:     protected function getpreviewinit($cid, $data, $pObj)
 * 11406:     protected function savewebpagepreviewtodb($pcid, $pcommentid, $conf)
 * 11645:     protected function cleanupdbandfiles($conf, $uploadedfile='', $originalfilename='')
 * 11797:     protected function getwebpagecache($pcid, $pcommentid, $conf, $url='', $isbeforefetch = FALSE)
 * 11966:     protected function read_dir($dir, $array = array())
 * 11995:     protected function commentShowWebpagepreview ($rowattachmentid, $rowattachment_subid, $conf, $pObj, $cid, $topwebsitepreview, $fromAjax,
			$row = array(), $isforemailnotification = FALSE)
 * 12488:     protected function makeAttachementPicture($picturefilename, $conf, $descriptionbyuser, $originalfilename, $firstname, $lastname, $fetoctocusertoinsert)
 *
 *              SECTION: eID-Interface functions
 * 12667:     public function handleeID($uid, $conf, $pObj, $messagetodisplay, $refreshurl)
 *
 *              SECTION: Reply on comments functions
 * 12771:     protected function getCommentBoxDisplay($commentid, $conf, $level, $fromAjax, $triggeredlevel=0, $levelexpandoverride=0)
 * 12847:     protected function getCommentBoxChildrenDisplayIsCollapsed($commentschildrenids, $conf, $level, $fromAjax, $triggeredlevel=0, $levelexpandoverride=0)
 *
 *              SECTION: Fe-usergroup functions
 * 12919:     public function usersGroupmembers($pObj, $fromAjax, $conf, $communitybuddies = FALSE)
 * 12999:     protected function isUserOnline($feuser_uid)
 *
 *              SECTION: Recent comments functions
 * 13040:     public function getRecentComments($pObj, $conf, $feuserid)
 * 13104:     protected function comments_getRecentComments($rows, $conf, $pObj)
 * 13412:     protected function trimContent($text, $conf, $maxChars=0, $dospecialChars=TRUE)
 * 13454:     protected function createRCLinks($text, $refID, $commentID, $prefix, $externalprefix, $singlePid, $conf, $show_uid, $okrowsi)
 *
 *              SECTION: Report comments functions
 * 13516:     protected function getCommentsReportLink($params, &$pObj, $fromAjax, $pid)
 * 13564:     public function mainReport($content, $conf, $pObj, $piVars)
 * 13590:     protected function processReportForm(array &$errors, $conf, $pObj, $piVars)
 * 13742:     protected function showReportThanks($pObj)
 * 13757:     protected function showReportForm(array $errors, $conf, $pObj, $piVars)
 * 13846:     protected function getReportCaptcha($required, $error, $conf, $pObj)
 *
 *              SECTION: IP-blocking functions
 * 13895:     protected function IPBlockSpamCheck(&$pObj)
 * 13913:     protected function sendNotificationIPBlock($params)
 * 13927:     protected function checkSMTPService($hostname, $port)
 * 13948:     private function getIpAddr()
 * 13964:     private function checkTableBLs($ipaddr)
 * 13975:     private function checkLocalBL($ipaddr)
 * 13988:     private function checkStaticBL($ipaddr)
 * 14020:     private function checkNetworkBLs($ipaddr)
 * 14045:     protected function trackdebug ($trackingfunction)
 *
 *              SECTION: Top ratings functions
 * 14072:     public function showtopRatings($conf, $pObj)
 * 14087:     protected function topratings ($conf, $pObj)
 *
 * TOTAL FUNCTIONS: 124
...
class toctoc_comment_lib extends tslib_pibase {
	...
	private $expandiconCSSmargin = 'margin: 6px 0 0 4px;'; //                *** fixed here, apart for boxmdel koogle (margin of the icon for
																				// expanding relpies on comments)
	private $firstCSSmargin='margin: 0 4px;';  //                            *** fixed here, used for eID-requests, f(handleeID), it's the margin
																				// of the "processing-image"
	private $picuploadCSSmargin='margin: 0 4px;';  //picupload               *** fixed here, apart for boxmdel koogle (attachments, margin of image preview pics)
	private $pdfuploadCSSmargin='margin: 2px 4px 0;';  //PdfUpload           *** fixed here, apart for boxmdel koogle (attachments, margin of pdf preview pics)
	private $wpplogoCSSmargin='margin: 4px 0 0;';					//       *** fixed here (margin of logos in webpage previews (wpp)
	private $userimagesize = 96;	//3*18;                                  *** fixed here (size of generated user pic, it will be downsized by CSS later
	private $userimagesizeform = 96; //3*18;                                 *** fixed here
	private $levelindent=22; //32/2+6                                        *** will be recalculaded later as f(boxmodelLevelIndent, UserImageSize, boxmodelSpacing)
	private $levelindentbordersize = '1px solid';               //           *** fixed here
	private $levelindentCSSborderradius='border-radius: 10px 4px 0 0;';//   *** fixed here
	...
	public function maincomments($ref, $conf = NULL, $fromAjax = FALSE, $pid=0, $feuserid = 0, $cmd = '', &$pObj, $piVars = array(),
		...
		if (count($confSess)>0) {
			...
			$_SESSION['lastTotalrows']= $confSess['lastTotalrows'];
			$_SESSION['lastStartpoint']= $confSess['lastStartpoint'];
			if (intval($confSess['lastStartpoint'])!=0) {
				$pObj->totalrows= $confSess['lastTotalrows'];
				$pObj->startpoint= $confSess['lastStartpoint'];
			}
		...
		} elseif ($conf['theme.']['boxmodelLevelIndent']==2) {
			$this->levelindent=round($conf['UserImageSize']/2)+(3*round($conf['theme.']['boxmodelSpacing']/2));
		} else {
			$this->levelindent=round($conf['theme.']['boxmodelSpacing']);
		}
		...
		if ($fromAjax === TRUE) {
			...
			if ($this->checkExternalUid($conf, $pObj)) {
				...
				if ($cmd === 'showform') {
					...
					$userAJAXimagarr2[0]=$_SESSION['commentListCount'];
				...
				if ($cmd === 'updateAJAXdata') {
					$content .= $this->updateAjaxData($feuserid);
				}

				if (($cmd === 'browse') || ($cmd === 'browsehide')) {
					$this->processBrowserSubmission($conf, $pObj, $cmd, $ref);
					$_SESSION['lastTotalrows']= $pObj->totalrows;
					$_SESSION['lastStartpoint']= $pObj->startpoint;
 ...
 WHERE deleted= 0 AND pid='.$conf['storagePid']. ' AND seen>0 AND reference = "'.$_SESSION['commentListRecord'].'"';
		...
		if ($conf['advanced.']['useSharing'] !=0) {
			...
			if (intval($conf['advanced.']['useSharing']) ==1) {
				...
				if ($conf['advanced.']['dontUseSharingFacebook'] !=1 ) {
					$sharehtmlfacebook='<a title="' .$this->pi_getLLWrap($pObj, 'text_share_on', $fromAjax) .
						' facebook" rel="nofollow" href="javascript:void(0)" class="facebook">f</a>';
				...
				if ($conf['advanced.']['dontUseSharingTwitter'] !=1 ) {
					...
					//lang: 'en'
					$sharehtmltwitter='<a href="javascript:void(0)" rel="nofollow" title="' .$this->pi_getLLWrap($pObj, 'text_share_on', $fromAjax) .
						' Twitter" class="twitter">t</a>';
				...
				if ($conf['advanced.']['dontUseSharingGoogle'] !=1 ) {
					...
					$sharehtmlgoogle='<a title="' .$this->pi_getLLWrap($pObj, 'text_share_on', $fromAjax) .
						' Google+" rel="nofollow" href="javascript:void(0)" class="googleplus">+1</a>';
					if ($sharelistfacebookputacomma) {
						$sharelistfacebook .= ',';
						$sharelistfacebookputacomma = FALSE;
						$sharebuttonfacebook .= ',';
				...
				if ($conf['advanced.']['dontUseSharingLinkedIn'] !=1 ) {
					...
					$hasValidSharingItems = $hasValidSharingItems+1;
					$sharehtmllinkedin='<a href="javascript:void(0)" rel="nofollow" title="' .$this->pi_getLLWrap($pObj, 'text_share_on', $fromAjax) .
						' LinkedIn" class="linkedin">L</a>';
				...
				if ($conf['advanced.']['dontUseSharingStumbleupon'] !=1 ) {
					...
					$hasValidSharingItems = $hasValidSharingItems+1;
					$sharehtmlstumbleupon='<a href="javascript:void(0)" rel="nofollow" title="' .$this->pi_getLLWrap($pObj, 'text_share_on', $fromAjax) .
						' Stumbleupon" class="stumbleupon">S</a>';
		...
		for ($i=0;$i<$countarticlerating;$i++){
			...
			if (trim($articlerating[$i]['voteing'])!='') {
				...
				$voteHTML.=$divstart.str_replace('<div class="tx-tc-rts-container">',
						 '<div class="tx-tc-rts-container"  onmouseover="jQuery(\'#tx-tc-rts-disp2-'.$scopeid.
						' .tx-tc-rts-vote-bar div span[title]\').tooltip({position: \'top right\', offset: [5,-5],effect: \'fade\', opacity: 1, '.
						'tipClass: \'tooltip2\'});jQuery(\'#tx-tc-rts-disp2-'.
						$scopeid.' .tx-tc-rts-vote-bar div a[title]\').tooltip({position: \'top right\',offset: [5,-5],effect: '.
						'\'fade\',opacity: 1, tipClass: \'tooltip2\'});jQuery(\'#tx-tc-rts-dp-'.
						$scopeid.' .tx-tc-rts-vote-bar div span[title]\').tooltip({position: \'top right\', offset: '.
						'[5,-5],effect: \'fade\', opacity: 1, tipClass: \'tooltip2\'});jQuery(\'#tx-tc-rts-dp-'.
						$scopeid.' .tx-tc-rts-vote-bar div a[title]\').tooltip({position: \'top right\',offset: '.
						'[5,-5],effect: \'fade\',opacity: 1, tipClass: \'tooltip2\'});">'.
						$scopetitlehtml,
						$articlerating[$i]['voteing']) .
		...
		// kick out the scopes and remaining voting area
			$voteHTML=substr($voteHTML, 0, strpos($voteHTML, 'tx-tc-rts-container')-12);
		}
		...
		if (($conf['ratings.']['ratingsOnly'] ==0) && ($conf['code'] != 'COMMENTS') && ($conf['ratings.']['mode'] != 'static')) {
			...
			if (intval($conf['advanced.']['useCommentLink']) ==0 ) {
				...
				$stylecomment=substr($stylecomment, 0, strlen($stylecomment)-1) . ' display: none;"';
			}
		...
		if (intval($conf['advanced.']['userCommentResponseLevels']) >0) {
			...
			'"></a><div class="tx-tc-ct-ry-frameover" style="display:none;opacity:0;filter: alpha(opacity=0); " id="tx-tc-ct-ry-frame-' .
			$_SESSION['commentListCount'] . '" >';

			$replyoncommentHTML.= '<div onclick="toctoc_coc_close(' . $_SESSION['commentListCount'] . ');" id="tx-tc-cts-cocfuncs-' .
			$_SESSION['commentListCount'] . '"  class="tx-tc-cocfuncs" style="display:none;" >';
			...
			'" class="tx-tc-coc" style="display:none;" >';
		...
		if ((intval($conf['ratings.']['useShortTopLikes']) == 1) || (intval($conf['ratings.']['useLikeDislikeStyle']) == 1)) {
			$repl='id="tx-tc-myrts-dp-'.$externalrefscope.'-';
			$replw='style="display:none;" id="tx-tc-myrts-dp-'.$externalrefscope.'-';
			$wrkareaHTML=str_replace('id="tx-tc-myrtstop-'.$externalrefscope.'"', 'id="tx-tc-myrtstop-'.$externalrefscope.
					'" style="display:none;"', $topareaHTML);
			$topareaHTML=str_replace($repl, $replw, $wrkareaHTML);
		}
		...
		if (($conf['ratings.']['ratingsOnly'] == 1) && ($conf['ratings.']['enableRatings'] == 0)) {
			//only sharing line
			$displaycommentsbox=' style="display:none;" ';
		}
		...
		$markers = array(
				...
				'###UID###' => $pObj->externalUid,
				'###REF###' => $externalref,
				'###DISPLAYCOMMENTSBOXCSS###' => $displaycommentsbox,
				'###REFCSS###' => $externalref,
				'###CID###' => $_SESSION['commentListCount'],
	...
	public function comments_getComments(&$rows, $conf, $pObj, $feuserid, $fromAjax, $pid, $levelhlt=0) {
		...
		foreach ($rows as $row) {
			...
			if (($commentisowned==TRUE) ||
				...
				if ($row['children'] == '') {
					if (intval($conf['advanced.']['allowCommentDeletion']) == 1) {
						...
						$deletelinkout .= '\'' .  $this->locationHeaderUrlsubDir(). t3lib_extMgm::siteRelPath('toctoc_comments') .
						'res/css/themes/' . $conf['theme.']['selectedTheme'] . '/img/deletecommentfe.png';
						...
						$deletelinkout .= ' id="toctoc_comments_pi1_submit_uid' . $iuid . '" value="" title="' .
				...
				if (array_key_exists($row['uid'], $userarr)) {
					...
					$editlinkout .= '\'' .  $this->locationHeaderUrlsubDir(). t3lib_extMgm::siteRelPath('toctoc_comments') .
					'res/css/themes/' . $conf['theme.']['selectedTheme'] . '/img/editcommentfe.png';
					$editlinkout .= $imgFile;
					$editlinkout .= "'";
					$editlinkout .= ') no-repeat top left;" name="toctoc_comments_pi1[submitedit]" ';
					$editlinkout .= ' onmouseout="' . $mouseouthtml . '" onclick="' . $submithtml . ';" ';
					$editlinkout .= ' id="toctoc_comments_pi1_submitedit_uid' . $iuid . '" value="" title="' .
			...
			if ((($row['toctoc_commentsfeuser_feuser'] == intval($feuserid)) && (intval($feuserid) !==0)) ||
				...
				if ($row['tx_commentsnotify_notify']==1) {
					if (($conf['advanced.']['commentatorNotifybyIP'] == 1) || ((intval($feuserid)!==0) && ($conf['advanced.']['commentatorNotifybyIP'] == 0))) {
						...
						$denotifylinkout .= '\'' .  $this->locationHeaderUrlsubDir(). t3lib_extMgm::siteRelPath('toctoc_comments') . 'res/css/themes/' .
						$conf['theme.']['selectedTheme'] . '/img/denotifycommentfe.png';
						$denotifylinkout .= "'";
						...
						$denotifylinkout .= ' id="toctoc_comments_pi1_submit_uid' . $iuid . '" value="" title="' .
			...
			if (strlen($row['content'])>$conf['commentCropLength']) {
				...
				if (count($textcroppedrightarr)>1) {
					...
					$textcroppedleft .=$textcroppedrightarr[0] . $bbterminatorarr[0] .
										...
										$row['uid'] . '" style="display: none;">' . $bbterminatorarr[1] . substr($textcroppedright, strlen($textcroppedrightarr[0])) .
					'</span>';
			...
			$commentAjaxDatavar= 'commentsAjaxData' . $_SESSION['commentListCount'] . '6g9' . $row['uid'] . '6g9';
			...
			if (($conf['advanced.']['userCommentResponseLevels'] != 0) && ($this->isCommentingClosed($conf, $pObj) == 0) &&
				...
				if (trim($conf['code'])!='COMMENTS') {
					...
					$subcommentlink='';
					$subcommentlink.='<span id="tx-tc-ct-div-ry-' . $row['uid'] . '"><a rel="nofollow" href="javascript:commentreply(' . $row['uid'] .
					', previewselcomment' . $_SESSION['commentListCount'] . ', ' . $_SESSION['commentListCount'] . ', \'' . $externalref . '\');" title="' .
					$subcomment . '">' . $subcomment .'</a>'.'</span>';
				}
				...
				if (($conf['advanced.']['replyModeInline']==1)) {
					...
					if (trim($conf['code'])!='COMMENTS') {
						...
						$externalref . '\', \'triggerform\', \'' . $_SESSION['commentListCount'] . '6g9' . $row['uid'] . '6g9' . '\');" title="' .
						$subcomment . '">' . $subcomment .'</a>'.'</span>';
					}
					...
					if  ((intval($conf['advanced.']['loginRequired'])==1) && (intval($conf['useUserImage'])==0)) {
							$spacingleft=intval(15+$conf['theme.']['boxmodelSpacing']);
						}
					...
					$subcommentreply.= '<div class="tx-tc-ct-rply" style="margin: 0 0 0 ' . $spacingleft . 'px;' . $cssdisplayreplybox .
					'" id="tx-tc-cts-rply-' . $row['uid'] . '">';
					...
					$subcommentreply= str_replace('\'' . $this->pi_getLLWrap($pObj, 'pi1_template.add_comment', $fromAjax), '\'' .
							$this->pi_getLLWrap($pObj, 'pi1_template.replytocomment', $fromAjax), $subcommentreply);
					$subcommentreply= str_replace('commentsAjaxData' . $_SESSION['commentListCount'] . '6g9' .
							$row['uid'] . '6g9', 'commentsAjaxData' . $_SESSION['commentListCount'], $subcommentreply);
					$subcommentreply= str_replace('commentsAjaxDataAtt' . $_SESSION['commentListCount'] . '6g9' .
							$row['uid'] . '6g9', 'commentsAjaxDataAtt' . $_SESSION['commentListCount'], $subcommentreply);
					$subcommentreply.= '</div>';

					$commentAjaxDatavar= 'commentsAjaxData' . $_SESSION['commentListCount'];
				...
				if ($row['children'] != '') {
					...
					if ($conf['advanced.']['displayChildComments']==1) {
						...
						if ($nbrchildren==1) {
							...
							'" class="tx-tc-nbrsubcomments"><a class="tx-tc-lnkexp-2" href="javascript:void(0)" rel="nofollow"
									onclick="tctrvw(0, ' . $row['uid'] . ', \'' . $row['children'] . '\', \'' . $row['allchildren'] . '\')">' .
					...
					for ($i=0;$i<2;$i++) {
							if ($i==0) {
								...
								if (!$this->getCommentBoxChildrenDisplayIsCollapsed($row['children'], $conf, $row['level'], $fromAjax, $levelhlt,
									...
									$opa.='display:none;';
									$txthaschildern=str_replace('tctrvw(0', 'tctrvw(1', $txthaschildern);
			...
			if (($row['level']>0) || ($displaycss !=''))  {
				...
				if ($row['level']>0)  {
					...
					$stylesubcommentHTML .= 'margin: 0 0 0 ' . $shiftleft . 'px; border-left: #' . $conf['theme.']['borderColor'] . ' ' .
					$this->levelindentbordersize . ';' . $this->levelindentCSSborderradius . '';
				}
			...
			if (intval($conf['advanced.']['countCommentViews']) ==1 ) {
				$jscommentviewcounter='<script type="text/javascript">if (!cntctrl' . $row['uid'] . ') {var cntctrl' . $row['uid'] .
				'=comments_view(\'tx_toctoc_comments_comments_'.$row['uid'].'\',\'' . $_SESSION['feuserid'] . '\', ' . $commentAjaxDatavar .', 1);};</script>';
		 ...
		 WHERE deleted= 0 AND pid='.$conf['storagePid']. ' AND seen>0 AND reference = "tx_toctoc_comments_comments_'.$row['uid'].'"';
			...
			$text =$this->replaceBBs($text, FALSE);
			...
			if (($conf['advanced.']['useEmoji'] > 0) || is_array($conf['smilies.'])) {
				if ($haseditlink!=TRUE) {
					...
					$tippjsemoji=' onmouseover="jQuery(\'#' . $cssidpart . $iuid .
					' .emoji[title]\').tooltip({offset: [-1,0],effect: \'fade\',opacity: 1, tipClass: \'tooltipemoji\'});jQuery(\'#' . $cssidpart .
					$iuid.' img[title]\').tooltip({offset: [-1,0],effect: \'fade\',opacity: 1, tipClass: \'tooltipemoji\'});" ';
				}
			...
			if ($conf['ratings.']['useLikeDislikeStyle']==1) {
				...
				if ($conf['ratings.']['useVotes']==1) {
					...
					'<p class="tx-tc-text" id="tx-tc-ct-box-cttxt'.$idchange . '-' . $row['uid'] .'">';
					$extractedcommentvoting =str_replace('class="tx-tc-overrating-voting"',
														 'class="tx-tc-overrating-voting"  onmouseover="
															jQuery(\'#tx-tc-rts-disp2-tx_toctoc_comments_comments_' .
															$row['uid'] . ' .tx-tc-rts-vote-bar div span[title]\').tooltip({position: \'top right\',
															offset: [5,-5],effect: \'fade\', opacity: 1, tipClass: \'tooltip2\'});
															jQuery(\'#tx-tc-rts-disp2-tx_toctoc_comments_comments_' .
															$row['uid'] . ' .tx-tc-rts-vote-bar div a[title]\').tooltip({position: \'top right\',
															offset: [5,-5],effect: \'fade\',opacity: 1, tipClass: \'tooltip2\'});
															jQuery(\'#tx-tc-rts-dp-tx_toctoc_comments_comments_' .
															$row['uid'] . ' .tx-tc-rts-vote-bar div span[title]\').tooltip({position: \'top right\',
															offset: [5,-5],effect: \'fade\', opacity: 1, tipClass: \'tooltip2\'});
															jQuery(\'#tx-tc-rts-dp-tx_toctoc_comments_comments_' .
															$row['uid'] . ' .tx-tc-rts-vote-bar div a[title]\').tooltip({position: \'top right\',
					...
					$extractedcommentvoting =str_replace('return false"></div>', 'return false"></span>', $extractedcommentvoting);
				...
				$namepart= '<span id="txtcnamepart' . $iuid . '" style="float:left;">'.  $namepart;
				if ($conf['advanced.']['useNameCommentSeparator'] == 1) {
			...
			$uclink =  $this->t3substituteMarkerArray($templateuclink, array(
					'###COMMENT_ID###' => $iuid,
					'###CID###' => $_SESSION['commentListCount'],
					'###TOCTOCUID###' => base64_encode($row['toctoc_comments_user']),
			...
			if ((intval($conf['useUserImage']) == 0) && ($conf['advanced.']['wallExtension'] == 0)) {
				$markerArray['###NAMEPART###']= str_replace('<span ', '<span class="tx-tc-nameclasslink" onclick="' . $uclink . ';"', $markerArray['###NAMEPART###']);
			}
	...
	protected function comments_getCommentsBrowser($rpp, $startpoint, $totalrows, $pObj, $fromAjax, $conf) {
		...
		if ($conditionshow==TRUE) {
			...
			$result .= '<div class="tx-tc-cts-ctsbrowse" id="tx-tc-cts-ctsbrowse-' . $_SESSION['commentListCount']  . '"><form id="bf' .
		...
		if ($conditionhide==TRUE) {
			...
			$_SESSION['commentListCount']  . '"><form id="bfh' . $_SESSION['commentListCount']  . '" ';
			$result .= ' class="comment-browseform-hide" ';
			$result .= ' method="post" action="">';
			$result .= '<input id="toctoc_comments_pi1_submit_bhcid' . $_SESSION['commentListCount']  . '" ';
			$result .= ' type="button" title="';
			$result .= '" value="' . $this->pi_getLLWrap($pObj, 'pi1_template.commentsbrowse.hide_older_rows_text', $fromAjax) .
			'" name="toctoc_comments_pi1[submit]"';
			$result .= ' onclick="' . $submithtml . '" onmouseover="this.style.textDecoration=\'underline\';" onmouseout="this.style.textDecoration=\'none\';"';
			...
			if (count($this->AJAXimages)==0) {
				if (count($_SESSION['AJAXimages'])!=0) {
					...
					foreach($rowsfeuserimages as $keyimage) {
						...
						$keyFeUserDbField=$this->fixFeUserPic($keyimage[$conf['advanced.']['FeUserDbField']]);
						$femp='';
						...
						if ($tmpimgstr==='') {
							...
							$img['params'] = 'style="' . $userimagestyle . '" class="'.$profileimgclass . $classonline . '" title="'. $usernametitle .
							'"  id="tx-tc-cts-img-"';
	 ...
	 */
	private function form($conf, &$pObj, $piVars, $fromAjax, $pid, $ifeuserid=0, $userpic, $commentid=0, $replylevel=0,
			$fromcomments = FALSE) {
		...
		if (count($requiredFields)>0) {
			$requiredhint='<div class="tx-tc-ct-form-field"><span class="tx-tc-ct-reqhint">' . $requiredMark . ': ' .
		...
		if (!$fromAjax) {
			...
			if ($processimage==TRUE) {
				for ($i=0;$i<2;$i++) {
					...
					$img['params'] = 'style="' . $opa . '" class="' . $profileimgclass . '" title="' . $alttext . '" '. $onclick .
		...
		if (($conf['attachments.']['usePicUpload']==1) || ($conf['attachments.']['usePdfUpload']==1)) {
			...
			if ($conf['attachments.']['usePicUpload']==1) {
				$uploadpictext='<img class="tx-tc-upload" style="display:block;' . $firstmargin . '" align="left" src="' .
			...
			if ($conf['attachments.']['usePdfUpload']==1) {
				$uploadpdftext='<img class="tx-tc-upload" style="display:block;' . $secondmargin . '" align="left" src="' .
				$this->locationHeaderUrlsubDir() .  t3lib_extMgm::siteRelPath('toctoc_comments') . 'res/css/themes/' . $conf['theme.']['selectedTheme'] .
			...
			$uploadpictextweb='<a href="javascript:void(0)" rel="nofollow"  onclick="jQuery(\'.tooltip2\').hide();tcOpenFile(\'toctoc_comments_pi1_' .
			$tmpcid . 'uploadpic\', \'pic\');return;">'. $uploadpictext .'</a>';
			$uploadpdftextweb='<a href="javascript:void(0)" rel="nofollow"  onclick="jQuery(\'.tooltip2\').hide();tcOpenFile(\'toctoc_comments_pi1_' .
			$tmpcid . 'uploadpic\', \'pdf\');return;">'. $uploadpdftext .'</a>';
		...
		$prvcttext='<div style="" class="' . $cssstyleeye . '-dp" id="tx-tc-cts-prv-ct-dp-' . $tmpcid . '"><img id="prv-ct-' . $tmpcid . '" class="' .
		$cssstyleeye .
		...
		if ($conf['advanced.']['allowCommentPreview']==1) {
			$htmlprvcttextweb='<a href="javascript:void(0)" rel="nofollow"  onclick="previewct(commentsAjaxData' . $tmpcid . ',\'' .
			$tmpcid . '\', 1);return;">'. $prvcttext .'</a>';
			...
			$prvcommentHTML.= '<div onclick="jQuery(\'.tooltip\').hide();previewct(\'\', \'' . $tmpcid . '\',0);" id="tx-tc-cts-div-ct-prv-' .
			$tmpcid . '"  class="tx-tc-ct-prv" style="display:none;" >';
			...
			'" class="tx-tc-ct-prv" style="cursor: pointer;display:block;" />';
			$prvcommentHTML.= '</div>';
			$prvcommentHTML.= '<div class="tx-tc-ct-prv-frame" style="display:none;opacity:1;filter: alpha(opacity=100);" id="tx-tc-cts-prv-content-' .
			$tmpcid . '" >';
		...
		if ($conf['advanced.']['loginRequired'] == 1) {
			...
			$loggoutlink = '<a href="javascript:void(0)" rel="nofollow" onmouseout"overlogout=0;" onmouseover="overlogout=1;" onclick="tt_logout(\'' .
			$tmpcid . '\',\'' . $externalref . '\');return;">'. $this->pi_getLLWrap($pObj, 'pi1_template.logout', $fromAjax) .'</a>';

			$loggout='<div style="float:left;margin-top:-6px;text-align: right;">' . $this->pi_getLLWrap($pObj, 'pi1_template.loggedinas', $fromAjax) . ' '.
			htmlspecialchars(trim($_SESSION['submitCommentVars'][$cid]['firstname'] . ' ' . $_SESSION['submitCommentVars'][$cid]['lastname'])) . '<br>' .
			$loggoutlink . '</div>';
		}
		...
		if ($conf['advanced.']['watermarkFormFields'] == 1) {
			...
			$label_WEB_SITE = '';
			$label_LOCATION = '';
			$TEXT_FIRST_NAME=str_replace(':', '', $TEXT_FIRST_NAME);
			$TEXT_LAST_NAME=str_replace(':', '', $TEXT_LAST_NAME);
			$TEXT_EMAIL=str_replace(':', '', $TEXT_EMAIL);
			$TEXT_WEB_SITE=str_replace(':', '', $TEXT_WEB_SITE);
			$TEXT_LOCATION=str_replace(':', '', $TEXT_LOCATION);
			$pcehldr_FIRST_NAME = 'placeholder="' . $TEXT_FIRST_NAME.'" ';
			$pcehldr_LAST_NAME = 'placeholder="' . $TEXT_LAST_NAME.'" ';
			$pcehldr_EMAIL = 'placeholder="' . $TEXT_EMAIL.'" ';
			$pcehldr_WEB_SITE = 'placeholder="' . $TEXT_WEB_SITE.'" ';
			$pcehldr_LOCATION = 'placeholder="' . $TEXT_LOCATION.'" ';
			$jswm_FIRST_NAME = '<script type="text/javascript">
			jQuery(function () {
				jQuery(\'#toctoc_comments_pi1_'.$output_cid.'firstname\').watermark(\'' . $TEXT_FIRST_NAME.'\', {left: 0, top: 0, fallback: true});
			});
			</script>';
			$jswm_LAST_NAME = '<script type="text/javascript">
			jQuery(function () {
				jQuery(\'#toctoc_comments_pi1_'.$output_cid.'lastname\').watermark(\'' . $TEXT_LAST_NAME .'\', {left: 0, top: 0, fallback: true});
			});
			</script>';
			$jswm_EMAIL =  '<script type="text/javascript">
			jQuery(function () {
				jQuery(\'#toctoc_comments_pi1_'.$output_cid.'email\').watermark(\'' . $TEXT_EMAIL.'\', {left: 0, top: 0, fallback: true});
			});
			</script>';
			$jswm_WEB_SITE =  '<script type="text/javascript">
			jQuery(function () {
				jQuery(\'#toctoc_comments_pi1_'.$output_cid.'homepage\').watermark(\'' . $TEXT_WEB_SITE.'\', {left: 0, top: 0, fallback: true});
			});
			</script>';
			$jswm_LOCATION =  '<script type="text/javascript">
			jQuery(function () {
				jQuery(\'#toctoc_comments_pi1_'.$output_cid.'location\').watermark(\'' . $TEXT_LOCATION.'\', {left: 0, top: 0, fallback: true});
			});
			</script>';
		} else {
			$label_FIRST_NAME = '<label class="tx-tc-ct-label" for="toctoc_comments_pi1_firstname" style="width: '.$labelwidth.'px">'.
			$TEXT_FIRST_NAME.$tREQUIRED_FIRSTNAME.'</label>';
			$label_LAST_NAME = '<label class="tx-tc-ct-label" for="toctoc_comments_pi1_lastname" style="width: '.$labelwidth.'px">'.
			$TEXT_LAST_NAME.$tREQUIRED_LASTNAME.'</label>';
			$label_EMAIL = '<label class="tx-tc-ct-label" for="toctoc_comments_pi1_email" style="width: '.$labelwidth.'px">'.
			$TEXT_EMAIL.$tREQUIRED_EMAIL.'</label>';
			$label_WEB_SITE = '<label class="tx-tc-ct-label" for="toctoc_comments_pi1_homepage" style="width: '.$labelwidth.'px">'.
			$TEXT_WEB_SITE.$tREQUIRED_HOMEPAGE.'</label>';
			$label_LOCATION = '<label class="tx-tc-ct-label" for="toctoc_comments_pi1_location" style="width: '.$labelwidth.'px">'.
			$TEXT_LOCATION.$tREQUIRED_LOCATION.'</label>';
			$pcehldr_FIRST_NAME = '';
			$pcehldr_LAST_NAME = '';
			$pcehldr_EMAIL = '';
			$pcehldr_WEB_SITE = '';
		...
		$subformhtml =  $this->t3substituteMarkerArray($subformTemplate, array(
				...
				'###LABEL_WEB_SITE###' => $label_WEB_SITE,
				'###LABEL_LOCATION###' => $label_LOCATION,
				'###PHDR_FIRST_NAME###' => $pcehldr_FIRST_NAME,
				'###PHDR_LAST_NAME###' => $pcehldr_LAST_NAME,
				'###PHDR_EMAIL###' => $pcehldr_EMAIL,
				'###PHDR_WEB_SITE###' => $pcehldr_WEB_SITE,
				'###PHDR_LOCATION###' => $pcehldr_LOCATION,
				'###WMJS_FIRST_NAME###' => $jswm_FIRST_NAME,
				'###WMJS_LAST_NAME###' => $jswm_LAST_NAME,
				'###WMJS_EMAIL###' => $jswm_EMAIL,
				'###WMJS_WEB_SITE###' => $jswm_WEB_SITE,
				'###WMJS_LOCATION###' => $jswm_LOCATION,
				'###SITE_REL_PATH###' =>  $this->locationHeaderUrlsubDir() .  t3lib_extMgm::siteRelPath('toctoc_comments'),
				'###LANG###' => $_SESSION['activelang'],
				'###PVSHEIGHT###' => $conf['attachments.']['webpagePreviewHeight'],
				'###NAVICLOSEMGLEFT###' => 24,
		...
		$tapadding='1';
		if ($conf['advanced.']['useEmoji']>0) {
			$tapadding=28+intval(intval($conf['theme.']['boxmodelSpacing'])/2);
			$smilieselectorttooltipjs = ' onmouseover="jQuery(\'#tx-tc-smilie-icon-'.$output_cid.'[title]\').tooltip({offset: [-1,0],effect: \'fade\',opacity: 1});" ';
			$smilieselectorttooltipjsemojis = ' onmouseover="jQuery(\'.tx-tc-smilie-popup .tx-tc-emo-bot-nav span[title]\').tooltip({offset: [-1,0], ' .
			'effect: \'fade\',opacity: 1, tipClass: \'tooltipemoji\'});jQuery(\'.tx-tc-smilie-popup .moji[title]\').tooltip({offset: [-1,0],effect: '.
			'\'fade\',opacity: 1, tipClass: \'tooltipemoji\'});jQuery(\'.tx-tc-smilie-popup img[title]\').tooltip({offset: [-1,0],effect: '.
			'\'fade\',opacity: 1, tipClass: \'tooltipemoji\'});" ';
			$smilieselectorttooltip = ' title="' .$this->pi_getLLWrap($pObj, 'pi1_template.add_emoji', $fromAjax) .'"';
			$smilieselectorhtml='<span '. $smilieselectorttooltipjs .'><a href="javascript:void(0)" onclick="jQuery(\'.tooltip\').hide();selectemoji(\'' .
			$output_cid . '\');" id="tx-tc-smilie-iconlink-'.$output_cid.'" rel="nofollow" ><div class="tx-tc-smilie-icon" id="tx-tc-smilie-icon-'.
			$output_cid.'" '.$smilieselectorttooltip .'></div></a></span><div class="tx-tc-smilie-popup" id="tx-tc-smilie-popup-'.$output_cid.
		...
		$outhtml = str_replace('commentsAjaxData' . $_SESSION['commentListCount'] . '6g9' . $commentid . '6g9', 'commentsAjaxData' .
				$_SESSION['commentListCount'], $outhtml);
		$outhtml = str_replace('commentsAjaxDataImg' . $_SESSION['commentListCount'] . '6g9' . $commentid . '6g9', 'commentsAjaxDataImg' .
				$_SESSION['commentListCount'], $outhtml);
		$outhtml = str_replace('commentsAjaxThisData' . $_SESSION['commentListCount'] . '6g9' . $commentid . '6g9', 'commentsAjaxThisData' .
				$_SESSION['commentListCount'], $outhtml);
		$outhtml = str_replace('commentsAjaxDataC' . $_SESSION['commentListCount'] . '6g9' . $commentid . '6g9', 'commentsAjaxDataC' .
				$_SESSION['commentListCount'], $outhtml);
		$outhtml = str_replace('commentsAjaxDataAtt' . $_SESSION['commentListCount'] . '6g9' . $commentid . '6g9', 'commentsAjaxDataAtt' .
				$_SESSION['commentListCount'], $outhtml);
		$this->trackdebug('form COMMENT_FORM');
	...
	protected function form_updatePostVarsWithFeUserData($conf, $piVars, $feuserid, $fromAjax, $userpic, $cid, $output_cid) {
			...
			if ($fe_user_user_uid) {
				...
				*/
					...
					if (count($rows) ==1) {
						foreach($rows as $key) {
							$keyFeUserDbField=$this->fixFeUserPic($key[$conf['advanced.']['FeUserDbField']]);
	...
	protected function form_getCaptcha($pObj, $conf, $fromAjax) {
		...
		if ($captchaType == 1) {
			...
			$retstr = $this->t3substituteMarkerArray($template, array(
					...
					'###AJAXDATACONF###' => '&srcbcc=' . $conf['spamProtect.']['freecapBackgoundcolor'] . '&srctc=' . $conf['spamProtect.']['freecapTextcolor']
					 . '&srcnbc=' . $conf['spamProtect.']['freecapNumberchars'] . '&srch=' . $conf['spamProtect.']['freecapHeight'] . '&mtm=' .
	...
	public function updateComment($conf, $pObj, $ctid, $content, $pid, $plugincacheid) {
		...
		$contenttext =$this->replaceBBs($contenttext, FALSE);
	...
	public function sendNotificationEmail($uid, $plugincacheid, $points, $action, $conf, $pObj, $fromAjax, $piVars, $pid, $fetoctocusertoinsert,
		...
		if (($conf['spamProtect.']['confirmedOptIn'] ==1) && ($optinemail!='')) {
			if ($conf['HTMLEmail']) {
				...
				$contentformail=$this->replaceBBs($piVars['content'], FALSE);
		...
		if (($action=='approve') || ($action=='info') || ($action=='rating')) {
			if (t3lib_div::validEmail($toEmail) && t3lib_div::validEmail($fromEmail)) {
				...
				if ($conf['HTMLEmail']) {
					...
					$contentformail=$this->replaceBBs($contentformail, FALSE);
					$contentformail =$this->addleadingspace($contentformail);
					$saveconfemoji=$conf['advanced.']['useEmoji'];
	 ...
	 * @return	string		HTML
	 */
	protected function replaceBBs($content, $purge=FALSE) {
			$cntBBs=count($this->BBs);
			for ($i=0;$i<$cntBBs;$i++) {
				if (($purge==TRUE)) {
	...
	protected function getAjaxJSData($feuserid, $pid, $languagecode, $conf, $pObj, $cid, $fromAjax, $dataonly=FALSE, $externalref) {
		...
		if (($_SESSION['AJAXCid'] != $cid) || ($fromAjax)) {
            ...
            if ($ajaxDatalogin !='') {
            	$ajaxDataloginstr='
var commentsAjaxDataLogin' . $cid . " = '" . rawurlencode($ajaxDatalogin) . "';";
            } else {
            	$ajaxDataloginstr='';
            }
            ...
            if ($ajaxDatalogout !='') {
            	$ajaxDatalogoutstr='
var commentsAjaxDataLogout' . $cid . " = '" . rawurlencode($ajaxDatalogout) . "';";
            } else {
            	$ajaxDatalogoutstr='';
            }
...
var commentsAjaxDataLoginSess' . $cid . " = '" . rawurlencode($ajaxDataloginSess) . "';";
            } else {
            	$ajaxDataloginSessstr='';
            }

			$jsAjaxData = '	<script type="text/javascript">';
			$jsAjaxData .= 'var commentsAjaxData' . $cid . " = '" . rawurlencode($ajaxData) . "';
var commentsAjaxDataAtt" . $cid . " = '" . rawurlencode($ajaxDataAttachments) . "';" . $ajaxDataloginstr . $ajaxDatalogoutstr . $ajaxDataloginSessstr . '</script>';
			if ($dataonly) {
	 ...
	 */
	protected function updateAjaxData($feuserid) {
	...
	protected function getAjaxLoggedInData($forjsvariable, $outputloginstate) {
		...
		if ($forjsvariable=='conf') {
			...
			$confCopyo['lastTotalrows']= $_SESSION['lastTotalrows'];
			$confCopyo['lastStartpoint']= $_SESSION['lastStartpoint'];
	...
	{
		if (isset($_SERVER['HTTP_USER_AGENT']) && ((strpos($_SERVER['HTTP_USER_AGENT'], 'MSIE') !== FALSE) ||
				(strpos($_SERVER['HTTP_USER_AGENT'], 'MSIE 1') === TRUE))) {
			return TRUE;
		} else{
			return FALSE;
		}
	...
	$commentspics, $scopeid = 0) {
		...
		if (($cmd=='vote') || ($cmd=='votearticle')) {
			...
			if ($myrating['comment'][0]['commentdate']) {
				...
				if (intval($conf['advanced.']['countCommentViews']) ==1 ) {
					$jscommentviewcounter='<script type="text/javascript">if (!cntctrl' .
					$myrating['comment'][0]['commentuid']  . ') {var cntctrl' .$myrating['comment'][0]['commentuid']  .
					'=comments_view(\'tx_toctoc_comments_comments_'.$myrating['comment'][0]['commentuid'] .'\',\'' .
					$_SESSION['feuserid'] . '\', \'' . $ajaxData .'\', 1, '. $conf['storagePid'] .');};</script>';
			...
			if ($prefix != 'tx_toctoc_comments_comments_') {
				...
				if ($cntsess>1) {
					if (intval($conf['ratings.']['useLikeDislike']) !=0) {
						if (intval($conf['ratings.']['useLikeDislikeStyle']) == 0) {
							$brforfirstscope='<br class="tx-tc-solobr-articlerating">';
						}
					...
					if (trim($mylikehtmlout.$mydislikehtmlout)!='') {
						if (intval($conf['advanced.']['useSharing']) == 1) {
							$brforfirstscope='<br class="tx-tc-secondbr-articlerating">';
						}
		...
		if ($returnasarray) {
			...
			$contentarr['voteing']=str_replace('<div class="tx-tc-rts-container">',
					'<div class="tx-tc-rts-container"  onmouseover="
					jQuery(\'#tx-tc-rts-disp2-'.
					htmlspecialchars($refforvote).' .tx-tc-rts-vote-bar div span[title]\').tooltip({position: \'top right\',
					offset: [5,-5],effect: \'fade\', opacity: 1, tipClass: \'tooltip2\'});jQuery(\'#tx-tc-rts-disp2-'.
					htmlspecialchars($refforvote).' .tx-tc-rts-vote-bar div a[title]\').tooltip({position: \'top right\',
					offset: [5,-5],effect: \'fade\',opacity: 1, tipClass: \'tooltip2\'});jQuery(\'#tx-tc-rts-dp-'.
					htmlspecialchars($refforvote).' .tx-tc-rts-vote-bar div span[title]\').tooltip({position: \'top right\',
					offset: [5,-5],effect: \'fade\',opacity: 1, tipClass: \'tooltip2\'});jQuery(\'#tx-tc-rts-dp-'.
					htmlspecialchars($refforvote).' .tx-tc-rts-vote-bar div a[title]\').tooltip({position: \'top right\',
					offset: [5,-5],effect: \'fade\',opacity: 1, tipClass: \'tooltip2\'});">', $contentarr['voteing']);
			if ($conf['ratings.']['useLikeDislikeStyle']==1) {
				if (substr($ref, 0, 6) == 'tx_toc') {
	...
	private function makeiLikeText($conf, $pObj, $cmd, $ref, $fromAjax, $myrating = array(), $mylikeval, $mydis='', $template, $cid, $commentspics, $extpreffortext) {
		...
		if ($otherscount>0) {
			...
			$others ='<span id="tx-tc-oth' . $mydis . '-' . $ref . '"><a class="tx_tc_othertitle" rel="nofollow" href="javascript:void(0)" title="' .
			implode('<br>', $otheruserarray)  . '">' .  $others . '</a></span>' . $othersjs;
			if ((count($namedlikearr)>0) || ($mylikeval>0)) {
	...
	public function pi_getLLWrap($pObj, $llkey, $fromAjax) {
		...
		if (!$fromAjax) {
			...
			if (!isset($_SESSION['piGllW'][$lan][$llkey])) {
				$language = &$GLOBALS['LANG'];
				if ($pObj->conf['ll.'][$lan . '.'][str_replace('.', '-', $llkey)]!='') {
					$_SESSION['piGllW'][$lan][$llkey]= $pObj->conf['ll.'][$lan . '.'][str_replace('.', '-', $llkey)];
				} else {
...
<script> function emoselpage (idshow,idhide) {
...
<body>';
		...
		if (intval($conf['advanced.']['useInternalSmiliesInEmojiSelector'])==1){
			foreach ($this->smilies as $path => $smilieArray) {
				foreach ($smilieArray as $smilie) {
					$image = '<a href="javascript:insertemoji(\'' . str_replace("'", "\'", $smilie) . '\');" rel="nofollow"><img class="tx-tc-emo-int-img" alt="' .
					$smilie . '" title="' . $path
		...
		if ($conf['advanced.']['useEmoji']>=1) {
			...
			for ($i=0;$i<$countemojiCategoryArr;$i++) {
				...
				if ($debug==1) {
					print count($contentarr) . ' emojis in ' . $emojiCategoryArrKeysArr[$i] . ', ' . intval(count($contentarr)/21) . ' pages and ' .
					count($contentarr) % $pagepacketsize . ' emojis<br>';
				}
			...
			foreach ($smilingpagescategory as $smilingpage) {
				...
				for ($i=0;$i<$categorypagescount[($smilingcat_i+$smilingcatstart_i)];$i++) {
					...
					if ($i==$smiling_i) {
						...
						$htmlcontent .= '<a href="javascript:emoselpage(\''.$smilingcat_i . '-' . $i.'\',\''.$smilingcat_i . '-' . $smiling_i.'\');">' .
				...
				for ($i=$smilingcatstart_i;$i<6;$i++) {
					...
					if (($i-$smilingcatstart_i)==($smilingcat_i)) {
						...
						$htmlcontent .= '<a href="javascript:emoselpage(\''.($i-$smilingcatstart_i) . '-0\',\''.($smilingcat_i) . '-' . $smiling_i.'\');">' .
		...
		if ($debug==1) {
			print $htmlcontent;
			print_r($smilingpagescategories);
			exit;
		}
	 ...
	 * @return	void		...
	 */
	private function send_mail ($toEMail, $subject, $message, $html, $fromEMail, $fromName, $attachment='') {
	 ...
	 * @return	string		HTML to display
	 */
	public function getwebpagepreview($cmd, $cid, $data, $pObj, $conf) {
		$outhtml='';
		if ($cmd=='startpreview') {
			...
			$outhtml = $this->savewebpagepreviewtodb('p'. $cid, 'p'. $data['commentid'], $conf);
	 ...
	 */
	protected function savewebpagepreviewtodb($pcid, $pcommentid, $conf) {
		$debug='';
		...
		$newUid=0;
		$newUid = $this->getwebpagecache($pcid, $pcommentid, $conf, '', FALSE);
				...
				if (count($rowsattmmm) >0) {
					if ($conf_cachetime > $rowsattmmm[0]['tstamp']) {
						...
						if ($rowsattmmm[0]['attachmentvariant']<3) {
							...
							$desriptionweb = $this->replaceBBs($desriptionweb, TRUE);
			...
			if ($rowsatt[0]['attachmentvariant']< 3) {
				...
				if ((trim($rowsatt[0]['photos_etc']!='')) && ($rowattachment_subid!=999)) {
					...
					if ($rowsatt[0]['attachmentvariant']==1) {
						...
						$picstr .= '<img title="' . htmlspecialchars($rowsatt[0]['title']) . '" style="display: block;" ' . $palign . ' src="' .
						...
						if (count($rrpic)>0) {
							...
							if (!$isforemailnotification) {
								$picsrcstr= '<img title="' . htmlspecialchars($rowsatt[0]['title']) . '" style="display: block;" ' . $palign .
						...
						if (count($rrpic)>1) {
							...
							if (!$isforemailnotification) {
								...
								'" style="display: block;" ' . $palign . ' src="' . $plinkhomepage . $this->webpagepreviewsavefolder . $rrpicpicweb . '" ';
								$picstr .= 'class="tx-tc-images-img" />';
							} else {
								$picstr = '<img title="' . htmlspecialchars($rowsatt[0]['title']) . '" style="display: block;" ' . $palign .
								' src="' . $plinkhomepage . $this->webpagepreviewsavefolder . $rowsatt[0]['photo_main'] . '" ';
								$picstr .= ' />';
							}
			...
			if (!$isforemailnotification) {
				if ($rowsatt[0]['attachmentvariant']==1) {
					...
					if (count($sourcearr) >=3) {
						...
						$videohtml = '';
						$vidstyleimg=' width="140" style="display:' . $displaystyle . '; margin: 0 -4px 0 -8px; max-height: ' .
						$conf['attachments.']['webpagePreviewHeight'] . 'px;" id="vidimg' . $cid . 'index1"';
				...
				if ($rowsatt[0]['attachmentvariant']==1) {
					...
					if (count(explode('@@@', $rowsatt[0]['photo_main'])) > 1 ) {
						...
						$picstr='<img style="display:block; max-height: ' . $conf['attachments.']['webpagePreviewHeight'] . 'px;" id="vidimg' . $cid .
						'index1" src="' . $rowsatt[0]['photo_main'] . '" class="tx-tc-pvs-vid-img" />';
	...
	public function handleeID($uid, $conf, $pObj, $messagetodisplay, $refreshurl) {
		...
		if ($refreshurl !='') {
			...
			if ($refresh!='') {
				...
				$waittext.='<img style="display:block;' . $this->firstCSSmargin . '" align="left" src="' .  $this->locationHeaderUrlsubDir() .
 ...
 */
		...
		if (!$fromAjax) {
			...
			if (($conf['advanced.']['wallExtension'] != 0) || ($communitybuddies))  {
				...
				if (($conf['advanced.']['wallExtension'] == 2) || (($communitybuddies) && (t3lib_extMgm::isLoaded('cwt_community'))))  {
					...
					$where ='deleted=0 AND hidden=0 AND (fe_users_uid=' . $GLOBALS['TSFE']->fe_user->user['uid'] . ' OR buddy_uid=' . $GLOBALS['TSFE']->fe_user->user['uid'] .')';
		...
		foreach ($rows as $row) {
			...
			if ($ownershipok==1) {
				...
				if ($skiprow==FALSE) {
					...
					$commenttext =$this->replaceBBs($commenttext, TRUE);
					$saveconfemoji=$conf['advanced.']['useEmoji'];
					...
					if ($link !=$commenttext) {
						...
						$markerArray = array(
								...
								'</span><span id="tx-tc-rctdatetime-' .$row['uid'] .'" style="display:none;">'.$row['crdate'].'</span>',
	...
	protected function processReportForm(array &$errors, $conf, $pObj, $piVars) {
		...
		if ($piVars['submit']) {
			...
			if (count($errors) == 0) {
				...
				$myhomepagelink=$myhomepagelinkarr[1];
				$infoleft='';
				$contentformail=$this->replaceBBs($comment['content'], FALSE);
				$contentformail =$this->addleadingspace($contentformail);
				$saveconfemoji=$conf['advanced.']['useEmoji'];
	...
	public function showtopRatings($conf, $pObj) {
		...
		$this->cObj = t3lib_div::makeInstance('tslib_cObj');
		$this->cObj->start('', '');
		$retstr = $this->topratings($conf, $pObj);
		return $retstr;
	}
		...
		if ($conf['topRatings.']['topRatingsMode']==0){
			...
			while ($rowsmerged1 = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($resultmerged1)) {
				...
				if (intval($date)!=0) {
					...
					//$conf['topRatings.']['showCountTopViewsLastView']=1;
			...
			if ($ownershipok==1) {
				...
				if ($skiprow==FALSE) {
					...
					if (strpos($row['link'], 'href=')>0) {
						...
						$titletxt='<div class="tx-tc-trt-entry" style="margin-left: ' . $margincontent. 'px">' . $titleimage . $titlelink . ' &#124; ' .
						$row['date'] . '</div>';
