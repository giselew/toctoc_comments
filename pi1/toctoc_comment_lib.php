 ...
 *  483:     protected function checkExternalUid($conf,$pObj)
 *
 *              SECTION: Commenting functions
 *  511:     public function comments($conf,&$pObj,$fromAjax,$feuserid,$pid)
 * 1751:     protected function makeemoji($text, $conf, $debug)
 * 1875:     public function comments_getComments(&$rows,$conf,$pObj,$feuserid,$fromAjax,$pid,$levelhlt=0)
 * 2652:     protected function comments_getComments_getRatings(&$row,$conf,$pObj,$feuserid, $fromAjax, $scopeid=0)
 * 2670:     protected function comments_getComments_getEmail($email)
 * 2684:     protected function isCommentingClosed($conf,$pObj)
 * 2731:     protected function commentingClosed($pObj,$fromAjax,$conf)
 * 2752:     protected function comments_getCommentsBrowser($rpp,$startpoint,$totalrows,$pObj,$fromAjax, $conf)
 * 2870:     protected function build_AJAXImages ($conf, $pObj, $usergenderexistsstr='', $fromAjax=false)
 * 3042:     public function comments_getComments_fe_user($params, $conf, $pObj,$commentid,$fromAjax, $commentusername)
 * 3139:     protected function fixFeUserPic ($conf, $pic)
 * 3158:     protected function addleadingspace($text)
 * 3179:     protected function check_comment_ownership ($rowexternal_ref,$feuserid)
 * 3201:     public function previewcomment($data, $pObj, $conf, $fromAjax=true)
 *
 *              SECTION: functions for images cache (AJAX)
 * 3229:     protected function setAJAXimage($image,$feuserid)
 * 3240:     protected function getAJAXimage($feuserid,$commentid)
 * 3271:     protected function setAJAXimageCache($image,$imageoriginal,$feuserid)
 * 3282:     protected function getAJAXimageCache($commentuserimageout)
 * 3296:     protected function makeImageSprite($conf)
 *
 *              SECTION: Form functions
 * 3436:     public function form($conf, &$pObj, $piVars,$fromAjax,$pid, $ifeuserid=0, $userpic, $commentid=0, $replylevel=0, $fromcomments = false, $cidfromajaxcomments = 0)
 * 4112:     protected function form_updatePostVarsWithFeUserData($conf, $piVars, $feuserid, $fromAjax, $userpic, $cid, $output_cid)
 * 4425:     protected function form_getCaptcha($pObj, $conf,$fromAjax)
 * 4473:     protected function form_wrapError($field, $pObj, $conf)
 *
 *              SECTION: Comments page-browser functions
 * 4506:     protected function processBrowserSubmission($conf, $pObj, $piVars, $fromAjax, $cid, $cmd,$pid, $ref)
 *
 *              SECTION: Inserting and Updating Comments functions
 * 4570:     public function processSubmission($conf,$pObj,$piVars,$fromAjax,$feuserid,$pid,$lang)
 * 5005:     public function updateComment($conf,$pObj,$ctid,$content,$pid, $plugincacheid)
 * 5078:     protected function processSubmission_checkTypicalSpam($pObj, $conf, $piVars, $lang, $fromAjax)
 * 5151:     protected function processSubmission_validate($piVars, $conf, $pObj,$fromAjax)
 * 5221:     public function sendNotificationEmail($uid, $plugincacheid, $points, $action, $conf, $pObj, $fromAjax, $piVars, $pid,$fetoctocusertoinsert,$attachment_id=0 ,$attachment_subid=0, $optinemail='')
 * 5599:     protected function emailOptIn($conf, $optinemail, $optin_ip)
 * 5667:     protected function checkCOI($conf,$email,$checkip=true)
 *
 *              SECTION: language handling functions
 * 5700:     public function fixLL(&$conf)
 * 5721:     protected function fixLL_internal($LL, &$ll, $prefix = '')
 *
 *              SECTION: mostly formatting functions
 * 5744:     protected function createLinks($text, $conf)
 * 5766:     protected function applyStdWrap($text, $stdWrapName, $conf = null, $pObj = null)
 * 5785:     protected function checkCustomFunctionCodes($code, $pObj)
 * 5807:     protected function isNoCacheUrl($url)
 * 5828:     protected function substituteMarkersAndSubparts($template, array $markers, array $subparts, $pObj)
 *
 *              SECTION: session and cache functions
 * 5853:     public function resetSessionVars($resetcontext, $alsoajaxvar = true)
 * 5903:     public function getClearCacheIds($conf, $pid = 0, $repectsession = true)
 * 5962:     protected function getClearCacheExternal_ref_uids($conf, $external_ref_uid = '', $repectsession = true)
 * 5986:     public function setPluginCacheControlTstamp ($external_ref_uid_list)
 * 6049:     public function initCaches()
 * 6100:     protected function clearPagesCaches($conf,$pid,$plugincacheid)
 *
 *              SECTION: Smilie functions
 * 6129:     public function parseSmilieArray($data)
 * 6146:     public function replaceSmilies($content,$conf)
 * 6163:     protected function checkbbcrop($content,$commentCropLength)
 * 6189:     protected function replaceBBs($content,$conf,$purge=false)
 *
 *              SECTION: functions for the AJAx interface with jQuery/JavaScript
 * 6231:     protected function getAjaxJSData($feuserid, $pid, $languagecode, $conf, $pObj, $cid, $fromAjax, $dataonly=false,$externalref)
 * 6272:     protected function getAjaxData($feuserid, $pid, $languagecode, $conf, $cid,$ref)
 * 6315:     protected function getAjaxDataAttachments($conf,$fromAjax,$pObj)
 * 6336:     protected function getAjaxJSDataCommentImgs($cid, $pObj,$fromAjax)
 * 6372:     protected function getAjaxJSDataComments($cid,$pObj,$fromAjax)
 * 6410:     protected function getAjaxDataComments($pObj, $cid)
 * 6426:     protected function getAjaxDataImgs()
 *
 *              SECTION: URL- and IP-related functions
 * 6442:     public function getCurrentIp()
 * 6456:     public function hasValidItemUrl($piVars)
 * 6477:     public function getDefaultConfig($pluginkey='')
 * 6490:     protected function ae_detect_ie()
 *
 *              SECTION: Rating functions
 * 6518:     public function getRatingDisplay($ref, $conf = null, $fromAjax = 0, $pid=0, $returnasarray = false, $feuserid = 0, $cmd = 'vote',
			$pObj = null, $cid, $fromcomments,$commentspics = array(), $scopeid=0)
 * 6572:     public function isVoted($ref,$conf,$pObj,$scopeid=0,$feuserid=0)
 * 6621:     protected function getBarWidth($rating,$conf)
 * 6635:     protected function getRatingInfo($ref,$pObj, $feuserid=-1,$conf,$scopeid=0)
 * 6760:     protected function generateRatingContent($ref, $template, $conf, $fromAjax, $pid, $returnasarray, $feuserid, $pObj,$cmd, $cid,$fromcomments, $commentspics, $scopeid = 0)
 * 7504:     function makeiLikeText($conf, $pObj, $cmd, $ref, $fromAjax, $myrating = array(),$mylikeval,$mydis='',$template,$cid, $commentspics,$extpreffortext)
 *
 *              SECTION: TYPO3 workarounds
 * 7850:     public function enableFields($tableName,$pObj)
 * 7870:     public function pi_getLLWrap($pObj, $llkey, $fromAjax)
 * 7901:     protected function t3getSubpart ($pObj, $templateCode, $templateMarker)
 * 7924:     protected function t3substituteMarkerArray ($pObj, $content, $markContentArray, $wrap = '', $uppercase = 0, $deleteUnused = 0)
 * 7946:     protected function t3substituteMarker ($pObj, $template, $marker, $markContent)
 * 7960:     protected function t3fileResource ($pObj, $usetemplateFile)
 * 7977:     protected function getcheck ($ref, $i, $ajaxData, $ratingscheck)
 * 7998:     protected function formatDate($date, $pObj, $fromAjax, $conf)
 * 8124:     protected function getPageURL($fromAjax = false,$pid = 0)
 * 8146:     protected function locationHeaderUrlsubDir($withleadingslash = true)
 * 8168:     protected function getCleanHTML ($html)
 *
 *              SECTION: Usercards
 * 8197:     public function getSmiliesCard($conf)
 * 8209:     function emoselpage (idshow,idhide)
 * 8463:     public function getUserCard($basedimgstr,$basedtoctocuid,$conf,$pObj,$commentid)
 *
 *              SECTION: Mail functions
 * 8756:     protected function getMailTo($mailAddress, $linktxt = '', $initP = '?')
 * 8799:     protected function encryptEmail($string,$back=0)
 * 8835:     protected function encryptCharcode($n,$start,$end,$offset)
 * 8857:     function send_mail ($toEMail,$subject,$message,$html,$fromEMail,$fromName,$attachment='')
 * 9009:     protected function slashName ($name, $apostrophe='"')
 *
 *              SECTION: Commentator-Notification functions
 * 9033:     public function handleCommentatorNotifications($uid, $conf,$pObj, $fromeID = false, $pid=0,$fromAjax=1)
 *
 *              SECTION: Administrator comment response functions
 * 9222:     protected function getCommentsReponse(&$params, &$pObj, $conf)
 *
 *              SECTION: Website preview functions
 * 9266:     public function cleanupfup($cmd, $cid, $data, $pObj, $fromAjax, $uploadedfile, $conf, $originalfilename)
 * 9286:     public function getwebpagepreview($cmd, $cid, $data, $pObj, $fromAjax, $websitepreviewid=0, $conf)
 * 9331:     protected function getpreviewinit($cid,$data, $pObj)
 * 9369:     protected function savewebpagepreviewtodb($pcid,$pcommentid,$userurl,$conf, $pObj)
 * 9582:     protected function cleanupdbandfiles($conf,$uploadedfile='',$originalfilename='')
 * 9714:     protected function getwebpagecache($pcid,$pcommentid, $conf, $url='', $isbeforefetch = false, $pObj)
 * 9858:     protected function read_dir($dir, $array = array())
 * 9886:     protected function commentShowWebpagepreview ($rowattachmentid,$rowattachment_subid,$conf,$pObj,$cid,$topwebsitepreview,$fromAjax, $row = array(), $isforemailnotification = false)
 * 10304:     protected function makeAttachementPicture($picturefilename,$conf,$descriptionbyuser,$originalfilename, $firstname, $lastname,$fetoctocusertoinsert)
 *
 *              SECTION: eID-Interface functions
 * 10473:     public function handleeID($uid, $conf,$pObj,$messagetodisplay,$refreshurl)
 *
 *              SECTION: Reply on comments functions
 * 10570:     protected function getCommentBoxDisplay($commentid,$conf,$level,$fromAjax,$triggeredlevel=0,$levelexpandoverride=0)
 * 10635:     protected function getCommentBoxChildrenDisplayIsCollapsed($commentschildrenids,$conf,$level,$fromAjax,$triggeredlevel=0,$levelexpandoverride=0)
 *
 *              SECTION: Fe-usergroup functions
 * 10696:     public function usersGroupmembers($pObj, $fromAjax, $conf, $communitybuddies = false)
 * 10763:     protected function isUserOnline($feuser_uid)
 *
 *              SECTION: Recent comments functions
 * 10800:     public function getRecentComments($pObj, $conf,$feuserid)
 * 10861:     protected function comments_getRecentComments($rows,$conf,$pObj)
 * 11143:     protected function trimContent($text,$conf, $maxChars=0, $dospecialChars=true)
 * 11181:     protected function createRCLinks($text, $refID, $commentID, $prefix, $table, $externalprefix, $singlePid, $conf,$show_uid,$okrowsi)
 *
 *              SECTION: Report comments functions
 * 11241:     protected function getCommentsReportLink($params, &$pObj, $conf ,$fromAjax,$pid)
 * 11285:     public function mainReport($content, $conf, $pObj, $piVars)
 * 11309:     protected function processReportForm(array &$errors,$conf, $pObj, $piVars)
 * 11445:     protected function showReportThanks($conf,$pObj)
 * 11459:     protected function showReportForm(array $errors,$conf,$pObj,$piVars)
 * 11540:     protected function getReportCaptcha($required, $error,$conf,$pObj)
 *
 *              SECTION: IP-blocking functions
 * 11584:     protected function IPBlockSpamCheck(&$params, &$pObj)
 * 11601:     protected function sendNotificationIPBlock($params, &$pObj)
 * 11614:     protected function checkSMTPService($hostname, $port)
 * 11636:     private function getIpAddr()
 * 11651:     private function checkTableBLs($ipaddr)
 * 11662:     private function checkLocalBL($ipaddr)
 * 11675:     private function checkStaticBL($ipaddr)
 * 11701:     function checkNetworkBLs($ipaddr)
 * 11725:     protected function trackdebug ($trackingfunction)
 *
 *              SECTION: Top ratings functions
 * 11752:     public function showtopRatings($conf, $pObj)
 * 11766:     protected function topratings ($conf, $pObj)
 *
 * TOTAL FUNCTIONS: 123
			...
			if (count($this->AJAXimages)==0) {
				if (count($_SESSION['AJAXimages'])!=0) {
					...
					if ($conf['advanced.']['FeUserDbField'] != 'image') {
				 	...
				 	$rowsfeuserimages = $GLOBALS['TYPO3_DB']->exec_SELECTgetRows('fe_users.uid AS uid, '. $usergenderexistsstr .' fe_users.image AS image, ' .$imagefield .'fe_users.lastlogin AS lastlogin, fe_users.is_online AS is_online, fe_users.last_name AS lastname, fe_users.username AS username, fe_users.first_name AS firstname, fe_users.name AS feusername',
