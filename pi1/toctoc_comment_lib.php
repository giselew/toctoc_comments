 ...
 *  615:     public function comments_getComments(&$rows,$conf,$pObj,$feuserid,$fromAjax)
 *  772:     protected function comments_getComments_fe_user($params, $conf, $pObj,$commentid,$fromAjax)
 *  927:     protected function setAJAXimage($image,$feuserid)
 *  938:     protected function getAJAXimage($feuserid,$commentid)
 *  969:     protected function setAJAXimageCache($image,$imageoriginal,$feuserid)
 *  980:     protected function getAJAXimageCache($commentuserimageout)
 *  998:     protected function comments_getComments_getRatings(&$row,$conf,$pObj,$feuserid, $fromAjax)
 * 1017:     protected function comments_getComments_getEmail($email)
 * 1035:     protected function comments_getCommentsBrowser($rpp,$startpoint,$totalrows,$pObj,$fromAjax)
 * 1116:     public function form($conf, &$pObj, $piVars,$fromAjax,$pid, $ifeuserid=0, $userpic)
 * 1391:     protected function form_updatePostVarsWithFeUserData($conf, &$pObj, $piVars, $feuserid, $fromAjax, $userpic, $cid)
 * 1673:     protected function form_getCaptcha($pObj, $conf,$fromAjax)
 * 1718:     protected function form_wrapError($field, $pObj, $conf)
 * 1746:     protected function processBrowserSubmission($conf, $pObj, $piVars, $fromAjax, $cid, $cmd,$pid)
 * 1810:     public function processSubmission($conf,$pObj,$piVars,$fromAjax,$feuserid,$pid,$lang)
 * 2093:     protected function processSubmission_checkTypicalSpam($conf, $piVars, $lang, $fromAjax)
 * 2146:     protected function processSubmission_validate($piVars, $conf, $pObj,$fromAjax)
 * 2209:     public function sendNotificationEmail($uid, $points, $action, $conf, $pObj, $fromAjax, $piVars, $pid)
 * 2298:     protected function isCommentingClosed($conf,$pObj)
 * 2344:     protected function commentingClosed($pObj,$fromAjax)
 * 2362:     public function fixLL(&$conf)
 * 2383:     protected function fixLL_internal($LL, &$ll, $prefix = '')
 * 2400:     protected function createLinks($text, $conf)
 * 2422:     protected function applyStdWrap($text, $stdWrapName, $conf, $pObj)
 * 2441:     protected function checkCustomFunctionCodes($code, $pObj)
 * 2463:     protected function isNoCacheUrl($url)
 * 2484:     protected function substituteMarkersAndSubparts($template, array $markers, array $subparts, $pObj)
 * 2501:     public function resetSessionVars($resetcontext, $alsoajaxvar = true)
 * 2542:     public function getClearCacheIds($conf, $pid = 0)
 * 2569:     public function parseSmilieArray($data)
 * 2586:     public function replaceSmilies($content,$conf)
 * 2610:     protected function getAjaxJSData($feuserid, $pid, $languagecode, $conf, $pObj, $cid, $fromAjax, $dataonly=false)
 * 2647:     protected function getAjaxData($feuserid, $pid, $languagecode, $conf, $cid)
 * 2672:     protected function getAjaxJSDataCommentImgs($cid, $pObj,$fromAjax)
 * 2708:     protected function getAjaxJSDataComments($cid,$pObj,$fromAjax)
 * 2746:     protected function getAjaxDataComments($pObj, $cid)
 * 2771:     protected function getAjaxDataImgs()
 * 2784:     public function getCurrentIp()
 * 2798:     public function hasValidItemUrl($piVars)
 * 2820:     public function getDefaultConfig()
 * 2838:     public function getRatingDisplay($ref, $conf = null, $fromAjax = 0, $pid=0, $returnasarray = false, $feuserid = 0, $cmd = 'vote',
			$pObj = null, $cid, $fromcomments)
 * 2883:     public function isVoted($ref,$conf,$pObj)
 * 2901:     protected function getBarWidth($rating,$conf)
 * 2914:     protected function getRatingInfo($ref,$pObj, $feuserid=-1,$conf)
 * 2987:     protected function generateRatingContent($ref, $template, $conf, $fromAjax, $pid, $returnasarray, $feuserid, $pObj,$cmd, $cid,$fromcomments)
 * 3351:     public function enableFields($tableName,$pObj)
 * 3371:     public function pi_getLLWrap($pObj, $llkey, $fromAjax)
 * 3389:     protected function getcheck ($ref, $i, $ajaxData, $ratingscheck)
 * 3412:     protected function formatDate($date, $pObj, $fromAjax, $conf)
 * 3532:     protected function getPageURL($fromAjax = false,$pid = 0)
 * 3558:     protected function getCleanHTML ($html)
 * 3579:     public function getUserCard($basedimgstr,$basedtoctocuid,$conf,$pObj,$commentid)
 * 3859:     protected function getMailTo($mailAddress, $linktxt = '', $initP = '?')
 * 3902:     protected function encryptEmail($string,$back=0)
 * 3938:     protected function encryptCharcode($n,$start,$end,$offset)
 *
 * TOTAL FUNCTIONS: 58
...
// for design 2, buttons
				...
				if (intval($conf['advanced.']['dontUseSharingGoogle']) !==1 ) {
					...
					$sharebuttongoogle = "googlePlus: {size: 'tall'}";
				...
				if (intval($conf['advanced.']['dontUseSharingLinkedIn']) !==1 ) {
					...
					$sharebuttonlinkedin="counter: 'top'";
					if ($sharelistfacebookputacomma) {
						$sharelistfacebook .= ','; $sharelistfacebookputacomma = false;	$sharebuttonfacebook .= ',';
					}
					if ($sharelisttwitterputacomma) {
				...
				if (intval($conf['advanced.']['dontUseSharingStumbleupon']) !==1 ) {
					...
					$sharebuttonstumbleupon = "layout: '5'";
					if ($sharelistfacebookputacomma) {
						$sharelistfacebook .= ','; $sharelistfacebookputacomma = false;	$sharebuttonfacebook .= ',';
					}
					if ($sharelisttwitterputacomma) {
	...
	protected function form_getCaptcha($pObj, $conf,$fromAjax) {
		...
		if ($captchaType == 2) {
			...
			return $pObj->cObj->substituteMarkerArray($template, array(
					...
					'###SITE_REL_PATH###' => '/' . t3lib_extMgm::siteRelPath('toctoc_comments'),
					'###TEXT_ENTER_CODE###' => $this->pi_getLLWrap($pObj, 'pi1_template.enter_code', $fromAjax),
					'###AJAXDATACONF###' => $this->getAjaxJSData($feuserid,$GLOBALS['TSFE']->id,$GLOBALS['TSFE']->lang,$conf,$this, $tmpcid,$fromAjax,true),
					'###SR_FREECAP_CANT_READ###' => $this->pi_getLLWrap($pObj, 'pi1_template.captcha_cant_read', $fromAjax),
					'###SITE_REL_PATH###' => '/' . t3lib_extMgm::siteRelPath('toctoc_comments'),
			));
		}
				...
				if ($myrating['comment'][0]['commentdate']){
					//if (!$fromcomments) {
						...
						$commentdatehtml = $pObj->cObj->substituteMarkerArray($commentdateSub, array(
							'###COMMENT_DATE###' => $fromAjax . " " . $this->formatDate($myrating['comment'][0]['commentdate'], $pObj, $fromAjax, $conf) .$commentcontinuation,
								));
					//}
		        }
			//}
