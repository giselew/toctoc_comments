 ...
 *
 *
 *  122: class toctoc_comment_lib  extends tslib_pibase
 *  152:     public function maincomments($ref, $conf = null, $fromAjax = false, $pid=0, $feuserid = 0, $cmd = '', &$pObj, $piVars = array(),
		$cid = 0, $sessioncapdata = '', $userpic='', $commentspics = array(), $check='', $AjaxData = '', $ajaxextref= '', $tctreestate = null)
 *  270:     protected function checkExternalUid($conf,$pObj)
 *
 *              SECTION: Comments Functions
 *  298:     public function comments($conf,&$pObj,$fromAjax,$feuserid,$pid)
 *  950:     public function comments_getComments(&$rows,$conf,$pObj,$feuserid,$fromAjax,$pid)
 * 1398:     protected function comments_getComments_fe_user($params, $conf, $pObj,$commentid,$fromAjax)
 * 1601:     protected function setAJAXimage($image,$feuserid)
 * 1612:     protected function getAJAXimage($feuserid,$commentid)
 * 1643:     protected function setAJAXimageCache($image,$imageoriginal,$feuserid)
 * 1654:     protected function getAJAXimageCache($commentuserimageout)
 * 1672:     protected function comments_getComments_getRatings(&$row,$conf,$pObj,$feuserid, $fromAjax)
 * 1691:     protected function comments_getComments_getEmail($email)
 * 1708:     protected function comments_getCommentsBrowser($rpp,$startpoint,$totalrows,$pObj,$fromAjax)
 * 1789:     public function form($conf, &$pObj, $piVars,$fromAjax,$pid, $ifeuserid=0, $userpic)
 * 2192:     protected function form_updatePostVarsWithFeUserData($conf, &$pObj, $piVars, $feuserid, $fromAjax, $userpic, $cid)
 * 2495:     protected function form_getCaptcha($pObj, $conf,$fromAjax)
 * 2541:     protected function form_wrapError($field, $pObj, $conf)
 * 2569:     protected function processBrowserSubmission($conf, $pObj, $piVars, $fromAjax, $cid, $cmd,$pid)
 * 2631:     public function processSubmission($conf,$pObj,$piVars,$fromAjax,$feuserid,$pid,$lang)
 * 3002:     public function updateComment($conf,$pObj,$ctid,$content,$pid)
 * 3105:     protected function processSubmission_checkTypicalSpam($pObj, $conf, $piVars, $lang, $fromAjax)
 * 3169:     protected function processSubmission_validate($piVars, $conf, $pObj,$fromAjax)
 * 3235:     public function sendNotificationEmail($uid, $points, $action, $conf, $pObj, $fromAjax, $piVars, $pid,$fetoctocusertoinsert)
 * 3467:     protected function isCommentingClosed($conf,$pObj)
 * 3513:     protected function commentingClosed($pObj,$fromAjax)
 * 3531:     public function fixLL(&$conf)
 * 3552:     protected function fixLL_internal($LL, &$ll, $prefix = '')
 * 3569:     protected function createLinks($text, $conf)
 * 3591:     protected function applyStdWrap($text, $stdWrapName, $conf = null, $pObj = null)
 * 3610:     protected function checkCustomFunctionCodes($code, $pObj)
 * 3632:     protected function isNoCacheUrl($url)
 * 3653:     protected function substituteMarkersAndSubparts($template, array $markers, array $subparts, $pObj)
 * 3670:     public function resetSessionVars($resetcontext, $alsoajaxvar = true)
 * 3712:     public function getClearCacheIds($conf, $pid = 0)
 * 3739:     public function parseSmilieArray($data)
 * 3756:     public function replaceSmilies($content,$conf)
 * 3780:     protected function getAjaxJSData($feuserid, $pid, $languagecode, $conf, $pObj, $cid, $fromAjax, $dataonly=false)
 * 3820:     protected function getAjaxData($feuserid, $pid, $languagecode, $conf, $cid)
 * 3855:     protected function getAjaxDataAttachments($conf,$fromAjax,$pObj)
 * 3877:     protected function getAjaxJSDataCommentImgs($cid, $pObj,$fromAjax)
 * 3913:     protected function getAjaxJSDataComments($cid,$pObj,$fromAjax)
 * 3951:     protected function getAjaxDataComments($pObj, $cid)
 * 3969:     protected function getAjaxDataImgs()
 * 3982:     public function getCurrentIp()
 * 3996:     public function hasValidItemUrl($piVars)
 * 4018:     public function getDefaultConfig()
 * 4036:     public function getRatingDisplay($ref, $conf = null, $fromAjax = 0, $pid=0, $returnasarray = false, $feuserid = 0, $cmd = 'vote',
			$pObj = null, $cid, $fromcomments)
 * 4081:     public function isVoted($ref,$conf,$pObj)
 * 4099:     protected function getBarWidth($rating,$conf)
 * 4112:     protected function getRatingInfo($ref,$pObj, $feuserid=-1,$conf)
 * 4183:     protected function generateRatingContent($ref, $template, $conf, $fromAjax, $pid, $returnasarray, $feuserid, $pObj,$cmd, $cid,$fromcomments)
 * 4577:     public function enableFields($tableName,$pObj)
 * 4597:     public function pi_getLLWrap($pObj, $llkey, $fromAjax)
 * 4615:     protected function getcheck ($ref, $i, $ajaxData, $ratingscheck)
 * 4636:     protected function formatDate($date, $pObj, $fromAjax, $conf)
 * 4756:     protected function getPageURL($fromAjax = false,$pid = 0)
 * 4782:     protected function getCleanHTML ($html)
 * 4807:     public function getUserCard($basedimgstr,$basedtoctocuid,$conf,$pObj,$commentid)
 * 5113:     protected function getMailTo($mailAddress, $linktxt = '', $initP = '?')
 * 5156:     protected function encryptEmail($string,$back=0)
 * 5192:     protected function encryptCharcode($n,$start,$end,$offset)
 * 5213:     public function handleCommentatorNotifications($uid, $conf,$pObj, $fromeID = false, $pid=0)
 * 5314:     protected function getCommentsReponse(&$params, &$pObj, $conf)
 * 5349:     protected function getCommentsReportLink($params, &$pObj, $conf ,$fromAjax,$pid)
 * 5396:     public function getwebpagepreview($cmd, $cid, $data, $pObj, $fromAjax, $websitepreviewid=0, $conf)
 * 5441:     protected function getpreviewinit($cid,$data)
 * 5474:     protected function savewebpagepreviewtodb($pcid,$pcommentid,$userurl,$conf)
 * 5659:     protected function cleanupdbandfiles($conf)
 * 5738:     protected function getwebpagecache($pcid,$pcommentid, $conf, $url='', $isbeforefetch = false)
 * 5849:     protected function read_dir($dir, $array = array())
 * 5873:     function send_mail (
			$toEMail,
			$subject,
			$message,
			$html,
 ...
 * 6009:     protected function slashName ($name, $apostrophe='"')
 * 6026:     protected function commentShowWebpagepreview ($rowattachmentid,$rowattachment_subid,$conf,$pObj,$cid,$topwebsitepreview)
 * 6120:     public function handleeID($uid, $conf,$pObj,$messagetodisplay,$refreshurl)
 * 6176:     protected function getCommentBoxDisplay($commentid,$conf,$level,$fromAjax)
 * 6229:     protected function getCommentBoxChildrenDisplayIsCollapsed($commentschildrenids,$conf,$level,$fromAjax)
 * 6276:     protected function usersGroupmembers($pObj, $fromAjax)
 *
 * TOTAL FUNCTIONS: 76
...
// for design 2, buttons
	...
	public function comments_getComments(&$rows,$conf,$pObj,$feuserid,$fromAjax,$pid) {
		...
		foreach ($rows as $row) {
			...
			if (($conf['advanced.']['userCommentResponseLevels'] != 0) && ($this->isCommentingClosed($conf,$pObj) == 0) && ($row['level']<$conf['advanced.']['userCommentResponseLevels'])) {
				...
				if (t3lib_extMgm::isLoaded('comments_report')) {
			...
			if (t3lib_extMgm::isLoaded('comments_report')) {
				...
				if ($fromAjax) {
					...
					if (is_array($tempMarkers =$this->getCommentsReportLink($params, $pObj, $conf, $fromAjax, $pid))) {
						//$tempMarkers['###TX_COMMENTSREPORT###'] ='<div class="tx-tc-ct-report">' . $tempMarkers['###TX_COMMENTSREPORT###'] . '</div>';
						$markerArray = $tempMarkers;
					}
					...
					if($markerArray['###TX_COMMENTSREPORT###'] !=''){
						...
						$linkendtext=str_replace($linkendtag,'',$totllinktext);
						$imghtml = '<img height="15" width="15" src="' . '/' . t3lib_extMgm::siteRelPath('toctoc_comments') . 'res/img/report.png" title="' . $linkendtext .'">';
						$markerArray['###TX_COMMENTSREPORT###'] =$linktag .$imghtml.$linkendtag;
					}
					$markerArray['###TX_COMMENTSREPORT###'] ='<div class="tx-tc-ct-report">' . $markerArray['###TX_COMMENTSREPORT###'] . '</div>';
					$markerArray['###LINE_COMMENTSREPORTREPLYSTART###'] = '<div class="tx-tc-ct-ry-report-line"  id="tx-tc-ct-ry-rl-' . $row['uid'] .  '">';
					$markerArray['###LINE_COMMENTSREPORTREPLYEND###'] = '<div>&nbsp;</div></div>';
	...
	*/
	...
	public function sendNotificationEmail($uid, $points, $action, $conf, $pObj, $fromAjax, $piVars, $pid,$fetoctocusertoinsert) {
		...
		if (t3lib_div::validEmail($toEmail) && t3lib_div::validEmail($fromEmail)) {
			...
			if (t3lib_extMgm::isLoaded('comments_ipbl')) {
				//post-handle response ip-block-links:
	...
	*/
	...
	protected function getCommentsReportLink($params, &$pObj, $conf ,$fromAjax,$pid) {
		...
		$usetemplateFile= str_replace('/EXT:toctoc_comments', 'typo3conf/ext/toctoc_comments',$conf['tx_commentsreport_pi1.']['templateFile']);
		$usetemplateFile= str_replace('EXT:toctoc_comments', 'typo3conf/ext/toctoc_comments',$usetemplateFile);
		$usetemplateFile= str_replace('/EXT:comments_report', 'typo3conf/ext/comments_report',$conf['tx_commentsreport_pi1.']['templateFile']);
		$usetemplateFile= str_replace('EXT:comments_report', 'typo3conf/ext/comments_report',$usetemplateFile);
		if (!$fromAjax) {
			$templateCode = $pObj->cObj->fileResource($usetemplateFile);
		} else  {
			$templateCode = @file_get_contents(PATH_site . $usetemplateFile);
		...
		$queryParams='&tx_commentsreport_pi1[info]=' . base64_encode(serialize(array(
					'url' => $theURLe,
					'uid' => $params['row']['uid'],
					)));
	...
	public function getwebpagepreview($cmd, $cid, $data, $pObj, $fromAjax, $websitepreviewid=0, $conf) {
		...
		if ($cmd=='startpreview') {
			...
			$outhtml = $this->getpreviewinit($cid,$data);
	 ...
	 */
	protected function getpreviewinit($cid,$data) {
		$this->lastpreviewid=0;
