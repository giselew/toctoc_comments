 ...
 *
 *
 *  186: class toctoc_comment_lib  extends tslib_pibase
 *  216:     public function maincomments($ref, $conf = null, $fromAjax = false, $pid=0, $feuserid = 0, $cmd = '', &$pObj, $piVars = array(),
		$cid = 0, $sessioncapdata = '', $userpic='', $commentspics = array(), $check='', $AjaxData = '', $ajaxextref= '', $tctreestate = null)
 *  337:     protected function checkExternalUid($conf,$pObj)
 *
 *              SECTION: Commenting functions
 *  365:     public function comments($conf,&$pObj,$fromAjax,$feuserid,$pid)
 * 1087:     public function comments_getComments(&$rows,$conf,$pObj,$feuserid,$fromAjax,$pid)
 * 1546:     protected function comments_getComments_getRatings(&$row,$conf,$pObj,$feuserid, $fromAjax)
 * 1565:     protected function comments_getComments_getEmail($email)
 * 1579:     protected function isCommentingClosed($conf,$pObj)
 * 1625:     protected function commentingClosed($pObj,$fromAjax)
 * 1644:     protected function comments_getCommentsBrowser($rpp,$startpoint,$totalrows,$pObj,$fromAjax)
 * 1739:     protected function build_AJAXImages ($conf,$usergenderexistsstr='')
 * 1872:     public function comments_getComments_fe_user($params, $conf, $pObj,$commentid,$fromAjax)
 *
 *              SECTION: functions for images cache (AJAX)
 * 1965:     protected function setAJAXimage($image,$feuserid)
 * 1976:     protected function getAJAXimage($feuserid,$commentid)
 * 2007:     protected function setAJAXimageCache($image,$imageoriginal,$feuserid)
 * 2018:     protected function getAJAXimageCache($commentuserimageout)
 *
 *              SECTION: Form functions
 * 2043:     public function form($conf, &$pObj, $piVars,$fromAjax,$pid, $ifeuserid=0, $userpic)
 * 2445:     protected function form_updatePostVarsWithFeUserData($conf, &$pObj, $piVars, $feuserid, $fromAjax, $userpic, $cid)
 * 2733:     protected function form_getCaptcha($pObj, $conf,$fromAjax)
 * 2781:     protected function form_wrapError($field, $pObj, $conf)
 *
 *              SECTION: Comments page-browser functions
 * 2814:     protected function processBrowserSubmission($conf, $pObj, $piVars, $fromAjax, $cid, $cmd,$pid, $ref)
 *
 *              SECTION: Inserting and Updating Comments functions
 * 2880:     public function processSubmission($conf,$pObj,$piVars,$fromAjax,$feuserid,$pid,$lang)
 * 3263:     public function updateComment($conf,$pObj,$ctid,$content,$pid)
 * 3365:     protected function processSubmission_checkTypicalSpam($pObj, $conf, $piVars, $lang, $fromAjax)
 * 3438:     protected function processSubmission_validate($piVars, $conf, $pObj,$fromAjax)
 * 3504:     public function sendNotificationEmail($uid, $points, $action, $conf, $pObj, $fromAjax, $piVars, $pid,$fetoctocusertoinsert)
 *
 *              SECTION: language handling functions
 * 3751:     public function fixLL(&$conf)
 * 3772:     protected function fixLL_internal($LL, &$ll, $prefix = '')
 *
 *              SECTION: mostly formatting functions
 * 3795:     protected function createLinks($text, $conf)
 * 3817:     protected function applyStdWrap($text, $stdWrapName, $conf = null, $pObj = null)
 * 3836:     protected function checkCustomFunctionCodes($code, $pObj)
 * 3858:     protected function isNoCacheUrl($url)
 * 3879:     protected function substituteMarkersAndSubparts($template, array $markers, array $subparts, $pObj)
 *
 *              SECTION: session and cache functions
 * 3902:     public function resetSessionVars($resetcontext, $alsoajaxvar = true)
 * 3946:     public function getClearCacheIds($conf, $pid = 0)
 *
 *              SECTION: Smilie functions
 * 3986:     public function parseSmilieArray($data)
 * 4003:     public function replaceSmilies($content,$conf)
 *
 *              SECTION: functions for the AJAx interface with jQuery/JavaScript
 * 4033:     protected function getAjaxJSData($feuserid, $pid, $languagecode, $conf, $pObj, $cid, $fromAjax, $dataonly=false)
 * 4073:     protected function getAjaxData($feuserid, $pid, $languagecode, $conf, $cid)
 * 4108:     protected function getAjaxDataAttachments($conf,$fromAjax,$pObj)
 * 4130:     protected function getAjaxJSDataCommentImgs($cid, $pObj,$fromAjax)
 * 4166:     protected function getAjaxJSDataComments($cid,$pObj,$fromAjax)
 * 4204:     protected function getAjaxDataComments($pObj, $cid)
 * 4224:     protected function getAjaxDataImgs()
 *
 *              SECTION: URL- and IP-related functions
 * 4243:     public function getCurrentIp()
 * 4257:     public function hasValidItemUrl($piVars)
 * 4279:     public function getDefaultConfig()
 *
 *              SECTION: Rating functions
 * 4303:     public function getRatingDisplay($ref, $conf = null, $fromAjax = 0, $pid=0, $returnasarray = false, $feuserid = 0, $cmd = 'vote',
			$pObj = null, $cid, $fromcomments,$commentspics = array())
 * 4349:     public function isVoted($ref,$conf,$pObj)
 * 4367:     protected function getBarWidth($rating,$conf)
 * 4380:     protected function getRatingInfo($ref,$pObj, $feuserid=-1,$conf)
 * 4481:     protected function generateRatingContent($ref, $template, $conf, $fromAjax, $pid, $returnasarray, $feuserid, $pObj,$cmd, $cid,$fromcomments, $commentspics)
 *
 *              SECTION: TYPO3 workarounds
 * 5029:     public function enableFields($tableName,$pObj)
 * 5049:     public function pi_getLLWrap($pObj, $llkey, $fromAjax)
 * 5067:     protected function getcheck ($ref, $i, $ajaxData, $ratingscheck)
 * 5088:     protected function formatDate($date, $pObj, $fromAjax, $conf)
 * 5208:     protected function getPageURL($fromAjax = false,$pid = 0)
 * 5234:     protected function getCleanHTML ($html)
 *
 *              SECTION: Usercards
 * 5267:     public function getUserCard($basedimgstr,$basedtoctocuid,$conf,$pObj,$commentid)
 *
 *              SECTION: Mail functions
 * 5583:     protected function getMailTo($mailAddress, $linktxt = '', $initP = '?')
 * 5626:     protected function encryptEmail($string,$back=0)
 * 5662:     protected function encryptCharcode($n,$start,$end,$offset)
 * 5684:     function send_mail (
			$toEMail,
			$subject,
			$message,
			$html,
 ...
 * 5820:     protected function slashName ($name, $apostrophe='"')
 *
 *              SECTION: Commentator-Notification functions
 * 5843:     public function handleCommentatorNotifications($uid, $conf,$pObj, $fromeID = false, $pid=0)
 *
 *              SECTION: Administrator comment response functions
 * 5950:     protected function getCommentsReponse(&$params, &$pObj, $conf)
 *
 *              SECTION: Website preview functions
 * 5994:     public function getwebpagepreview($cmd, $cid, $data, $pObj, $fromAjax, $websitepreviewid=0, $conf)
 * 6040:     protected function getpreviewinit($cid,$data, $pObj)
 * 6073:     protected function savewebpagepreviewtodb($pcid,$pcommentid,$userurl,$conf)
 * 6258:     protected function cleanupdbandfiles($conf)
 * 6339:     protected function getwebpagecache($pcid,$pcommentid, $conf, $url='', $isbeforefetch = false)
 * 6450:     protected function read_dir($dir, $array = array())
 * 6475:     protected function commentShowWebpagepreview ($rowattachmentid,$rowattachment_subid,$conf,$pObj,$cid,$topwebsitepreview)
 *
 *              SECTION: eID-Interface functions
 * 6576:     public function handleeID($uid, $conf,$pObj,$messagetodisplay,$refreshurl)
 *
 *              SECTION: Reply on comments functions
 * 6639:     protected function getCommentBoxDisplay($commentid,$conf,$level,$fromAjax)
 * 6698:     protected function getCommentBoxChildrenDisplayIsCollapsed($commentschildrenids,$conf,$level,$fromAjax)
 *
 *              SECTION: Fe-usergroup functions
 * 6752:     protected function usersGroupmembers($pObj, $fromAjax)
 *
 *              SECTION: Recent comments functions
 * 6798:     public function getRecentComments($pObj, $conf,$feuserid)
 * 6862:     protected function comments_getRecentComments($rows,$conf,$pObj)
 * 7004:     protected function trimContent($text,$conf)
 * 7031:     protected function createRCLinks($text, $refID, $commentID, $prefix, $table, $externalprefix, $singlePid, $conf)
 *
 *              SECTION: Report comments functions
 * 7072:     protected function getCommentsReportLink($params, &$pObj, $conf ,$fromAjax,$pid)
 * 7107:     public function mainReport($content, $conf, $pObj, $piVars)
 * 7131:     protected function processReportForm(array &$errors,$conf, $pObj, $piVars)
 * 7259:     protected function showReportThanks($conf,$pObj)
 * 7272:     protected function showReportForm(array $errors,$conf,$pObj,$piVars)
 * 7355:     protected function getReportCaptcha($required, $error,$conf,$pObj)
 *
 *              SECTION: IP-blocking functions
 * 7399:     protected function IPBlockSpamCheck(&$params, &$pObj)
 * 7418:     protected function sendNotificationIPBlock($params, &$pObj)
 * 7432:     private function getIpAddr()
 * 7447:     private function checkTableBLs($ipaddr)
 * 7458:     private function checkLocalBL($ipaddr)
 * 7471:     private function checkStaticBL($ipaddr)
 * 7498:     function checkNetworkBLs($ipaddr)
 * 7528:     function makeiLikeText($conf, $pObj, $cmd, $ref, $fromAjax, $myrating = array(),$mylikeval,$mydis='',$template,$cid, $commentspics,$extpreffortext)
 *
 * TOTAL FUNCTIONS: 94
		...
		if ($conf['ratings.']['ratingsOnly'] ==0) {
			...
			if ($conf['advanced.']['commentsShowCount'] ==1) {
				$tmpwhere=' approved=1 AND ' . ($tmpint ?
					...
					$this->enableFields('tx_toctoc_comments_comments',$pObj) . $whereplus . ' AND (' . $pObj->where_dpck . ')';
			...
			//print  '<br>where:' .$pObj->where . '<br>3:' .  $pObj->where_dpck;
			list($row) = $GLOBALS['TYPO3_DB']->exec_SELECTgetRows('COUNT(*) AS counter',
			...
			//print  '<br>where; ' . $pObj->where . '<br>4: ' .  $pObj->where_dpck;//exit;
...
// for design 2, buttons
				...
				if ((intval($conf['ratings.']['useDislike']) ===1 ) && ($articlerating['idislike']!='')) {
					...
					if ($articlerating['ilike']!='') {
						...
						if ($articlerating['ilike'] !='') {
							...
							//$ilikedislikeHTML= substr($ilikedislikeHTML,1);
						}
			...
			if (count($this->AJAXimages)==0) {
				if (count($_SESSION['AJAXimages'])!=0) {
					...
					$start_time_for_conversion = microtime();
				 	...
				 	$rowsfeuserimages = $GLOBALS['TYPO3_DB']->exec_SELECTgetRows('fe_users.uid AS uid, '. $usergenderexistsstr .' fe_users.image AS image, fe_users.lastlogin AS lastlogin, fe_users.is_online AS is_online, fe_users.last_name AS lastname', 'fe_users,tx_toctoc_comments_comments',
				...
				if ($info['t'] > 0) {
					...
					if ($isSpam < $cutOffPoint) {
						...
						if (!$isApproved) {
							...
							if (intval($_SESSION['commentListIndex']['cid' . $_SESSION['commentListRecord']]['startIndex']) !=0) {
	...
	*/
	...
	protected function cleanupdbandfiles($conf) {
		...
		if ($dirsep == '\\') {
			// windows
			$txdirname= str_replace('/','\\',str_replace($repstr,'',dirname(__FILE__)) . $dirsep . $this->webpagepreviewsavefolder);
		} else {
			$txdirname=str_replace($repstr,'',dirname(__FILE__)) . $dirsep . $this->webpagepreviewsavefolder;
		}
	...
	protected function commentShowWebpagepreview ($rowattachmentid,$rowattachment_subid,$conf,$pObj,$cid,$topwebsitepreview) {
		...
		if (count($rowsatt)>0) {
			...
			if ($rowsatt[0]['photos_etc']!='') {
				...
				$picstr .= '<div>';
				$picstr .= '<img title="' . $rowsatt[0]['title'] . '" style="display: block;" src="/' . $this->webpagepreviewsavefolder . $rrpicpic . '" ';
