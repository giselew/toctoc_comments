 ...
 *  290:     public function maincomments($ref, $conf = null, $fromAjax = FALSE, $pid=0, $feuserid = 0, $cmd = '', &$pObj, $piVars = array(),
 ...
 *  551:     protected function checkExternalUid($conf, $pObj)
 *
 *              SECTION: Commenting functions
 *  579:     public function comments($conf,&$pObj, $fromAjax, $feuserid, $pid)
 * 1916:     protected function makeemoji($text, $conf, $debug)
 * 2038:     public function comments_getComments(&$rows, $conf, $pObj, $feuserid, $fromAjax, $pid, $levelhlt=0)
 * 2966:     protected function comments_getComments_getRatings(&$row, $conf, $pObj, $feuserid, $fromAjax, $scopeid=0)
 * 2985:     protected function comments_getComments_getEmail($email)
 * 2999:     protected function isCommentingClosed($conf, $pObj)
 * 3046:     protected function commentingClosed($pObj, $fromAjax, $conf)
 * 3067:     protected function comments_getCommentsBrowser($rpp, $startpoint, $totalrows, $pObj, $fromAjax, $conf)
 * 3185:     protected function build_AJAXImages ($conf, $pObj, $usergenderexistsstr='', $fromAjax=FALSE)
 * 3359:     public function comments_getComments_fe_user($params, $conf, $pObj, $commentid, $fromAjax, $commentusername)
 * 3456:     protected function fixFeUserPic ($conf, $pic)
 * 3475:     protected function addleadingspace($text)
 * 3501:     protected function check_comment_ownership ($rowexternal_ref, $feuserid)
 * 3523:     public function previewcomment($data, $pObj, $conf, $fromAjax=TRUE)
 *
 *              SECTION: functions for images cache (AJAX)
 * 3551:     protected function setAJAXimage($image, $feuserid)
 * 3562:     protected function getAJAXimage($feuserid, $commentid)
 * 3593:     protected function setAJAXimageCache($image, $imageoriginal, $feuserid)
 * 3604:     protected function getAJAXimageCache($commentuserimageout)
 * 3618:     protected function makeImageSprite($conf)
 *
 *              SECTION: Form functions
 * 3766:     public function form($conf, &$pObj, $piVars, $fromAjax, $pid, $ifeuserid=0, $userpic, $commentid=0, $replylevel=0, $fromcomments = FALSE, $cidfromajaxcomments = 0)
 * 4563:     protected function form_updatePostVarsWithFeUserData($conf, $piVars, $feuserid, $fromAjax, $userpic, $cid, $output_cid)
 * 4876:     protected function form_getCaptcha($pObj, $conf, $fromAjax)
 * 4924:     protected function form_wrapError($field, $pObj, $conf)
 *
 *              SECTION: Comments page-browser functions
 * 4957:     protected function processBrowserSubmission($conf, $pObj, $piVars, $fromAjax, $cid, $cmd, $pid, $ref)
 *
 *              SECTION: Inserting and Updating Comments functions
 * 5021:     public function processSubmission($conf, $pObj, $piVars, $fromAjax, $feuserid, $pid, $lang)
 * 5456:     public function updateComment($conf, $pObj, $ctid, $content, $pid, $plugincacheid)
 * 5529:     protected function processSubmission_checkTypicalSpam($pObj, $conf, $piVars, $lang, $fromAjax)
 * 5602:     protected function processSubmission_validate($piVars, $conf, $pObj, $fromAjax)
 * 5673:     public function sendNotificationEmail($uid, $plugincacheid, $points, $action, $conf, $pObj, $fromAjax, $piVars, $pid, $fetoctocusertoinsert, $attachment_id=0 , $attachment_subid=0, $optinemail='', $ratingpageid=0)
 * 6119:     protected function emailOptIn($conf, $optinemail, $optin_ip)
 * 6187:     protected function checkCOI($conf, $email, $checkip=TRUE)
 *
 *              SECTION: language handling functions
 * 6220:     public function fixLL(&$conf)
 * 6241:     protected function fixLL_internal($LL, &$ll, $prefix = '')
 *
 *              SECTION: mostly formatting functions
 * 6264:     protected function createLinks($text, $conf)
 * 6286:     protected function applyStdWrap($text, $stdWrapName, $conf = null, $pObj = null)
 * 6305:     protected function checkCustomFunctionCodes($code, $pObj)
 * 6327:     protected function isNoCacheUrl($url)
 * 6348:     protected function substituteMarkersAndSubparts($template, array $markers, array $subparts, $pObj)
 *
 *              SECTION: session and cache functions
 * 6374:     public function resetSessionVars($resetcontext, $alsoajaxvar = TRUE)
 * 6424:     public function getClearCacheIds($conf, $pid = 0, $repectsession = TRUE)
 * 6483:     protected function getClearCacheExternal_ref_uids($conf, $external_ref_uid = '', $repectsession = TRUE)
 * 6507:     public function setPluginCacheControlTstamp ($external_ref_uid_list)
 * 6570:     public function initCaches()
 * 6621:     protected function clearPagesCaches($conf, $pid, $plugincacheid)
 *
 *              SECTION: Smilie functions
 * 6656:     public function parseSmilieArray($data)
 * 6673:     public function replaceSmilies($content, $conf)
 * 6690:     protected function checkbbcrop($content, $commentCropLength)
 * 6716:     protected function replaceBBs($content, $conf, $purge=FALSE)
 *
 *              SECTION: functions for the AJAx interface with jQuery/JavaScript
 * 6758:     protected function getAjaxJSData($feuserid, $pid, $languagecode, $conf, $pObj, $cid, $fromAjax, $dataonly=FALSE, $externalref)
 * 6823:     protected function getAjaxData($feuserid, $pid, $languagecode, $conf, $cid, $ref)
 * 6862:     protected function updateAjaxData($feuserid)
 * 6879:     protected function getAjaxLoggedInData($forjsvariable, $outputloginstate)
 * 6932:     protected function getAjaxDataAttachments($conf, $fromAjax, $pObj)
 * 6953:     protected function getAjaxJSDataCommentImgs($cid, $pObj, $fromAjax)
 * 6989:     protected function getAjaxJSDataComments($cid, $pObj, $fromAjax)
 * 7027:     protected function getAjaxDataComments($pObj, $cid)
 * 7043:     protected function getAjaxDataImgs()
 *
 *              SECTION: URL- and IP-related functions
 * 7059:     public function getCurrentIp()
 * 7073:     public function hasValidItemUrl($piVars)
 * 7094:     public function getDefaultConfig($pluginkey='')
 * 7107:     protected function ae_detect_ie()
 *
 *              SECTION: Rating functions
 * 7135:     public function getRatingDisplay($ref, $conf = null, $fromAjax = 0, $pid=0, $returnasarray = FALSE, $feuserid = 0, $cmd = 'vote',
			$pObj = null, $cid, $fromcomments, $commentspics = array(), $scopeid=0)
 * 7191:     public function isVoted($ref, $conf, $pObj, $scopeid=0, $feuserid=0)
 * 7227:     protected function getBarWidth($rating, $conf)
 * 7241:     protected function getRatingInfo($ref, $pObj, $feuserid=-1, $conf, $scopeid=0)
 * 7366:     protected function generateRatingContent($ref, $template, $conf, $fromAjax, $pid, $returnasarray, $feuserid, $pObj, $cmd, $cid, $fromcomments, $commentspics, $scopeid = 0)
 * 8247:     function makeiLikeText($conf, $pObj, $cmd, $ref, $fromAjax, $myrating = array(), $mylikeval, $mydis='', $template, $cid, $commentspics, $extpreffortext)
 *
 *              SECTION: TYPO3 workarounds
 * 8640:     public function enableFields($tableName, $pObj)
 * 8660:     public function pi_getLLWrap($pObj, $llkey, $fromAjax)
 * 8695:     protected function t3getSubpart ($pObj, $templateCode, $templateMarker)
 * 8718:     protected function t3substituteMarkerArray ($pObj, $content, $markContentArray, $wrap = '', $uppercase = 0, $deleteUnused = 0)
 * 8740:     protected function t3substituteMarker ($pObj, $template, $marker, $markContent)
 * 8754:     protected function t3fileResource ($pObj, $usetemplateFile)
 * 8771:     protected function getcheck ($ref, $i, $ajaxData, $ratingscheck)
 * 8792:     protected function formatDate($date, $pObj, $fromAjax, $conf)
 * 8918:     protected function getPageURL($fromAjax = FALSE, $pid = 0)
 * 8940:     protected function locationHeaderUrlsubDir($withleadingslash = TRUE)
 * 8962:     protected function getCleanHTML ($html)
 *
 *              SECTION: Usercards
 * 8991:     public function getSmiliesCard($conf)
 * 9256:     public function getUserCard($basedimgstr, $basedtoctocuid, $conf, $pObj, $commentid)
 *
 *              SECTION: Mail functions
 * 9555:     protected function getMailTo($mailAddress, $linktxt = '', $initP = '?')
 * 9598:     protected function encryptEmail($string, $back=0)
 * 9634:     protected function encryptCharcode($n, $start, $end, $offset)
 * 9656:     private function send_mail ($toEMail, $subject, $message, $html, $fromEMail, $fromName, $attachment='')
 * 9809:     protected function slashName ($name, $apostrophe='"')
 *
 *              SECTION: Commentator-Notification functions
 * 9833:     public function handleCommentatorNotifications($uid, $conf, $pObj, $fromeID = FALSE, $pid=0, $fromAjax=1)
 *
 *              SECTION: Administrator comment response functions
 * 10025:     protected function getCommentsReponse(&$params, &$pObj, $conf)
 *
 *              SECTION: Website preview functions
 * 10069:     public function cleanupfup($cmd, $cid, $data, $pObj, $fromAjax, $uploadedfile, $conf, $originalfilename)
 * 10089:     public function getwebpagepreview($cmd, $cid, $data, $pObj, $fromAjax, $websitepreviewid=0, $conf)
 * 10134:     protected function getpreviewinit($cid, $data, $pObj)
 * 10172:     protected function savewebpagepreviewtodb($pcid, $pcommentid, $userurl, $conf, $pObj)
 * 10385:     protected function cleanupdbandfiles($conf, $uploadedfile='', $originalfilename='')
 * 10517:     protected function getwebpagecache($pcid, $pcommentid, $conf, $url='', $isbeforefetch = FALSE, $pObj)
 * 10669:     protected function read_dir($dir, $array = array())
 * 10697:     protected function commentShowWebpagepreview ($rowattachmentid, $rowattachment_subid, $conf, $pObj, $cid, $topwebsitepreview, $fromAjax, $row = array(), $isforemailnotification = FALSE)
 * 11124:     protected function makeAttachementPicture($picturefilename, $conf, $descriptionbyuser, $originalfilename, $firstname, $lastname, $fetoctocusertoinsert)
 *
 *              SECTION: eID-Interface functions
 * 11293:     public function handleeID($uid, $conf, $pObj, $messagetodisplay, $refreshurl)
 *
 *              SECTION: Reply on comments functions
 * 11390:     protected function getCommentBoxDisplay($commentid, $conf, $level, $fromAjax, $triggeredlevel=0, $levelexpandoverride=0)
 * 11455:     protected function getCommentBoxChildrenDisplayIsCollapsed($commentschildrenids, $conf, $level, $fromAjax, $triggeredlevel=0, $levelexpandoverride=0)
 *
 *              SECTION: Fe-usergroup functions
 * 11516:     public function usersGroupmembers($pObj, $fromAjax, $conf, $communitybuddies = FALSE)
 * 11583:     protected function isUserOnline($feuser_uid)
 *
 *              SECTION: Recent comments functions
 * 11620:     public function getRecentComments($pObj, $conf, $feuserid)
 * 11681:     protected function comments_getRecentComments($rows, $conf, $pObj)
 * 11963:     protected function trimContent($text, $conf, $maxChars=0, $dospecialChars=TRUE)
 * 12001:     protected function createRCLinks($text, $refID, $commentID, $prefix, $table, $externalprefix, $singlePid, $conf, $show_uid, $okrowsi)
 *
 *              SECTION: Report comments functions
 * 12061:     protected function getCommentsReportLink($params, &$pObj, $conf , $fromAjax, $pid)
 * 12105:     public function mainReport($content, $conf, $pObj, $piVars)
 * 12129:     protected function processReportForm(array &$errors, $conf, $pObj, $piVars)
 * 12265:     protected function showReportThanks($conf, $pObj)
 * 12279:     protected function showReportForm(array $errors, $conf, $pObj, $piVars)
 * 12360:     protected function getReportCaptcha($required, $error, $conf, $pObj)
 *
 *              SECTION: IP-blocking functions
 * 12404:     protected function IPBlockSpamCheck(&$params, &$pObj)
 * 12421:     protected function sendNotificationIPBlock($params, &$pObj)
 * 12434:     protected function checkSMTPService($hostname, $port)
 * 12456:     private function getIpAddr()
 * 12471:     private function checkTableBLs($ipaddr)
 * 12482:     private function checkLocalBL($ipaddr)
 * 12495:     private function checkStaticBL($ipaddr)
 * 12521:     private function checkNetworkBLs($ipaddr)
 * 12545:     protected function trackdebug ($trackingfunction)
 *
 *              SECTION: Top ratings functions
 * 12572:     public function showtopRatings($conf, $pObj)
 * 12586:     protected function topratings ($conf, $pObj)
 *
 * TOTAL FUNCTIONS: 124
...
class toctoc_comment_lib  extends tslib_pibase {
	...
	private $expandiconCSSmargin = 'margin: 6px 0 0 4px;'; //                *** fixed here, apart for boxmdel koogle (margin of the icon for expanding relpies on comments)
	private $firstCSSmargin='margin: 0 4px;';  //                            *** fixed here, used for eID-requests, f(handleeID), it's the margin of the "processing-image"
	...
	private $taareaCSSheightinit=32;  //                                     *** will be later = 4 + (intval($conf['theme.']['boxmodelTextareaLineHeight'])*intval($conf['theme.']['boxmodelTextareaNbrLines']))  + (2 * intval($conf['theme.']['boxmodelSpacing']));
	...
	public function maincomments($ref, $conf = null, $fromAjax = FALSE, $pid=0, $feuserid = 0, $cmd = '', &$pObj, $piVars = array(),
		...
		$content = '';
		if (count($confSess)>0) {
			//$dgbsess = 'lastTotalrows1: ' . $confSess['lastTotalrows'];
			...
			if (is_array($confSess['ratingsscopesinternalm1table'])) {
				$_SESSION['ratingsscopesinternalm1table'] = array_replace_recursive($_SESSION['ratingsscopesinternalm1table'],$confSess['ratingsscopesinternalm1table']);
			}
			...
			//return $dgbsess . '<br>startpoint: ' . $pObj->startpoint . '<br><br>';exit;
		}
		...
		if ($fromAjax === TRUE) {
			...
			if ($this->checkExternalUid($conf, $pObj)) {
				...
				if ($cmd === 'showform') {
					...
					//print '<br>' . $userAJAXimag . '<br>' ;
					$content .= $this->form($conf, $pObj, $piVars, $fromAjax, $pid=0, $feuserid, $_SESSION['userAJAXimage'],0,0,FALSE,$ref);
				}
				...
				if (($cmd === 'browse') || ($cmd === 'browsehide')) {
					...
					$this->AJAXimages = $commentspics;
					$content .= $this->comments($conf, $pObj, $fromAjax, $feuserid, $pid) . '<div class="dummy"></div><div class="dummy2">' . $this->ajaxDataloginSess . '</div>';
					$content=$this->topareaHTML.$content;
					if ( $content=='') {
				...
				if ($commentscounter>=$conf['advanced.']['commentsShowCountLevel']) {
					if ($commentscounter==1) {
						if ($conf['advanced.']['commentsShowCountText'] ==0) {
							...
							// with text short
							$totalcommentscount='<span class="tx-tc-nbrofcomments srt">1 </span>' . $this->pi_getLLWrap($pObj, 'comments_recent.tx_toctoc_comments_comments', $fromAjax) ;
							$totalpostscount='<span class="tx-tc-nbrofcomments srt">1 </span>' . $this->pi_getLLWrap($pObj, 'comments_recent.tx_toctoc_comments_comments', $fromAjax);
						}
					} else {
						if ($conf['advanced.']['commentsShowCountText'] == 0) {
							// with old standard: text explicit
							if ($commentscounter==0) {
								...
								$totalcommentscount='<span class="tx-tc-nbrofcomments">'. $commentscounter . '</span> '. $this->pi_getLLWrap($pObj, 'pi1_template.commentsavailable', $fromAjax)  ;
								$totalpostscount='<span class="tx-tc-nbrofcomments">'. $commentscounter . '</span> ' . $this->pi_getLLWrap($pObj, 'pi1_template.postsavailable', $fromAjax)  ;
							}
						} elseif ($conf['advanced.']['commentsShowCountText'] == 1) {
							// only number
							...
							if ($commentscounter==0) {
								...
								$totalcommentscount='<span class="tx-tc-nbrofcomments srt">'. $commentscounter . ' </span>'.  $this->pi_getLLWrap($pObj, 'pi1_template.text_views_viewcountcomments', $fromAjax) ;
								$totalpostscount='<span class="tx-tc-nbrofcomments srt">'. $commentscounter . ' </span>'. $this->pi_getLLWrap($pObj, 'pi1_template.text_views_viewcountcomments', $fromAjax)  ;
							}
						}
					}
					if ($conf['prefixToTableMap.'][$conf['externalPrefix']] == 'fe_users') {
 ...
 WHERE deleted= 0 AND pid='.$conf['storagePid']. ' AND seen>0 AND reference = "'.$_SESSION['commentListRecord'].'"';
				...
				if (($conf['advanced.']['showCountViews'] ==1) && ($onlyCounting ==FALSE)) {
					...
					if ($conf['advanced.']['commentsShowCount'] ==1) {
						//$totalcommentsviewcount.= '<br class="tx-tc-br-articleview-2">';
					}
				} else {
				...
				$totalcommentsviewcount.= $this->t3substituteMarkerArray($pObj, $templatecommentviewscount, $markerscommentviewscount);
			}
		...
		for ($i=0;$i<count($articlerating);$i++){
			...
			if (trim($articlerating[$i]['voteing'])!='') {
				...
				$voteHTML.=$divstart.str_replace('<div class="tx-tc-rts-container">',
						...
						' .tx-tc-rts-vote-bar div span[title]\').tooltip({position: \'top right\', offset: [5,-5],effect: \'fade\', opacity: 1, tipClass: \'tooltip2\'});jQuery(\'#tx-tc-rts-disp2-'.
						$scopeid.' .tx-tc-rts-vote-bar div a[title]\').tooltip({position: \'top right\',offset: [5,-5],effect: \'fade\',opacity: 1, tipClass: \'tooltip2\'});jQuery(\'#tx-tc-rts-dp-'.
						$scopeid.' .tx-tc-rts-vote-bar div span[title]\').tooltip({position: \'top right\', offset: [5,-5],effect: \'fade\', opacity: 1, tipClass: \'tooltip2\'});jQuery(\'#tx-tc-rts-dp-'.
						$scopeid.' .tx-tc-rts-vote-bar div a[title]\').tooltip({position: \'top right\',offset: [5,-5],effect: \'fade\',opacity: 1, tipClass: \'tooltip2\'});">'.
						$scopetitlehtml,
		...
		if ((intval($conf['ratings.']['useShortTopLikes']) == 1) || (intval($conf['ratings.']['useLikeDislikeStyle']) == 1)) {
			//print 'id="tx-tc-myrts-dp-'.$externalrefscope.'-"';
			$repl='id="tx-tc-myrts-dp-'.$externalrefscope.'-';
			$replw='style="display:none;" id="tx-tc-myrts-dp-'.$externalrefscope.'-';
			$wrkareaHTML=str_replace('id="tx-tc-myrtstop-'.$externalrefscope.'"', 'id="tx-tc-myrtstop-'.$externalrefscope.'" style="display:none;"',$topareaHTML);
	...
	public function comments_getComments(&$rows, $conf, $pObj, $feuserid, $fromAjax, $pid, $levelhlt=0) {
		...
		foreach ($rows as $row) {
			...
			$txthaschildern='';
			//if (($txthaschildern != '') || ($conf['advanced.']['userCommentResponseLevels'] != 0) && ($this->isCommentingClosed($conf, $pObj) == 0) && ($row['level']<$conf['advanced.']['userCommentResponseLevels'])) {
			if (($conf['advanced.']['userCommentResponseLevels'] != 0) && ($this->isCommentingClosed($conf, $pObj) == 0) && ($row['level']<$conf['advanced.']['userCommentResponseLevels'])) {
				...
				if (($conf['advanced.']['replyModeInline']==1)) {
					...
					$subcommentreply.= '<div class="tx-tc-ct-rply" style="margin: 0 0 0 ' . $spacingleft . 'px;' . $cssdisplayreplybox . '" id="tx-tc-cts-rply-' . $row['uid'] . '">';
				...
				if ($row['children'] != '') {
					...
					if ($conf['advanced.']['displayChildComments']==1) {
						...
						if ($nbrchildren==1) {
							$txthaschildern= '<span id="tx-tc-cts-explink-' . $row['uid'] . '" class="tx-tc-nbrsubcomments"><a class="tx-tc-lnkexp-2" href="javascript:void(0)" rel="nofollow"
									onclick="tctrvw(0, ' . $row['uid'] . ', \'' . $row['children'] . '\', \'' . $row['allchildren'] . '\')">' .$nbrchildren . ' ' . $this->pi_getLLWrap($pObj, 'pi1_template.answer', $fromAjax);
						} else {
						 	$txthaschildern= '<span id="tx-tc-cts-explink-' . $row['uid'] . '" class="tx-tc-nbrsubcomments"><a class="tx-tc-lnkexp-2" href="javascript:void(0)" rel="nofollow"
									onclick="tctrvw(0, ' . $row['uid'] . ', \'' . $row['children'] . '\', \'' . $row['allchildren'] . '\')">' . $nbrchildren . ' ' . $this->pi_getLLWrap($pObj, 'pi1_template.answers', $fromAjax);
						}
						if ($nbrchildren!=$nbrallchildren) {
							if ($nbrallchildren==0) {
								$txthaschildern .= '';
							} elseif ($nbrallchildren==1) {
								$txthaschildern .= ', ' . $this->pi_getLLWrap($pObj, 'pi1_template.total', $fromAjax) . ' ' .  $nbrallchildren . ' ' . $this->pi_getLLWrap($pObj, 'pi1_template.subcomment', $fromAjax);
							} else {
								$txthaschildern .= ', ' . $this->pi_getLLWrap($pObj, 'pi1_template.total', $fromAjax) . ' ' .  $nbrallchildren . ' ' . $this->pi_getLLWrap($pObj, 'pi1_template.subcomments', $fromAjax);
							}
						}
			...
			if (intval($conf['advanced.']['countCommentViews']) ==1 ) {
				$jscommentviewcounter='<script type="text/javascript">if (!cntctrl' . $row['uid'] . ') {var cntctrl' . $row['uid'] . '=comments_view(\'tx_toctoc_comments_comments_'.$row['uid'].'\',\'' . $_SESSION['feuserid'] . '\', ' . $commentAjaxDatavar .', 1);};</script>';
		 ...
		 WHERE deleted= 0 AND pid='.$conf['storagePid']. ' AND seen>0 AND reference = "tx_toctoc_comments_comments_'.$row['uid'].'"';
			...
			if (($conf['advanced.']['useEmoji'] > 0) || is_array($conf['smilies.'])) {
				if ($haseditlink!=TRUE) {
					if (($conf['ratings.']['useLikeDislikeStyle'] == 0) || ($conf['useUserImage'] == 0)) {
					...
					$tippjsemoji=' onmouseover="jQuery(\'#' . $cssidpart . $iuid . ' .emoji[title]\').tooltip({offset: [-1,0],effect: \'fade\',opacity: 1, tipClass: \'tooltipemoji\'});jQuery(\'#' . $cssidpart . $iuid.' img[title]\').tooltip({offset: [-1,0],effect: \'fade\',opacity: 1, tipClass: \'tooltipemoji\'});" ';
				}
			}
			$commentusername=trim($row['firstname'] . ' ' . $row['lastname']);
			$allratingmarkers=$this->comments_getComments_getRatings($row, $conf, $pObj, $feuserid, $fromAjax,0);
			if ($conf['ratings.']['useLikeDislikeStyle']==1) {
				//print $allratingmarkers;exit;
				$allratingmarkersarr=explode('<div class="tx-tc-overrating-date">', $allratingmarkers);
				//print count($allratingmarkersarr);
						$allratingmarkersarrrest=explode('<p id="tx-tc-ct-box-cttxt', $allratingmarkersarr[1]);
						//print count($allratingmarkersarrrest);
				$allratingmarkers=$allratingmarkersarr[0].$allratingmarkersarrrest[1];
				//print $allratingmarkersarrrest[1];exit;
				//$allratingmarkers=str_replace('id="tx-tc-ct-box-cttxt-','id="tx-tc-ct-box-cttxt-start-', $allratingmarkers);
				...
				//print  $extractedcommentdate . '<br>' ;exit;
				//$extractedcommentdate=str_replace('id="tx-tc-ct-box-cttxt-','id="tx-tc-ct-box-cttxt-start-', $extractedcommentdate);
				$brratingsovercomment='<br>';// class="tx-tc-brovercomment">';
				$idchange = '';
				if ($conf['ratings.']['useVotes']==1) {
					//if ($conf['ratings.']['useLikeDislike']==1) {
						$idchange = '-start';
					//print $extractedcommentdate;exit;
					//}
					...
					//$allratingmarkersarrvtrtsdp=explode('tx-tc-rts-dp', $allratingmarkersarrvt[1]);
					$extractedcommentvoting =  substr($allratingmarkersarrvt[1],0,(strlen($allratingmarkersarrvt[1])-23));
					$extractedcommentvoting =str_replace('<span','<div','<span class="tx-tc-overrating-voting" id="tx-tc-rts-disp2-tx_toctoc_comments_comments_' . $row['uid'] . '"><span class="tx-tc-rts-dp">'. $extractedcommentvoting . '</span><br>');
					$extractedcommentvoting = '</p>' . str_replace('</span','</div', $extractedcommentvoting) . '<p class="tx-tc-text" id="tx-tc-ct-box-cttxt'.$idchange . '-' . $row['uid'] .'">';
					...
					//$extractedcommentdate=str_replace($extractedcommentvoting,'' , $extractedcommentdate);
					$extractedcommentdate=$newdatec. $extractedcommentvoting;
					//print $extractedcommentvoting;exit;

				} else {

				}
				//print $conf['ratings.']['useVotes'] .', ' .$extractedcommentdate;exit;
				$ratingsundercomment='';
				$ratingsovercomment=$allratingmarkers;
				$namepart= '<span style="float:left;">'.  $namepart;
				if ($conf['advanced.']['useNameCommentSeparator'] == 1) {
				...
				if (count($_SESSION['AJAXimages'])!=0) {
					...
					if ($conf['advanced.']['FeUserDbField'] != 'image') {
				 	...
				 	$rowsfeuserimages = $GLOBALS['TYPO3_DB']->exec_SELECTgetRows('fe_users.uid AS uid, '. $usergenderexistsstr .' fe_users.image AS image, ' .
							...
							'fe_users.uid=CONVERT(SUBSTRING(toctoc_comments_user,9), SIGNED INTEGER) AND fe_users.deleted=0 AND tx_toctoc_comments_feuser_mm.deleted=0',
					...
					foreach($rowsfeuserimages as $keyimage) {
						...
						if ($tmpimgstr==='') {
							...
							$img['params'] = 'style="' . $userimagestyle . '" class="'.$profileimgclass . $classonline . '" title="'. $usernametitle .'"  id="tx-tc-cts-img-"';
	...
	*/
	...
	public function form($conf, &$pObj, $piVars, $fromAjax, $pid, $ifeuserid=0, $userpic, $commentid=0, $replylevel=0, $fromcomments = FALSE, $cidfromajaxcomments = 0) {
		...
		if ($conf['advanced.']['loginRequired'] == 1) {
			...
			$loggoutlink = '<a href="javascript:void(0)" rel="nofollow" onmouseout"overlogout=0;" onmouseover="overlogout=1;" onclick="tt_logout(\'' . $tmpcid . '\',\'' . $externalref . '\');return;">'. $this->pi_getLLWrap($pObj, 'pi1_template.logout', $fromAjax) .'</a>';
		...
		if ($conf['advanced.']['watermarkFormFields'] == 1) {
			...
			$label_FIRST_NAME = '<label class="tx-tc-ct-label" for="toctoc_comments_pi1_firstname" style="width: '.$labelwidth.'px">'.$TEXT_FIRST_NAME.$tREQUIRED_FIRSTNAME.'</label>';
			$label_LAST_NAME = '<label class="tx-tc-ct-label" for="toctoc_comments_pi1_lastname" style="width: '.$labelwidth.'px">'.$TEXT_LAST_NAME.$tREQUIRED_LASTNAME.'</label>';
			$label_EMAIL = '<label class="tx-tc-ct-label" for="toctoc_comments_pi1_email" style="width: '.$labelwidth.'px">'.$TEXT_EMAIL.$tREQUIRED_EMAIL.'</label>';
			$label_WEB_SITE = '<label class="tx-tc-ct-label" for="toctoc_comments_pi1_homepage" style="width: '.$labelwidth.'px">'.$TEXT_WEB_SITE.$tREQUIRED_HOMEPAGE.'</label>';
			$label_LOCATION = '<label class="tx-tc-ct-label" for="toctoc_comments_pi1_location" style="width: '.$labelwidth.'px">'.$TEXT_LOCATION.$tREQUIRED_LOCATION.'</label>';
			$pcehldr_FIRST_NAME = '';
			$pcehldr_LAST_NAME = '';
			$pcehldr_EMAIL = '';
			$pcehldr_WEB_SITE = '';
		...
		if ($feuserid == 0) {
			if (intval($conf['advanced.']['loginRequired'])==0) {
				...
				$subformloginhtml=$this->t3substituteMarkerArray($pObj, $subformTemplate,
						array(
							'###THEMEOPACITY###'=> $themeopacity,
							'###SUBMITONCLICK###' => $submithtml,
							'###CID###' => $output_cid,
	...
	protected function form_getCaptcha($pObj, $conf, $fromAjax) {
		...
		if ($captchaType > 2) {
			print 'spamProtect.useCaptcha = "' . $conf['spamProtect.']['useCaptcha'] . '" is invalid'; exit;
		}
		if ($captchaType == 1) {
	 ...
	 * @return	void
	 */
	public function sendNotificationEmail($uid, $plugincacheid, $points, $action, $conf, $pObj, $fromAjax, $piVars, $pid, $fetoctocusertoinsert, $attachment_id=0 , $attachment_subid=0, $optinemail='', $ratingpageid=0) {
		...
		if (($action=='approve') ||  ($action=='info') ||  ($action=='rating')) {
			if (t3lib_div::validEmail($toEmail) && t3lib_div::validEmail($fromEmail)) {
				...
				if ($conf['HTMLEmail']) {
					...
					if ($action=='rating') {
						$infoleft='<b>' . $this->pi_getLLWrap($pObj, 'pi1_template.informationaboutdislikeevaluator', $fromAjax) . '</b><br />' . $this->getUserCard('', $fetoctocusertoinsert, $confwrk, $pObj, $uid);
					} else {
					...
					$contentformail=$this->replaceBBs($contentformail, $conf, FALSE);
					$contentformail =$this->addleadingspace($contentformail);
					$saveconfemoji=$conf['advanced.']['useEmoji'];
				...
				if ($action == 'info') {
					...
					$urlofcomment='<a href="' . $_SESSION['commentsPageIds'][$ratingpageid] . $qssep .'toctoc_comments_pi1[anchor]='. substr($conf['recentcomments.']['anchorPre'],1) . $uid . $conf['recentcomments.']['anchorPre'].$uid . '">' . $_SESSION['commentsPageTitles'][$ratingpageid] . '</a>';
...
var commentsAjaxDataAtt" . $cid . " = '" . rawurlencode($ajaxDataAttachments) . "';" . $ajaxDataloginstr . $ajaxDatalogoutstr . $ajaxDataloginSessstr . "</script>";
	...
	protected function updateAjaxData($feuserid) {
		...
		return rawurlencode($_SESSION['ajaxData']);
	}
	/**
			...
			if ($tmpint) {
				...
				if ($conf['advanced.']['showCountCommentViews'] ==1){
					$recs['commentviews'] = $GLOBALS['TYPO3_DB']->exec_SELECTgetRows('SUM(seen) AS commentviews',
							'tx_toctoc_comments_feuser_mm',
							'pid=' . intval($conf['storagePid']) .
							' AND seen>0 AND reference="tx_toctoc_comments_comments_' . $commentidcandidate . '" '. $this->enableFields('tx_toctoc_comments_feuser_mm', $pObj));
				} else {
	...
	protected function generateRatingContent($ref, $template, $conf, $fromAjax, $pid, $returnasarray, $feuserid, $pObj, $cmd, $cid, $fromcomments, $commentspics, $scopeid = 0) {
		...
		if (($cmd=='vote') || ($cmd=='votearticle')) {
			...
			if ($myrating['comment'][0]['commentdate']) {
				...
				if (intval($conf['advanced.']['countCommentViews']) ==1 ) {
					$jscommentviewcounter='<script type="text/javascript">if (!cntctrl' .$myrating['comment'][0]['commentuid']  . ') {var cntctrl' .$myrating['comment'][0]['commentuid']  . '=comments_view(\'tx_toctoc_comments_comments_'.$myrating['comment'][0]['commentuid'] .'\',\'' . $_SESSION['feuserid'] . '\', \'' . $ajaxData .'\', 1, '. $conf['storagePid'] .');};</script>';
				...
				if (intval($conf['advanced.']['showCountCommentViews']) ==1 ) {
					if ($myrating['commentviews'][0]['commentviews'] > 0) {
						...
						$txtviews= '<span class="tx-tc-commentviews">&nbsp;' . $myrating['commentviews'][0]['commentviews'] . ' ' . $strviews. $commentcontinuation. '</span>';
						if ($commentcontinuation!='') {
							$stylecomment=' style="float:left;"';
						}
						$commentcontinuation='';
		...
		} elseif   ((trim($conf['externalPrefix'])=='tx_mininews_pi1') || (trim($conf['externalPrefix'])=='tx_ttnews') || (trim($conf['externalPrefix'])=='tx_news_pi1')) {
		...
		if (($conf['ratings.']['useLikeDislikeStyle']==1) || ((intval($conf['ratings.']['useShortTopLikes']) == 1) && ($cmd=='votearticle')))  {
			$mypiclikeSub = $this->t3getSubpart($pObj, $template, '###ILIKE_STATIC_SUB###');
			$mypiclikehtmlstaticSub =  $this->t3substituteMarkerArray($pObj, $mypiclikeSub, array(
					'###VALUE###' => $i,
					'###REF###' => $refforvote,
					'###PID###' => $pid,
					'###CID###' => $cid,
		...
		if (($conf['ratings.']['useLikeDislikeStyle']==1) || ((intval($conf['ratings.']['useShortTopLikes']) == 1) && ($cmd=='votearticle')))  {
			...
			$mypicdislikehtmlstaticSub =  $this->t3substituteMarkerArray($pObj, $mypicdislikeSub, array(
					'###VALUE###' => $i,
					'###REF###' => $refforvote,
					'###PID###' => $pid,
					'###CID###' => $cid,
		...
		if ($scopeid==0) {
			...
			if ($prefix != 'tx_toctoc_comments_comments_') {
				...
				if (trim($mylikehtmlout.$mydislikehtmlout)!='') {
					if (intval($conf['advanced.']['useSharing']) == 1) {
						$brforfirstscope='<br><br class="tx-tc-secondbr-articlerating">';
					}
				}
		...
		if ((intval($conf['ratings.']['useTopLikeDislike']) == 0 ) && ($cmd == 'votearticle')) {
			...
			/* if ($conf['ratings.']['useLikeDislikeStyle']==1) {
				$mylikestaticareahtml= $this->t3substituteMarkerArray($pObj, $subTemplateMyILikeArea, $areamarkersstatic);
			}	 */
		}
		...
		if ($returnasarray) {
			...
			if ($conf['ratings.']['useLikeDislikeStyle']==1) {
				if (substr($ref,0,6) == 'tx_toc') {
					$contentarr['voteing']='<div class="tx-tc-overrating">' . str_replace('<div id="tx-tc-rts-dp-tx_toctoc_comments_comments', '</div></div><div class="tx-tc-overrating-date"><div id="tx-tc-rts-dp-tx_toctoc_comments_comments', $contentarr['voteing'] );
				}
			}
			...
			if ($lastprocessedemail!=$rowsnd['email']) {
				...
				*/
				...
				if (t3lib_div::validEmail($conf['advanced.']['notificationForCommentatorEmail'])) {
					...
					$fromeIDreturn .=  '<br />'. $this->pi_getLLWrap($pObj, 'error.invalid.email', $fromAjax) . ' (TS-Setup) advanced.notificationForCommentatorEmail: '. $conf['advanced.']['notificationForCommentatorEmail'];
				}
	...
	protected function topratings ($conf, $pObj) {
		...
		if ($conf['topRatings.']['topRatingsMode']==0){
			...
			$querymerged='SELECT DISTINCT MIN(pagetstampseen) as pageid,
					reference As ref,
					(CASE WHEN seen = 0 THEN 0 ELSE 1 END) *(CASE WHEN deleted = 0 THEN 1 ELSE 0 END) *(CASE WHEN reference_scope = 0 THEN 1 ELSE 1 END) * (CASE WHEN tstampseen > '.$datesince.' THEN 1 ELSE' . $addonsqlforoldviews .'END) as okdate,
					...
					FROM tx_toctoc_comments_feuser_mm
					GROUP BY reference,  (CASE WHEN seen = 0 THEN 0 ELSE 1 END) *(CASE WHEN deleted = 0 THEN 1 ELSE 0 END) * (CASE WHEN reference_scope = 0 THEN 1 ELSE 1 END) * (CASE WHEN tstampseen > '.$datesince.' THEN 1 ELSE' . $addonsqlforoldviews .'END)
					HAVING ' . $restrictor . 'okdate>0  AND nbrvotes>= ' . $numberofvotesrequired
					. ' ORDER BY okdate DESC, sumilikedislikevote DESC, nbrvotes DESC, ref';
			$resultmerged1= $GLOBALS['TYPO3_DB']->sql_query($querymerged);
			while ($rowsmerged1 = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($resultmerged1)) {
			...
			$querymerged='SELECT DISTINCT
					...
					reference As ref,
					(CASE WHEN (seen = 0 AND ilike=0 AND myrating=0) THEN 0 ELSE 1 END) *(CASE WHEN deleted = 0 THEN 1 ELSE 0 END) * (CASE WHEN tstampseen > '.$datesince.' THEN 1 ELSE ' . $addonsqlforoldactivities .' END) as okdate,
					sum(seen)+' . intval($conf['advanced.']['activityMultiplicatorRating']) .'*(sum(ilike)+sum(idislike)+sum(CASE WHEN myrating>0 THEN 1 ELSE 0 END)) as sumilikedislike,
					count(uid) as nbrvotes,
					...
					sum(seen) as sumseen,
					sum(seen)+' . intval($conf['advanced.']['activityMultiplicatorRating']) .'*(sum(ilike)+sum(idislike)+sum(CASE WHEN myrating>0 THEN 1 ELSE 0 END)) as sumilikedislikevote
					FROM tx_toctoc_comments_feuser_mm
					GROUP BY reference,
							(CASE WHEN (seen = 0 AND ilike=0 AND myrating=0) THEN 0 ELSE 1 END) *(CASE WHEN deleted = 0 THEN 1 ELSE 0 END) *  (CASE WHEN tstampseen > '.$datesince.' THEN 1 ELSE ' . $addonsqlforoldactivities .' END)
				...
				if (!is_array($_SESSION['prefixes'])) {
					...
					if (($displayfieldsareset==FALSE) || ($topratingsimagesfolderset==FALSE)) {
						...
						if ($displayfieldsareset==FALSE) {
							...
							$GLOBALS['TYPO3_DB']->sql_query("UPDATE tx_toctoc_comments_prefixtotable SET displayfields='full_name start_date photo_main sys_language_uid, biography' WHERE pi1_key='tx_rouge'");
							$GLOBALS['TYPO3_DB']->sql_query("UPDATE tx_toctoc_comments_prefixtotable SET displayfields='full_name start_date photo_main sys_language_uid, biography' WHERE pi1_key='tx_wecstaffdirectory_pi1'");
		...
		if ($conf['topRatings.']['topRatingsMode']<4){
			...
			$text_topratings = str_replace('%s', $conf['topRatings.']['RatedItemsListCount'], $this->pi_getLLWrap($pObj, 'pi1_template.text_topviews', FALSE));
				...
				//5
				$text_topratings .= ' ' . str_replace('%s', $conf['topRatings.']['RatedItemsListCount'], $this->pi_getLLWrap($pObj, 'pi1_template.text_topactivities', FALSE));
						...
						if (strpos($row['link'], 'href=')>0) {
							...
							if ($conf['ratings.']['useNumberOfVotes'] != 1) {
								...
								if ($conf['topRatings.']['topRatingsMode']<4){
									...
									if ($conf['topRatings.']['topRatingsMode']==4){
										$row['nbrvotes']=', ' . $row['nbrvotes'] . ' ' . $this->pi_getLLWrap($pObj, 'pi1_template.text_count', FALSE) . ' ' . $this->pi_getLLWrap($pObj, 'pi1_template.text_views_viewcount', FALSE)  . '';
									} else {
										$row['nbrvotes']='' ;
									}
								}
