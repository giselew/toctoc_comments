 ...
 *
 *
 *  107: class toctoc_comment_lib  extends tslib_pibase
 *  128:     public function maincomments($ref, $conf = null, $fromAjax = false, $pid=0, $feuserid = 0, $cmd = '', &$pObj, $piVars = null,
		$cid = 0, $sessioncapdata = '', $userpic='', $commentspics = array(), $check='', $AjaxData = '', $ajaxextref= '')
 *  238:     protected function checkExternalUid($conf,$pObj)
 *
 *              SECTION: Comments Functions
 *  266:     public function comments($conf,&$pObj,$fromAjax,$feuserid,$pid)
 *  687:     public function comments_getComments(&$rows,$conf,$pObj,$feuserid,$fromAjax,$pid)
 *  966:     protected function comments_getComments_fe_user($params, $conf, $pObj,$commentid,$fromAjax)
 * 1121:     protected function setAJAXimage($image,$feuserid)
 * 1132:     protected function getAJAXimage($feuserid,$commentid)
 * 1163:     protected function setAJAXimageCache($image,$imageoriginal,$feuserid)
 * 1174:     protected function getAJAXimageCache($commentuserimageout)
 * 1192:     protected function comments_getComments_getRatings(&$row,$conf,$pObj,$feuserid, $fromAjax)
 * 1211:     protected function comments_getComments_getEmail($email)
 * 1229:     protected function comments_getCommentsBrowser($rpp,$startpoint,$totalrows,$pObj,$fromAjax)
 * 1310:     public function form($conf, &$pObj, $piVars,$fromAjax,$pid, $ifeuserid=0, $userpic)
 * 1627:     protected function form_updatePostVarsWithFeUserData($conf, &$pObj, $piVars, $feuserid, $fromAjax, $userpic, $cid)
 * 1909:     protected function form_getCaptcha($pObj, $conf,$fromAjax)
 * 1955:     protected function form_wrapError($field, $pObj, $conf)
 * 1983:     protected function processBrowserSubmission($conf, $pObj, $piVars, $fromAjax, $cid, $cmd,$pid)
 * 2047:     public function processSubmission($conf,$pObj,$piVars,$fromAjax,$feuserid,$pid,$lang)
 * 2394:     protected function processSubmission_checkTypicalSpam($pObj, $conf, $piVars, $lang, $fromAjax)
 * 2458:     protected function processSubmission_validate($piVars, $conf, $pObj,$fromAjax)
 * 2521:     public function sendNotificationEmail($uid, $points, $action, $conf, $pObj, $fromAjax, $piVars, $pid)
 * 2657:     protected function isCommentingClosed($conf,$pObj)
 * 2703:     protected function commentingClosed($pObj,$fromAjax)
 * 2721:     public function fixLL(&$conf)
 * 2742:     protected function fixLL_internal($LL, &$ll, $prefix = '')
 * 2759:     protected function createLinks($text, $conf)
 * 2781:     protected function applyStdWrap($text, $stdWrapName, $conf = null, $pObj = null)
 * 2800:     protected function checkCustomFunctionCodes($code, $pObj)
 * 2822:     protected function isNoCacheUrl($url)
 * 2843:     protected function substituteMarkersAndSubparts($template, array $markers, array $subparts, $pObj)
 * 2860:     public function resetSessionVars($resetcontext, $alsoajaxvar = true)
 * 2901:     public function getClearCacheIds($conf, $pid = 0)
 * 2928:     public function parseSmilieArray($data)
 * 2945:     public function replaceSmilies($content,$conf)
 * 2969:     protected function getAjaxJSData($feuserid, $pid, $languagecode, $conf, $pObj, $cid, $fromAjax, $dataonly=false)
 * 3006:     protected function getAjaxData($feuserid, $pid, $languagecode, $conf, $cid)
 * 3031:     protected function getAjaxJSDataCommentImgs($cid, $pObj,$fromAjax)
 * 3067:     protected function getAjaxJSDataComments($cid,$pObj,$fromAjax)
 * 3105:     protected function getAjaxDataComments($pObj, $cid)
 * 3130:     protected function getAjaxDataImgs()
 * 3143:     public function getCurrentIp()
 * 3157:     public function hasValidItemUrl($piVars)
 * 3179:     public function getDefaultConfig()
 * 3197:     public function getRatingDisplay($ref, $conf = null, $fromAjax = 0, $pid=0, $returnasarray = false, $feuserid = 0, $cmd = 'vote',
			$pObj = null, $cid, $fromcomments)
 * 3242:     public function isVoted($ref,$conf,$pObj)
 * 3260:     protected function getBarWidth($rating,$conf)
 * 3273:     protected function getRatingInfo($ref,$pObj, $feuserid=-1,$conf)
 * 3346:     protected function generateRatingContent($ref, $template, $conf, $fromAjax, $pid, $returnasarray, $feuserid, $pObj,$cmd, $cid,$fromcomments)
 * 3710:     public function enableFields($tableName,$pObj)
 * 3730:     public function pi_getLLWrap($pObj, $llkey, $fromAjax)
 * 3748:     protected function getcheck ($ref, $i, $ajaxData, $ratingscheck)
 * 3771:     protected function formatDate($date, $pObj, $fromAjax, $conf)
 * 3891:     protected function getPageURL($fromAjax = false,$pid = 0)
 * 3917:     protected function getCleanHTML ($html)
 * 3938:     public function getUserCard($basedimgstr,$basedtoctocuid,$conf,$pObj,$commentid)
 * 4218:     protected function getMailTo($mailAddress, $linktxt = '', $initP = '?')
 * 4261:     protected function encryptEmail($string,$back=0)
 * 4297:     protected function encryptCharcode($n,$start,$end,$offset)
 * 4317:     public function handleCommentatorNotifications($uid, $conf,$pObj, $fromeID = false, $pid=0)
 * 4375:     protected function getCommentsReponse(&$params, &$pObj, $conf)
 * 4410:     protected function getCommentsReportLink($params, &$pObj, $conf ,$fromAjax,$pid)
 *
 * TOTAL FUNCTIONS: 61
...
// for design 2, buttons
	...
	public function comments_getComments(&$rows,$conf,$pObj,$feuserid,$fromAjax,$pid) {
		...
		foreach ($rows as $row) {
			...
			$strdivactionbuttons = '<div  class="tx-toctoc-comments-comment-actionforms">';
			if (($row['toctoc_commentsfeuser_feuser'] == intval($feuserid)) && (intval($feuserid) !==0)){
				...
				$deletelinkout .= '<input type="hidden" value="1" name="typo3_user_int"></form></div>';
			...
			if ((($row['toctoc_commentsfeuser_feuser'] == intval($feuserid)) && (intval($feuserid) !==0)) || (($row['remote_addr'] ==  $strCurrentIP) && ($row['toctoc_commentsfeuser_feuser'] == intval($feuserid)))){
				...
				if ($row['tx_commentsnotify_notify']==1) {
					if (($conf['advanced.']['commentatorNotifybyIP'] == 1) ||((intval($feuserid)!==0) && ($conf['advanced.']['commentatorNotifybyIP'] == 0))) {
						...
						$denotifylinkout.='<form  class="tx-toctoc-comments-comment-denotifyform" id="dnf' . $iuid . '" action="';
						$denotifylinkout.= '" method="post">';
						$denotifylinkout .= '	<input type="button" class="tx-toctoc-comments-comment-denotifybutton" style="background: url(';
						$denotifylinkout .= "'../../../";
			...
			if ($denotifylinkout ==''){
				...
				if ($deletelinkout ==''){
					$denotifylinkout = $strdivactionbuttons . $denotifylinkout . '</div>';
				} else  {
			...
			if ((intval($conf['advanced.']['adminCommentResponse']) ===1) || ($markerArray['###COMMENT_RESPONSE###'] !='')) {
				if (t3lib_extMgm::isLoaded('comments_response')) {
					...
					$commentresponse_start='<p class="tx-toctoc-commentsresponse-text"><span class="tx-toctoc-commentsresponse-text-title">' . $this->pi_getLLWrap($pObj, 'pi1_template.admincommenttitle', $fromAjax) . '</span>';
	...
	public function form($conf, &$pObj, $piVars,$fromAjax,$pid, $ifeuserid=0, $userpic) {
		...
		if (!$fromAjax) {
			...
			if ($_SESSION['feuserid']) {
				...
				$feuserid=intval($_SESSION['feuserid']);
			} else {
				$feuserid=0;
			}
		...
		$strtexterrorlength = sprintf($this->pi_getLLWrap($pObj, 'pi1_template.texterrorlength',$fromAjax), $commentminlength);
		$subformhtml =  $pObj->cObj->substituteMarkerArray($subformTemplate, array(
				...
				'###TEXT_ERR_COMMENT_LENGTH###' => $strtexterrorlength,
				'###ERR_COMMENT_REQUIRED_LENGTH###' => intval($conf['minCommentLength']),
		...
		if ($callextfunc==1) {
			$subformTemplateNotifySub =  $pObj->cObj->substituteMarkerArray($subformTemplateNotify, array(
					'###DIVSHOW_NOTIFICATION###' => '<div class="tx-toctoc-comments-comment-form-field">',
		...
		if ($piVars['insert']=='insert') {
			...
			if (!($this->processSubmission_validate($piVars, $conf, $pObj,$fromAjax))) {
				...
				*/
				...
				$pObj->piVars=array();
				$pObj->piVars['notify']=$piVars['notify'];
				if (is_array($GLOBALS['TYPO3_CONF_VARS']['EXTCONF']['tocotc_comments']['processSubmission'])) {
					foreach ($GLOBALS['TYPO3_CONF_VARS']['EXTCONF']['tocotc_comments']['processSubmission'] as $userFunc) {
						$params = array(
								'record' => $record,
								'pObj' => $pObj,
						);
						if (($newRecord = t3lib_div::callUserFunction($userFunc, $params, $pObj))) {
							$record = $newRecord;
						}
					}
				}
		...
		if (t3lib_div::validEmail($toEmail) && t3lib_div::validEmail($fromEmail)) {
			...
			if ($action == 'info') {
				...
				$markers = array(
						...
						'###FIRSTNAME###' => $this->pi_getLLWrap($pObj, 'pi1_template.first_name', $fromAjax)  . ' ' . $piVars['firstname'],
						'###LASTNAME###' => $this->pi_getLLWrap($pObj, 'pi1_template.last_name', $fromAjax)  . ' ' . $piVars['lastname'],
						'###EMAIL###' => $this->pi_getLLWrap($pObj, 'pi1_template.email', $fromAjax)  . ' ' . $piVars['email'],
						'###LOCATION###' => $this->pi_getLLWrap($pObj, 'pi1_template.location', $fromAjax)  . ' ' . $piVars['location'],
						'###HOMEPAGE###' => $this->pi_getLLWrap($pObj, 'pi1_template.web_site', $fromAjax)  . ' ' . $piVars['homepage'],
						...
						'###DELETE_LINK###' => $this->pi_getLLWrap($pObj, 'email.textdeletecomment', $fromAjax)  . ' ' . t3lib_div::locationHeaderUrl('index.php?eID=toctoc_comments&uid=' . $uid . '&chk=' . $check . '&lng=' . $lang . '&clearCache=' . urlencode($clearCacheIds) . '&cmd=delete'),
						'###KILL_LINK###' => $this->pi_getLLWrap($pObj, 'email.textkillcomment', $fromAjax)  . ' ' . t3lib_div::locationHeaderUrl('index.php?eID=toctoc_comments&uid=' . $uid . '&chk=' . $check . '&lng=' . $lang . '&clearCache=' . urlencode($clearCacheIds) . '&cmd=kill'),
						'###SITE_REL_PATH###' => '/' . t3lib_extMgm::siteRelPath('toctoc_comments'),
				);
			} else {
				...
				$markers = array(
						...
						'###TEXTCONTENT###' => $this->pi_getLLWrap($pObj, 'pi1_template.content', $fromAjax) ,
						'###REMOTE_ADDR###' => $this->pi_getLLWrap($pObj, 'email.textpostedfromip', $fromAjax)  . ' ' . t3lib_div::getIndpEnv('REMOTE_ADDR'),
						'###APPROVE_LINK###' => $this->pi_getLLWrap($pObj, 'email.textapprovecomment', $fromAjax)  . ' ' . t3lib_div::locationHeaderUrl('index.php?eID=toctoc_comments&uid=' . $uid . '&chk=' . $check . '&lng=' . $lang . '&clearCache=' . urlencode($clearCacheIds) . '&cmd=approve&confenc=' . $confenc),
						'###DELETE_LINK###' => $this->pi_getLLWrap($pObj, 'email.textdeletecomment', $fromAjax)  . ' ' . t3lib_div::locationHeaderUrl('index.php?eID=toctoc_comments&uid=' . $uid . '&chk=' . $check . '&lng=' . $lang . '&clearCache=' . urlencode($clearCacheIds) . '&cmd=delete'),
						'###KILL_LINK###' => $this->pi_getLLWrap($pObj, 'email.textkillcomment', $fromAjax)  . ' ' . t3lib_div::locationHeaderUrl('index.php?eID=toctoc_comments&uid=' . $uid . '&chk=' . $check . '&lng=' . $lang . '&clearCache=' . urlencode($clearCacheIds) . '&cmd=kill'),
						'###SITE_REL_PATH###' => '/' . t3lib_extMgm::siteRelPath('toctoc_comments'),
				);
			...
			if (t3lib_extMgm::isLoaded('comments_ipbl')) {
				...
				$markers['###DELETE_LINK_AND_BLOCK###'] = $this->pi_getLLWrap($pObj, 'email.textdeleteblock', $fromAjax) . $markers['###DELETE_LINK_AND_BLOCK###'];
				$markers['###KILL_LINK_AND_BLOCK###'] = $this->pi_getLLWrap($pObj, 'email.textkillblock', $fromAjax) . $markers['###KILL_LINK_AND_BLOCK###'];
			}	else {
	...
	*/
	...
	public function handleCommentatorNotifications($uid, $conf,$pObj, $fromeID = false, $pid=0) {
		...
		while ($rowsnd = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($ressnd)) {
			$markerArray = array(
					'###RECEIVER_LASTNAME###' => $rowsnd['lastname'],
					'###RECEIVER_FIRSTNAME###' => $rowsnd['firstname'],
			...
			if($fromeID) {
				print  ('<br>'. $this->pi_getLLWrap($pObj, 'commentatorEmail.notificationsentto', $fromAjax) . ' : '. $rowsnd['email']);
			}
	...
	protected function getCommentsReportLink($params, &$pObj, $conf ,$fromAjax,$pid) {
		...
		$theURLr = $_SESSION['commentsPageIds'][$conf['tx_commentsreport_pi1.']['reportPid']];
