 ...
 *
 *
 *  184: class toctoc_comment_lib  extends tslib_pibase
 *  214:     public function maincomments($ref, $conf = null, $fromAjax = false, $pid=0, $feuserid = 0, $cmd = '', &$pObj, $piVars = array(),
		$cid = 0, $sessioncapdata = '', $userpic='', $commentspics = array(), $check='', $AjaxData = '', $ajaxextref= '', $tctreestate = null)
 *  332:     protected function checkExternalUid($conf,$pObj)
 *
 *              SECTION: Commenting functions
 *  360:     public function comments($conf,&$pObj,$fromAjax,$feuserid,$pid)
 * 1012:     public function comments_getComments(&$rows,$conf,$pObj,$feuserid,$fromAjax,$pid)
 * 1476:     protected function comments_getComments_getRatings(&$row,$conf,$pObj,$feuserid, $fromAjax)
 * 1495:     protected function comments_getComments_getEmail($email)
 * 1509:     protected function isCommentingClosed($conf,$pObj)
 * 1555:     protected function commentingClosed($pObj,$fromAjax)
 * 1574:     protected function comments_getCommentsBrowser($rpp,$startpoint,$totalrows,$pObj,$fromAjax)
 * 1650:     protected function comments_getComments_fe_user($params, $conf, $pObj,$commentid,$fromAjax)
 *
 *              SECTION: functions for images cache (AJAX)
 * 1859:     protected function setAJAXimage($image,$feuserid)
 * 1870:     protected function getAJAXimage($feuserid,$commentid)
 * 1901:     protected function setAJAXimageCache($image,$imageoriginal,$feuserid)
 * 1912:     protected function getAJAXimageCache($commentuserimageout)
 *
 *              SECTION: Form functions
 * 1937:     public function form($conf, &$pObj, $piVars,$fromAjax,$pid, $ifeuserid=0, $userpic)
 * 2340:     protected function form_updatePostVarsWithFeUserData($conf, &$pObj, $piVars, $feuserid, $fromAjax, $userpic, $cid)
 * 2643:     protected function form_getCaptcha($pObj, $conf,$fromAjax)
 * 2689:     protected function form_wrapError($field, $pObj, $conf)
 *
 *              SECTION: Comments page-browser functions
 * 2721:     protected function processBrowserSubmission($conf, $pObj, $piVars, $fromAjax, $cid, $cmd,$pid)
 *
 *              SECTION: Inserting and Updating Comments functions
 * 2788:     public function processSubmission($conf,$pObj,$piVars,$fromAjax,$feuserid,$pid,$lang)
 * 3161:     public function updateComment($conf,$pObj,$ctid,$content,$pid)
 * 3265:     protected function processSubmission_checkTypicalSpam($pObj, $conf, $piVars, $lang, $fromAjax)
 * 3338:     protected function processSubmission_validate($piVars, $conf, $pObj,$fromAjax)
 * 3404:     public function sendNotificationEmail($uid, $points, $action, $conf, $pObj, $fromAjax, $piVars, $pid,$fetoctocusertoinsert)
 *
 *              SECTION: language handling functions
 * 3651:     public function fixLL(&$conf)
 * 3672:     protected function fixLL_internal($LL, &$ll, $prefix = '')
 *
 *              SECTION: mostly formatting functions
 * 3695:     protected function createLinks($text, $conf)
 * 3717:     protected function applyStdWrap($text, $stdWrapName, $conf = null, $pObj = null)
 * 3736:     protected function checkCustomFunctionCodes($code, $pObj)
 * 3758:     protected function isNoCacheUrl($url)
 * 3779:     protected function substituteMarkersAndSubparts($template, array $markers, array $subparts, $pObj)
 *
 *              SECTION: session and cache functions
 * 3802:     public function resetSessionVars($resetcontext, $alsoajaxvar = true)
 * 3844:     public function getClearCacheIds($conf, $pid = 0)
 *
 *              SECTION: Smilie functions
 * 3875:     public function parseSmilieArray($data)
 * 3892:     public function replaceSmilies($content,$conf)
 *
 *              SECTION: functions for the AJAx interface with jQuery/JavaScript
 * 3922:     protected function getAjaxJSData($feuserid, $pid, $languagecode, $conf, $pObj, $cid, $fromAjax, $dataonly=false)
 * 3962:     protected function getAjaxData($feuserid, $pid, $languagecode, $conf, $cid)
 * 3997:     protected function getAjaxDataAttachments($conf,$fromAjax,$pObj)
 * 4019:     protected function getAjaxJSDataCommentImgs($cid, $pObj,$fromAjax)
 * 4055:     protected function getAjaxJSDataComments($cid,$pObj,$fromAjax)
 * 4093:     protected function getAjaxDataComments($pObj, $cid)
 * 4111:     protected function getAjaxDataImgs()
 *
 *              SECTION: URL- and IP-related functions
 * 4130:     public function getCurrentIp()
 * 4144:     public function hasValidItemUrl($piVars)
 * 4166:     public function getDefaultConfig()
 *
 *              SECTION: Rating functions
 * 4190:     public function getRatingDisplay($ref, $conf = null, $fromAjax = 0, $pid=0, $returnasarray = false, $feuserid = 0, $cmd = 'vote',
			$pObj = null, $cid, $fromcomments)
 * 4235:     public function isVoted($ref,$conf,$pObj)
 * 4253:     protected function getBarWidth($rating,$conf)
 * 4266:     protected function getRatingInfo($ref,$pObj, $feuserid=-1,$conf)
 * 4337:     protected function generateRatingContent($ref, $template, $conf, $fromAjax, $pid, $returnasarray, $feuserid, $pObj,$cmd, $cid,$fromcomments)
 *
 *              SECTION: TYPO3 workarounds
 * 4746:     public function enableFields($tableName,$pObj)
 * 4766:     public function pi_getLLWrap($pObj, $llkey, $fromAjax)
 * 4784:     protected function getcheck ($ref, $i, $ajaxData, $ratingscheck)
 * 4805:     protected function formatDate($date, $pObj, $fromAjax, $conf)
 * 4925:     protected function getPageURL($fromAjax = false,$pid = 0)
 * 4951:     protected function getCleanHTML ($html)
 *
 *              SECTION: Usercards
 * 4982:     public function getUserCard($basedimgstr,$basedtoctocuid,$conf,$pObj,$commentid)
 *
 *              SECTION: Mail functions
 * 5296:     protected function getMailTo($mailAddress, $linktxt = '', $initP = '?')
 * 5339:     protected function encryptEmail($string,$back=0)
 * 5375:     protected function encryptCharcode($n,$start,$end,$offset)
 * 5397:     function send_mail (
			$toEMail,
			$subject,
			$message,
			$html,
 ...
 * 5533:     protected function slashName ($name, $apostrophe='"')
 *
 *              SECTION: Commentator-Notification functions
 * 5556:     public function handleCommentatorNotifications($uid, $conf,$pObj, $fromeID = false, $pid=0)
 *
 *              SECTION: Administrator comment response functions
 * 5663:     protected function getCommentsReponse(&$params, &$pObj, $conf)
 *
 *              SECTION: Website preview functions
 * 5707:     public function getwebpagepreview($cmd, $cid, $data, $pObj, $fromAjax, $websitepreviewid=0, $conf)
 * 5753:     protected function getpreviewinit($cid,$data, $pObj)
 * 5786:     protected function savewebpagepreviewtodb($pcid,$pcommentid,$userurl,$conf)
 * 5971:     protected function cleanupdbandfiles($conf)
 * 6050:     protected function getwebpagecache($pcid,$pcommentid, $conf, $url='', $isbeforefetch = false)
 * 6161:     protected function read_dir($dir, $array = array())
 * 6186:     protected function commentShowWebpagepreview ($rowattachmentid,$rowattachment_subid,$conf,$pObj,$cid,$topwebsitepreview)
 *
 *              SECTION: eID-Interface functions
 * 6287:     public function handleeID($uid, $conf,$pObj,$messagetodisplay,$refreshurl)
 *
 *              SECTION: Reply on comments functions
 * 6350:     protected function getCommentBoxDisplay($commentid,$conf,$level,$fromAjax)
 * 6403:     protected function getCommentBoxChildrenDisplayIsCollapsed($commentschildrenids,$conf,$level,$fromAjax)
 *
 *              SECTION: Fe-usergroup functions
 * 6457:     protected function usersGroupmembers($pObj, $fromAjax)
 *
 *              SECTION: Recent comments functions
 * 6503:     public function getRecentComments($pObj, $conf,$feuserid)
 * 6567:     protected function comments_getRecentComments($rows,$conf,$pObj)
 * 6705:     protected function trimContent($text,$conf)
 * 6732:     protected function createRCLinks($text, $refID, $commentID, $prefix, $table, $externalprefix, $singlePid, $conf)
 *
 *              SECTION: Report comments functions
 * 6772:     protected function getCommentsReportLink($params, &$pObj, $conf ,$fromAjax,$pid)
 * 6806:     public function mainReport($content, $conf, $pObj, $piVars)
 * 6830:     protected function processReportForm(array &$errors,$conf, $pObj, $piVars)
 * 6958:     protected function showReportThanks($conf,$pObj)
 * 6971:     protected function showReportForm(array $errors,$conf,$pObj,$piVars)
 * 7054:     protected function getReportCaptcha($required, $error,$conf,$pObj)
 *
 *              SECTION: IP-blocking functions
 * 7099:     protected function IPBlockSpamCheck(&$params, &$pObj)
 * 7118:     protected function sendNotificationIPBlock($params, &$pObj)
 * 7132:     private function getIpAddr()
 * 7147:     private function checkTableBLs($ipaddr)
 * 7158:     private function checkLocalBL($ipaddr)
 * 7171:     private function checkStaticBL($ipaddr)
 * 7198:     function checkNetworkBLs($ipaddr)
 *
 * TOTAL FUNCTIONS: 92
...
class toctoc_comment_lib  extends tslib_pibase {
	...
	public function comments($conf,&$pObj,$fromAjax,$feuserid,$pid) {
		...
		if ($conf['ratings.']['ratingsOnly'] ==0) {
			...
			$condfeusergroup='';
			if (($conf['advanced.']['showFeUsercommentsOnlyInSameUserGroup']==1) && (intval($feuserid) !=0) &&($conf['advanced.']['showFeUsercomments']==1)) {
				$condfeusergroup = ' AND toctoc_commentsfeuser_feuser IN ('. $this->usersGroupmembers($pObj, $fromAjax) . ')';
			} else {
			...
			$rowsin = $GLOBALS['TYPO3_DB']->exec_SELECTgetRows('uid',
					'tx_toctoc_comments_comments', $pObj->where . $condfeusergroup, '', $sorting, $limitofrows . ' OFFSET ' . $start);
			...
			if ($uidsubselall==''){$uidsubselall='0';}
            $rowsin = $GLOBALS['TYPO3_DB']->exec_SELECTgetRows('toctoc_commentsfeuser_feuser, toctoc_comments_user, uid,approved,crdate,firstname,lastname,homepage,
					location,email,content,tx_commentsnotify_notify,remote_addr,SUBSTR(external_ref_uid,12) AS external_ref_uid,attachment_id,attachment_subid,parentuid,gender',
            		'tx_toctoc_comments_comments', 'uid IN (' . $uidsubselall . ')' . $condfeusergroup, '', $sorting);
...
// for design 2, buttons
		...
		if (intval($conf['attachments.']['useWebpagePreview']) ===1 ) {
			if (intval($conf['attachments.']['useTopWebpagePreview']) > 0) {
				$attachmentHTML= $this->commentShowWebpagepreview (intval($conf['attachments.']['useTopWebpagePreview']),intval($conf['attachments.']['topWebpagePreviewPicture']),$conf,$pObj,$_SESSION['commentListCount'],true);
			}
		}
		$replyoncommentHTML='';
		if (intval($conf['advanced.']['userCommentResponseLevels']) >0) {
			...
			$replyoncommentHTML.= '<img id="tx-tc-cts-nococ-' . $_SESSION['commentListCount'] . '" src="' . '/' . t3lib_extMgm::siteRelPath('toctoc_comments') . 'res/img/nopreviewpic.png" height="11" width="12" title="'.$this->pi_getLLWrap($pObj, 'pi1_template.closereply', $fromAjax).'" class="tx-tc-coc" style="display:none;" >';
		...
		foreach ($rows as $row) {
			...
			if (($row['toctoc_commentsfeuser_feuser'] == intval($feuserid)) && (intval($feuserid) !==0)){
				...
				if (array_key_exists($row['uid'], $userarr)) {
					...
					if ($conf['EditCommentImage']) {
						$fileinfo = t3lib_div::split_fileref($conf['EditCommentImage']);
						if (t3lib_div::inList('jpg,gif,jpeg,png',$fileinfo['fileext'])) {
							$imgFile = $GLOBALS['TSFE']->absRefPrefix.$conf['EditCommentImage'];
							$imgFile = $GLOBALS['TSFE']->getFileAbsFileName.$imgFile;
							$imgFile = str_replace('/EXT:toctoc_comments', 'typo3conf/ext/toctoc_comments',$imgFile);
							$imgFile = str_replace('EXT:toctoc_comments', 'typo3conf/ext/toctoc_comments',$imgFile);
						}
			...
			if ((($row['toctoc_commentsfeuser_feuser'] == intval($feuserid)) && (intval($feuserid) !==0)) || (($row['remote_addr'] ==  $strCurrentIP) && ($row['toctoc_commentsfeuser_feuser'] == intval($feuserid)))){
				...
				if ($row['tx_commentsnotify_notify']==1) {
					if (($conf['advanced.']['commentatorNotifybyIP'] == 1) ||((intval($feuserid)!==0) && ($conf['advanced.']['commentatorNotifybyIP'] == 0))) {
						...
						$denotifylinkout .= "'../../../";
						if ($conf['DenotifyCommentImage'] !== '') {
							$fileinfo = t3lib_div::split_fileref($conf['DenotifyCommentImage']);
							//$imgFile = 'giga.png';
							if (t3lib_div::inList('jpg,gif,jpeg,png',$fileinfo['fileext'])) {
								$imgFile = $GLOBALS['TSFE']->absRefPrefix.$conf['DenotifyCommentImage'];
								$imgFile = $GLOBALS['TSFE']->getFileAbsFileName.$imgFile;
								$imgFile = str_replace('/EXT:toctoc_comments', 'typo3conf/ext/toctoc_comments',$imgFile);
								$imgFile = str_replace('EXT:toctoc_comments', 'typo3conf/ext/toctoc_comments',$imgFile);
							}
						}
						$denotifylinkout .= $imgFile;
						$denotifylinkout .= "'";
			...
			if (strlen($row['content'])>$conf['commentCropLength']){
				...
				if (count($textcroppedrightarr)>1) {
					...
					$textcroppedleft .=$textcroppedrightarr[0] . ' <span style="display: inline;" id="tx-tc-acropped-' . $row['uid'] . '"><br /><a href="javascript:tcsroc(' . $row['uid'] . ');" title="' . $showrestofcomment . '">' . $showrestofcommenttext . '</a></span><span id="tx-tc-cropped-' . $row['uid'] . '" style="display: none;">' . substr($textcroppedright,strlen($textcroppedrightarr[0])) . '</span>';
			...
			if (intval($conf['attachments.']['useWebpagePreview']) ===1 ) {
				if ($row['attachment_id']> 0) {
					$attachmentHTML= $this->commentShowWebpagepreview ($row['attachment_id'],$row['attachment_subid'],$conf,$pObj,$iuid, false);
				}
			}
			...
			if (($conf['advanced.']['userCommentResponseLevels'] != 0) && ($this->isCommentingClosed($conf,$pObj) == 0) && ($row['level']<$conf['advanced.']['userCommentResponseLevels'])) {
				...
				$subcommentlink='';
				$subcommentlink.='<span id="tx-tc-ct-div-ry-' . $row['uid'] . '"><a href="javascript:commentreply(' . $row['uid'] . ', previewselcomment' . $_SESSION['commentListCount'] . ', ' . $_SESSION['commentListCount'] . ');" title="' .$subcomment . '">' . $subcomment .'</a>'.'</span>';
				...
				if ($row['children'] != '') {
					for ($i=0;$i<2;$i++) {
							...
							$tmpimgstr='';
							$userimgFile='/typo3conf/ext/toctoc_comments/res/img/'. $userconfimgFile . '.png';
							$expandcommentsHTML .= '<img src="' . $userimgFile . '" ' . 'style="' . $opa . $userimagestyle .
			...
			if (($row['level']>0) || ($displaycss !=''))  {
				...
				if ($row['level']>0)  {
					...
					$stylesubcommentHTML .= 'margin: 0 0 0 ' . $shiftleft . 'px; border-left:#dedede 1px solid;border-radius: 10px 4px 0 0 ;';
				}
				if ($displaycss !='')  {
...
// 					}
	...
	protected function comments_getCommentsBrowser($rpp,$startpoint,$totalrows,$pObj,$fromAjax) {
		$result='';
		$emptybrowsestr='<div class="tx-tc-cts-ctsbrowse"></div>';
		if ($startpoint > 0){
			$emptybrowsestr='';
			$submithtml='';
			...
			$result .= '<div class="tx-tc-cts-ctsbrowse"><form id="bf' . $_SESSION['commentListCount']  . '" class="comment-browseform" ';
		...
		if (($totalrows-$startpoint)>$rpp){
			...
			$result .= $emptybrowsestr . '<div class="tx-tc-cts-ctsbrowse-hide"><form id="bfh' . $_SESSION['commentListCount']  . '" ';
	...
	*/
	protected function comments_getComments_fe_user($params, $conf, $pObj,$commentid,$fromAjax) {
		...
		$usergenderexists=false;
		$usergenderexistsstr='';
		if (count($rowsfeuser)>0) {
			if (array_key_exists('gender',$rowsfeuser[0])) {
				if ($rowsfeuser[0]['gender']==1) {
					$usergenderexists=true;
			...
			if (count($this->AJAXimages)==0) {
				if (count($_SESSION['AJAXimages'])!=0) {
					...
					$start_time_for_conversion = microtime();
				 	...
				 	$rowsfeuserimages = $GLOBALS['TYPO3_DB']->exec_SELECTgetRows('fe_users.uid AS uid, '. $usergenderexistsstr .' fe_users.image AS image, fe_users.lastlogin AS lastlogin, fe_users.is_online AS is_online', 'fe_users,tx_toctoc_comments_comments',
					...
					$userimagestyle=$conf['advanced.']['FeUserImageStyle'] . ' width: ' . $userimagestylesize . 'px; height: ' . $userimagestylesize . 'px; margin-right:8px;';
					$commentuserimagepath = $conf['advanced.']['FeUserImagePath'];

					foreach($rowsfeuserimages as $keyimage) {

						$userconfimgFile='DefaultUserImage';
						if ($usergenderexists) {
							if ($keyimage['gender']==1) {
								$userconfimgFile='DefaultUserImageF';
							}
						}
						if ($conf[$userconfimgFile]) {
							$fileinfo = t3lib_div::split_fileref($conf[$userconfimgFile]);
							if (t3lib_div::inList('jpg,gif,jpeg,png',$fileinfo['fileext'])) {
								$userimgFile = $GLOBALS['TSFE']->absRefPrefix.$conf[$userconfimgFile];
						...
						if (strval($keyimage['image']!=='')) {
							$userimagesarr=explode(',',$keyimage['image']);
						...
						if ($tmpimgstr==='') {
							...
							$img['params'] = 'style="' . $userimagestyle . '" id="tx-tc-cts-img-"'  ;   //style="margin: -6px 6px -6px -3px; padding: 0pt 0pt 0pt 0px; border-radius: 2px 2px 2px 2px; border: 1px solid #97b0ee;" align="left"';
	...
	protected function getAJAXimage($feuserid,$commentid) {
		if ($this->AJAXimages[$feuserid]) {
			...
			$newstr='id="tx-tc-cts-img-' . $commentid . '"';
			$oldstr='id="tx-tc-cts-img-"' ;
	...
	public function form($conf, &$pObj, $piVars,$fromAjax,$pid, $ifeuserid=0, $userpic) {
		...
		//$userimagesize=18 it's fixed here for the little avatar-pics, no conf yet;
		$userimagesize=3*18;
		$userimagestylesize=18;
		...
		if (!$fromAjax) {
			...
			for ($i=0;$i<2;$i++) {
				if ($i==0) {
					$userconfimgFile='DefaultUserImage';
					$opa='opacity:1; filter: alpha(opacity=100);cursor:pointer; margin:0 1px 0 0;';
				...
				$img['file.']['10.']['file.']['width'] = $userimagesize .'c';
				$img['file.']['10.']['file.']['height'] = $userimagesize .'c';
				$img['params'] = 'style="' . $opa . $userimagestyle . '" title="' . $alttext . '" '. $onclick . ' id="tx-toctoc-comments-comments-img-gender-' . $i . '-placeholdercid"'  ;   //style="margin: -6px 6px -6px -3px; padding: 0pt 0pt 0pt 0px; border-radius: 2px 2px 2px 2px; border: 1px solid #97b0ee;" align="left"';
				$tmpimgstr = $pObj->cObj->IMAGE($img);
				...
				foreach($rows as $key) {
					...
					if ((!$piVars['imagetag'])  || $reloadpivars) {
						...
						if (($key['image'] !='') || ($key['image'])) {
							$userimagesarr=explode(',',$key['image']);
							if (count($userimagesarr)>0){
								$commentuserimageextr=  $userimagesarr[0];
							} else {
								$commentuserimageextr=  $key['image'];
							}
	...
	public function processSubmission($conf,$pObj,$piVars,$fromAjax,$feuserid,$pid,$lang) {
		...
		if ($piVars['insert']=='insert') {
				...
				if ($info['t'] > 0) {
					...
					if ($isSpam < $cutOffPoint) {
						...
						if (!$isApproved) {
							...
							$this->sendNotificationEmail($newUid, $isSpam, 'approve', $conf, $pObj, $fromAjax, $piVars, $pid,$fetoctocusertoinsert);
							// Clear cache
							$cacheidlist= strval($pid); // . ', 100';
							$pidList = t3lib_div::intExplode(',', $cacheidlist);
							...
							if ($conf['advanced.']['commentatorNotify'] ==1) {
								//$this->handleCommentatorNotifications(intval($newUid),$conf,$pObj,false,$pid);
							}
							...
							if (strlen($conf['spamProtect.']['informationEmail']) > 0){
								$this->sendNotificationEmail($newUid, $isSpam, 'info', $conf, $pObj, $fromAjax, $piVars, $pid,$fetoctocusertoinsert);
							}
			...
			if (($_SESSION['formTopMessage']) || ($_SESSION['formValidationErrors']['errorcode'])) {
				...
				$cacheidlist= strval($pid); // . ', 100';
				$pidList = t3lib_div::intExplode(',', $cacheidlist);
				//$pidList = $this->getClearCacheIds($conf,$pid);
				t3lib_div::requireOnce(PATH_t3lib . 'class.t3lib_tcemain.php');
				$tce = t3lib_div::makeInstance('t3lib_TCEmain');
	 ...
	 * @return	void
	 */
	public function sendNotificationEmail($uid, $points, $action, $conf, $pObj, $fromAjax, $piVars, $pid,$fetoctocusertoinsert) {
		if (($fromAjax)) {
		...
		if ($conf['HTMLEmail']) {
			$seperatorchar = '<br />';
			$approvelink = '<b><a href="'  . t3lib_div::locationHeaderUrl('index.php?eID=toctoc_comments&uid=' . $uid . '&chk=' . $check . '&lng=' . $lang . '&clearCache=' . urlencode($clearCacheIds) .
			...
			if (t3lib_extMgm::isLoaded('comments_response')) {
				...
				if (trim($markers['###RESPOND_LINK###']) !='') {
					if ($conf['HTMLEmail']) {
						...
						$markers['###RESPOND_LINK###'] = '<br /><a href="' . $markers['###RESPOND_LINK###'] . '">' . $strrespondlink . '</a>';
	...
	*/
	...
	protected function getAjaxData($feuserid, $pid, $languagecode, $conf, $cid) {
		...
		// we take only useWebpagePreview and webpagePreviewHeight, they are needed on and for submits
	...
	protected function generateRatingContent($ref, $template, $conf, $fromAjax, $pid, $returnasarray, $feuserid, $pObj,$cmd, $cid,$fromcomments) {
		...
		if ($mydislikeval==0) {
			...
			if ((substr($ref, 0,9)!=='tx_toctoc') || ($cmd=='votearticle')) {
				$mydislikepicalkt=$this->pi_getLLWrap($pObj, 'api_top' . $extpreffortext . 'dislikemaybe',$fromAjax);
				$mydislikehtml=$this->pi_getLLWrap($pObj, 'api_top' . $extpreffortext . 'dislike',$fromAjax);
				//exit;
			...
			if ((substr($ref, 0,9)!=='tx_toctoc') || ($cmd=='votearticle')) {
				$mydislikepicalkt=$this->pi_getLLWrap($pObj, 'api_top' . $extpreffortext . 'idislikethis',$fromAjax);
				$mydislikehtml=$this->pi_getLLWrap($pObj, 'api_top' . $extpreffortext . 'idislike',$fromAjax);
		...
		if ($mylikeval==0) {
			...
			if ((substr($ref, 0,9)!=='tx_toctoc') || ($cmd=='votearticle')) {
				$mylikepicalkt=$this->pi_getLLWrap($pObj, 'api_top' . $extpreffortext . 'likemaybe',$fromAjax);
				$mylikehtml=$this->pi_getLLWrap($pObj, 'api_top' . $extpreffortext . 'like',$fromAjax);
			} else {
			...
			$mylikepic='ilike.png';
			if ((substr($ref, 0,9)!=='tx_toctoc') || ($cmd=='votearticle')) {
				$mylikepicalkt=$this->pi_getLLWrap($pObj, 'api_top' . $extpreffortext . 'ilikethis',$fromAjax);
				$mylikehtml=$this->pi_getLLWrap($pObj, 'api_top' . $extpreffortext . 'ilike',$fromAjax);
			} else {
	...
	public function getUserCard($basedimgstr,$basedtoctocuid,$conf,$pObj,$commentid) {
				...
				if ($uccontact != '') {
					...
					$uccontact ='<div class="tx-tc-ct-uc-text-contact"><img class="tx-tc-ucpic" src="' . $mailurl . t3lib_extMgm::siteRelPath('toctoc_comments') .
								'res/img/uccontact.png" height="14" width="14" title="' .
						         	$this->pi_getLLWrap($pObj, 'pi1_template.uc.contact',true) . '">' . $nbsp . $uccontact .
						         '</div>';
				}
		...
		$content =  '<div class="tx-tc-ct-uc-pic">' .
		$imgstr . '<br />' .
		'<span class="tx-tc-ct-uc-fullname">' . $ucfullname . '</span>' . $addnameinfo .
		'</div>' .
		'<div class="tx-tc-ct-uc-text">' ;
	 ...
	 */
	public function handleCommentatorNotifications($uid, $conf,$pObj, $fromeID = false, $pid=0) {
		$fromAjax=1;
		...
		if ($newUid==0){
			...
			for ($i=0;$i<count($_SESSION[$pcid][$pcommentid]['images']);$i++) {
				...
				if (count($_SESSION[$pcid][$pcommentid]['images']) > 0) {
					...
					$photos_etc .= str_replace('/','',str_replace('uploads\\tx_toctoccomments\\temp\\','',str_replace('uploads/tx_toctoccomments/temp/','',$linktofile))) . ',';
			...
			if ($_SESSION[$pcid][$pcommentid]['logo'] !='') {
				...
				$photo_main = str_replace('uploads\\tx_toctoccomments\\temp\\','',str_replace('uploads/tx_toctoccomments/temp/','',$linktofile));
	 ...
	 * @return	void
	 */
	protected function cleanupdbandfiles($conf) {
		...
		if (DIRECTORY_SEPARATOR == '\\') {
			// windows
			$txdirname= str_replace('/','\\',$_SERVER['DOCUMENT_ROOT'] . DIRECTORY_SEPARATOR . $this->webpagepreviewsavefolder);
		} else {
			$txdirname=$_SERVER['DOCUMENT_ROOT'] . DIRECTORY_SEPARATOR . $this->webpagepreviewsavefolder;
		}
	 ...
	 */
	protected function commentShowWebpagepreview ($rowattachmentid,$rowattachment_subid,$conf,$pObj,$cid,$topwebsitepreview) {
		if ($topwebsitepreview) {
			...
			$rowsatt = $GLOBALS['TYPO3_DB']->exec_SELECTgetRows(
					'tx_toctoc_comments_attachment.systemurltext as systemurltext,
			...
			$attwhere = 'tx_toctoc_comments_attachment.uid = tx_toctoc_comments_attachment_mm.attachmentid AND ' .
					' tx_toctoc_comments_attachment.attachmentvariant= 1 AND tx_toctoc_comments_attachment.deleted=0 ' .
		...
		if (count($rowsatt)>0) {
			if ($rowsatt[0]['photo_main']!='') {
				...
				$logostr .= '<img src="/' . $this->webpagepreviewsavefolder . $rowsatt[0]['photo_main'] . '" style="margin: 4px 0 0;max-height: ' . $conf['attachments.']['webpagePreviewHeight'] . 'px; max-width:' . (30+$conf['attachments.']['webpagePreviewHeight']) . 'px; float:right; ">';
				$logostr .= '</div>';
			}
			$picstr='';
			$textareamargin=2;
			if ($rowsatt[0]['photos_etc']!='') {
				...
				$picstr .= '<div>';
				$picstr .= '<img style="display: block;" src="/' . $this->webpagepreviewsavefolder . $rrpicpic . '" ';
 ...
 */
	 ...
	 */
	protected function usersGroupmembers($pObj, $fromAjax) {
		if (!$fromAjax) {
			global $TSFE;
	...
	protected function comments_getRecentComments($rows,$conf,$pObj) {
		...
		foreach ($rows as $row) {
			...
			if ($prefix == 'pages_') {
				...
				$recentcommentsPluginpages=explode(',', $conf['recentcommentsPluginpages']);
				...
				if (count($recentcommentsPluginpages) >0) {
					...
					if ($j != -1) {
						$recentcommentsPluginRecords=explode(',', $conf['recentcommentsPluginRecords']);
			...
			$commenttext = $this->trimContent($row['content'],$conf);
			//Parse for Links and Smilies
			$this->smiliesPath = str_replace('EXT:toctoc_comments/','/' . t3lib_extMgm::siteRelPath('toctoc_comments'),$conf['smiliePath']);
			$this->smilies = $this->parseSmilieArray($conf['smilies.']);
			//$contenttext = $this->applyStdWrap(nl2br($this->createLinks($content, $conf)), 'content_stdWrap', $conf, $pObj);
			...
			$markerArray = array(
					...
					'###TITLE###' => $this->applyStdWrap($titleimage . $this->createRCLinks(strip_tags($row['refTitle']), $refID, $commentID, $prefix, $mmtable, $externalprefix, $pageid, $conf), 'recentComment_stdWrap', $conf, $pObj),
					'###COMMENT_CONTENT###' => $this->applyStdWrap($commentimage. $this->createRCLinks($commenttext, $refID, $commentID, $prefix, $mmtable, $externalprefix, $pageid, $conf), 'content_stdWrap', $conf, $pObj),
			);
			$entries[] = $pObj->cObj->substituteMarkerArray($template, $markerArray);
		}
		return implode('', $entries);
	 ...
	 */
	protected function trimContent($text,$conf) {
	 ...
	 * @return	string		Link
	 */
	protected function createRCLinks($text, $refID, $commentID, $prefix, $table, $externalprefix, $singlePid, $conf) {
		if ($conf['recentcomments.']['linkComments'] == 1) {
			$params = array(
	                $externalprefix .'[' . $table . ']' => $refID,
        	...
        	$moreParams='toctoc_comments_pi1[anchor]='.substr($conf['recentcomments.']['anchorPre'],1).$commentID;
			$conf = array(
        		'useCacheHash'     => 1,
        		'no_cache'         => 0,
