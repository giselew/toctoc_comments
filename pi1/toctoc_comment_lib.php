 ...
 *
 *
 *  201: class toctoc_comment_lib  extends tslib_pibase
 *  262:     public function maincomments($ref, $conf = null, $fromAjax = false, $pid=0, $feuserid = 0, $cmd = '', &$pObj, $piVars = array(),
		$cid = 0, $sessioncapdata = '', $userpic='', $commentspics = array(), $check='', $AjaxData = '', $ajaxextref= '', $tctreestate = null,$commentreplyid=0)
 *  419:     protected function checkExternalUid($conf,$pObj)
 *
 *              SECTION: Commenting functions
 *  447:     public function comments($conf,&$pObj,$fromAjax,$feuserid,$pid)
 * 1446:     protected function makeemoji($text, $conf)
 * 1534:     protected function unicodeString($str, $encoding=null)
 * 1553:     public function comments_getComments(&$rows,$conf,$pObj,$feuserid,$fromAjax,$pid,$levelhlt=0)
 * 2236:     protected function comments_getComments_getRatings(&$row,$conf,$pObj,$feuserid, $fromAjax)
 * 2255:     protected function comments_getComments_getEmail($email)
 * 2269:     protected function isCommentingClosed($conf,$pObj)
 * 2316:     protected function commentingClosed($pObj,$fromAjax,$conf)
 * 2337:     protected function comments_getCommentsBrowser($rpp,$startpoint,$totalrows,$pObj,$fromAjax, $conf)
 * 2455:     protected function build_AJAXImages ($conf, $pObj, $usergenderexistsstr='', $fromAjax=false)
 * 2620:     public function comments_getComments_fe_user($params, $conf, $pObj,$commentid,$fromAjax, $commentusername)
 * 2717:     protected function fixFeUserPic ($conf, $pic)
 * 2736:     protected function addleadingspace($text)
 * 2757:     protected function check_comment_ownership ($rowexternal_ref,$feuserid)
 * 2779:     public function previewcomment($data, $pObj, $conf, $fromAjax=true)
 *
 *              SECTION: functions for images cache (AJAX)
 * 2809:     protected function setAJAXimage($image,$feuserid)
 * 2820:     protected function getAJAXimage($feuserid,$commentid)
 * 2851:     protected function setAJAXimageCache($image,$imageoriginal,$feuserid)
 * 2862:     protected function getAJAXimageCache($commentuserimageout)
 *
 *              SECTION: Form functions
 * 2891:     public function form($conf, &$pObj, $piVars,$fromAjax,$pid, $ifeuserid=0, $userpic, $commentid=0, $replylevel=0, $fromcomments = false, $cidfromajaxcomments = 0)
 * 3526:     protected function form_updatePostVarsWithFeUserData($conf, &$pObj, $piVars, $feuserid, $fromAjax, $userpic, $cid, $output_cid)
 * 3807:     protected function form_getCaptcha($pObj, $conf,$fromAjax)
 * 3855:     protected function form_wrapError($field, $pObj, $conf)
 *
 *              SECTION: Comments page-browser functions
 * 3888:     protected function processBrowserSubmission($conf, $pObj, $piVars, $fromAjax, $cid, $cmd,$pid, $ref)
 *
 *              SECTION: Inserting and Updating Comments functions
 * 3969:     public function processSubmission($conf,$pObj,$piVars,$fromAjax,$feuserid,$pid,$lang)
 * 4432:     public function updateComment($conf,$pObj,$ctid,$content,$pid)
 * 4532:     protected function processSubmission_checkTypicalSpam($pObj, $conf, $piVars, $lang, $fromAjax)
 * 4605:     protected function processSubmission_validate($piVars, $conf, $pObj,$fromAjax)
 * 4674:     public function sendNotificationEmail($uid, $points, $action, $conf, $pObj, $fromAjax, $piVars, $pid,$fetoctocusertoinsert,$attachment_id=0 ,$attachment_subid=0, $optinemail='')
 * 5029:     protected function emailOptIn($conf, $optinemail, $optin_ip)
 * 5098:     protected function checkCOI($conf,$email,$checkip=true)
 *
 *              SECTION: language handling functions
 * 5133:     public function fixLL(&$conf)
 * 5154:     protected function fixLL_internal($LL, &$ll, $prefix = '')
 *
 *              SECTION: mostly formatting functions
 * 5177:     protected function createLinks($text, $conf)
 * 5199:     protected function applyStdWrap($text, $stdWrapName, $conf = null, $pObj = null)
 * 5218:     protected function checkCustomFunctionCodes($code, $pObj)
 * 5240:     protected function isNoCacheUrl($url)
 * 5261:     protected function substituteMarkersAndSubparts($template, array $markers, array $subparts, $pObj)
 *
 *              SECTION: session and cache functions
 * 5284:     public function resetSessionVars($resetcontext, $alsoajaxvar = true)
 * 5331:     public function getClearCacheIds($conf, $pid = 0)
 * 5364:     public function initCaches()
 *
 *              SECTION: Smilie functions
 * 5390:     public function parseSmilieArray($data)
 * 5407:     public function replaceSmilies($content,$conf)
 * 5424:     protected function checkbbcrop($content,$commentCropLength)
 * 5450:     protected function replaceBBs($content,$conf,$purge=false)
 *
 *              SECTION: functions for the AJAx interface with jQuery/JavaScript
 * 5491:     protected function getAjaxJSData($feuserid, $pid, $languagecode, $conf, $pObj, $cid, $fromAjax, $dataonly=false)
 * 5531:     protected function getAjaxData($feuserid, $pid, $languagecode, $conf, $cid)
 * 5573:     protected function getAjaxDataAttachments($conf,$fromAjax,$pObj)
 * 5594:     protected function getAjaxJSDataCommentImgs($cid, $pObj,$fromAjax)
 * 5630:     protected function getAjaxJSDataComments($cid,$pObj,$fromAjax)
 * 5668:     protected function getAjaxDataComments($pObj, $cid)
 * 5686:     protected function getAjaxDataImgs()
 *
 *              SECTION: URL- and IP-related functions
 * 5702:     public function getCurrentIp()
 * 5716:     public function hasValidItemUrl($piVars)
 * 5737:     public function getDefaultConfig($pluginkey='')
 * 5750:     protected function ae_detect_ie()
 *
 *              SECTION: Rating functions
 * 5778:     public function getRatingDisplay($ref, $conf = null, $fromAjax = 0, $pid=0, $returnasarray = false, $feuserid = 0, $cmd = 'vote',
			$pObj = null, $cid, $fromcomments,$commentspics = array())
 * 5824:     public function isVoted($ref,$conf,$pObj)
 * 5842:     protected function getBarWidth($rating,$conf)
 * 5855:     protected function getRatingInfo($ref,$pObj, $feuserid=-1,$conf)
 * 5957:     protected function generateRatingContent($ref, $template, $conf, $fromAjax, $pid, $returnasarray, $feuserid, $pObj,$cmd, $cid,$fromcomments, $commentspics)
 * 6531:     function makeiLikeText($conf, $pObj, $cmd, $ref, $fromAjax, $myrating = array(),$mylikeval,$mydis='',$template,$cid, $commentspics,$extpreffortext)
 *
 *              SECTION: TYPO3 workarounds
 * 6875:     public function enableFields($tableName,$pObj)
 * 6895:     public function pi_getLLWrap($pObj, $llkey, $fromAjax)
 * 6913:     protected function getcheck ($ref, $i, $ajaxData, $ratingscheck)
 * 6934:     protected function formatDate($date, $pObj, $fromAjax, $conf)
 * 7054:     protected function getPageURL($fromAjax = false,$pid = 0)
 * 7077:     protected function getCleanHTML ($html)
 *
 *              SECTION: Usercards
 * 7110:     public function getUserCard($basedimgstr,$basedtoctocuid,$conf,$pObj,$commentid)
 *
 *              SECTION: Mail functions
 * 7406:     protected function getMailTo($mailAddress, $linktxt = '', $initP = '?')
 * 7449:     protected function encryptEmail($string,$back=0)
 * 7485:     protected function encryptCharcode($n,$start,$end,$offset)
 * 7507:     function send_mail ($toEMail,$subject,$message,$html,$fromEMail,$fromName,$attachment='')
 * 7632:     protected function slashName ($name, $apostrophe='"')
 *
 *              SECTION: Commentator-Notification functions
 * 7656:     public function handleCommentatorNotifications($uid, $conf,$pObj, $fromeID = false, $pid=0,$fromAjax=1)
 *
 *              SECTION: Administrator comment response functions
 * 7837:     protected function getCommentsReponse(&$params, &$pObj, $conf)
 *
 *              SECTION: Website preview functions
 * 7881:     public function cleanupfup($cmd, $cid, $data, $pObj, $fromAjax, $uploadedfile, $conf, $originalfilename)
 * 7901:     public function getwebpagepreview($cmd, $cid, $data, $pObj, $fromAjax, $websitepreviewid=0, $conf)
 * 7947:     protected function getpreviewinit($cid,$data, $pObj)
 * 7985:     protected function savewebpagepreviewtodb($pcid,$pcommentid,$userurl,$conf, $pObj)
 * 8201:     protected function cleanupdbandfiles($conf,$uploadedfile='',$originalfilename='')
 * 8333:     protected function getwebpagecache($pcid,$pcommentid, $conf, $url='', $isbeforefetch = false, $pObj)
 * 8477:     protected function read_dir($dir, $array = array())
 * 8505:     protected function commentShowWebpagepreview ($rowattachmentid,$rowattachment_subid,$conf,$pObj,$cid,$topwebsitepreview,$fromAjax, $row = array(), $isforemailnotification = false)
 * 8919:     protected function makeAttachementPicture($picturefilename,$conf,$descriptionbyuser,$originalfilename, $firstname, $lastname,$fetoctocusertoinsert)
 *
 *              SECTION: eID-Interface functions
 * 9088:     public function handleeID($uid, $conf,$pObj,$messagetodisplay,$refreshurl)
 *
 *              SECTION: Reply on comments functions
 * 9182:     protected function getCommentBoxDisplay($commentid,$conf,$level,$fromAjax,$triggeredlevel=0,$levelexpandoverride=0)
 * 9247:     protected function getCommentBoxChildrenDisplayIsCollapsed($commentschildrenids,$conf,$level,$fromAjax,$triggeredlevel=0,$levelexpandoverride=0)
 *
 *              SECTION: Fe-usergroup functions
 * 9308:     public function usersGroupmembers($pObj, $fromAjax, $conf, $communitybuddies = false)
 * 9375:     protected function isUserOnline($feuser_uid)
 *
 *              SECTION: Recent comments functions
 * 9412:     public function getRecentComments($pObj, $conf,$feuserid)
 * 9473:     protected function comments_getRecentComments($rows,$conf,$pObj)
 * 9698:     protected function trimContent($text,$conf, $maxChars=0, $dospecialChars=true)
 * 9735:     protected function createRCLinks($text, $refID, $commentID, $prefix, $table, $externalprefix, $singlePid, $conf,$show_uid)
 *
 *              SECTION: Report comments functions
 * 9783:     protected function getCommentsReportLink($params, &$pObj, $conf ,$fromAjax,$pid)
 * 9820:     public function mainReport($content, $conf, $pObj, $piVars)
 * 9844:     protected function processReportForm(array &$errors,$conf, $pObj, $piVars)
 * 9972:     protected function showReportThanks($conf,$pObj)
 * 9985:     protected function showReportForm(array $errors,$conf,$pObj,$piVars)
 * 10067:     protected function getReportCaptcha($required, $error,$conf,$pObj)
 *
 *              SECTION: IP-blocking functions
 * 10111:     protected function IPBlockSpamCheck(&$params, &$pObj)
 * 10128:     protected function sendNotificationIPBlock($params, &$pObj)
 * 10142:     private function getIpAddr()
 * 10157:     private function checkTableBLs($ipaddr)
 * 10168:     private function checkLocalBL($ipaddr)
 * 10181:     private function checkStaticBL($ipaddr)
 * 10208:     function checkNetworkBLs($ipaddr)
 *
 * TOTAL FUNCTIONS: 109
...
class toctoc_comment_lib  extends tslib_pibase {
	...
	var $ajaxHeader = '';
	var $topareaHTML = '';
	var $numberOfLeadingBlanksToReplace = 50;
	...
	var $BBs=array (
				...
				array('[code]','<code>'),
				array('[/code]','</code>'),
		); // you can extend BBs here
	//CSS
	var $boxmodelspacing = 4;
	var $expandiconCSSmargin = 'margin: 6px 0 0 4px;';
	var $firstCSSmargin='margin: 0 4px;';  // waittext
	var $picuploadCSSmargin='margin: 0 4px;';  //picupload
	var $pdfuploadCSSmargin='margin: 2px 4px 0;';  //PdfUpload
	var $wpplogoCSSmargin='margin: 4px 0 0;';
	var $userimagesize = 96;
	var $userimagesizeform = 96; //3*18;
	var $levelindent=22; //32/2+6
	var $levelindentbordersize = '1px solid';
	var $levelindentCSSborderradius='border-radius: 10px 4px 0 0 ;';
	var $taCSSheight=20;
	var $taareaCSSheightinit=32;
		...
		if ($conf['advanced.']['useSharing'] !=0){
			...
			if (intval($conf['advanced.']['useSharing']) ==1) {
				...
				if (intval($conf['advanced.']['useSharingDesign']) ===0 ) {
					...
					$templatesharrre = $pObj->cObj->getSubpart($pObj->templateCode, '###SHARRREDESIGN3###');
				}elseif (intval($conf['advanced.']['useSharingDesign']) ===3 ) {
					$templatesharrre = $pObj->cObj->getSubpart($pObj->templateCode, '###SHARRREDESIGN4###');
				}elseif (intval($conf['advanced.']['useSharingDesign']) ===4 ) {
					$templatesharrre = $pObj->cObj->getSubpart($pObj->templateCode, '###SHARRREDESIGNADDTHIS###');
				}
		...
		if ($conf['ratings.']['enableRatings'] ==1) {
			if (intval($conf['ratings.']['useLikeDislike']) ===1 ) {
				if ((intval($conf['ratings.']['useDislike']) ===1 ) && ($articlerating['idislike']!='')) {
					...
					if (($articlerating['mydislikehtml'] !='') || (($conf['ratings.']['useVotes']==1) && ($conf['ratings.']['useTopVotes']==1)) || ($conf['advanced.']['useSharing']==1) ||
					...
					if ($articlerating['ilike']!='') {
						...
						if (intval($conf['ratings.']['useDislike']) ===0 ) {
							if (($articlerating['mylikehtml'] !='') || (($conf['ratings.']['useVotes']==1) && ($conf['ratings.']['useTopVotes']==1)) || ($conf['advanced.']['useSharing']==1)||
									(($conf['advanced.']['useSharing']==0) && (($conf['ratings.']['useVotes']==0) || ($conf['ratings.']['useTopVotes']==0)) && ($conf['ratings.']['ratingsOnly']==0))) {
								$ilikedislikeHTML.= '&nbsp;&middot;&nbsp;&nbsp;';
							}
						}
	 ...
	 */
	protected function makeemoji($text, $conf) {
		$explodekey="\r\n";
		...
		if ($conf['advanced.']['useEmoji']==1) {
			...
			$text= $this->libemoji->emoji_unified_to_html($text);

			$textspanarr =explode('<span class="emoji emoji',$text);
			if (count($textspanarr)>1) {
				...
				$text=implode('<span class="emoji emoji',$textspanarr);
			}
		}
		if (count($textstreams)>1) {
	 ...
	 * @return	[type]		...
	 */
	protected function unicodeString($str, $encoding=null) {
	...
	public function comments_getComments(&$rows,$conf,$pObj,$feuserid,$fromAjax,$pid,$levelhlt=0) {
		...
		foreach ($rows as $row) {
			...
			if (($conf['advanced.']['userCommentResponseLevels'] != 0) && ($this->isCommentingClosed($conf,$pObj) == 0) && ($row['level']<$conf['advanced.']['userCommentResponseLevels'])) {
				...
				//$conf['advanced.']['replyModeInline']=1;
			...
			if (($conf['advanced.']['wallExtension'] != 0) || ($conf['prefixToTableMap.'][$conf['externalPrefix']] == 'fe_users')) {
				...
				IF ($triggereduserid != $feuserid) {
					if ($conf['advanced.']['wallExtension'] != 0) {
						...
						$lastname2='';//$this->applyStdWrap(htmlspecialchars(''), 'firstName_stdWrap', $conf, $pObj);
						$firstname2='';//$this->applyStdWrap(htmlspecialchars(''), 'lastName_stdWrap', $conf, $pObj);
			...
			if ($conf['commentsreport.']['active']) {
					...
					if($markerArray['###TX_COMMENTSREPORT###'] !=''){
						...
						$imghtml = '<img height="15" width="15" src="' . '/' . t3lib_extMgm::siteRelPath('toctoc_comments') . 'res/css/themes/' . $conf['theme.']['selectedTheme'] . '/img/report.png" title="' . $linkendtext .'">';
	...
	*/
	...
	public function form($conf, &$pObj, $piVars,$fromAjax,$pid, $ifeuserid=0, $userpic, $commentid=0, $replylevel=0, $fromcomments = false, $cidfromajaxcomments = 0) {
		...
		$this->form_updatePostVarsWithFeUserData($conf, $pObj, $piVars,$feuserid,$fromAjax, $userpic, $cid, $output_cid);
		...
		//$this->userimagesizeform=3*18;  //54
		...
		if (($conf['attachments.']['usePicUpload']==1) || ($conf['attachments.']['usePdfUpload']==1)) {
			...
			if ($conf['attachments.']['usePicUpload']==1) {
				$uploadpictext='<img class="tx-tc-upload" style="display:block;' . $firstmargin . '" align="left" src="/' . t3lib_extMgm::siteRelPath('toctoc_comments') . 'res/css/themes/' . $conf['theme.']['selectedTheme'] . '/img/uploadpic.png" width="16" ';
			...
			if ($conf['attachments.']['usePdfUpload']==1) {
				$uploadpdftext='<img class="tx-tc-upload" style="display:block;' . $secondmargin . '" align="left" src="/' . t3lib_extMgm::siteRelPath('toctoc_comments') . 'res/css/themes/' . $conf['theme.']['selectedTheme'] . '/img/uploadpdf.png" width="16" ';
			...
			if (($fromAjax) && ($_SESSION['submitCommentVars'][$cid]['uploadpicid']!='')) {
				...
				if (($_SESSION['submitCommentVars'][$cid]['uploadpicid'] == 'adobepdf.png')) {
					$fullfppath= '/' . t3lib_extMgm::siteRelPath('toctoc_comments') . 'res/css/themes/' . $conf['theme.']['selectedTheme'] . '/img/' . $_SESSION['submitCommentVars'][$cid]['uploadpicid'];
		...
		$prvcttext	='<div style="" class="' . $cssstyleeye . '-dp" id="tx-tc-cts-prv-ct-dp-' . $tmpcid . '"><img id="prv-ct-' . $tmpcid . '" class="' . $cssstyleeye .
						'" style="display:none;" align="left" src="/' . t3lib_extMgm::siteRelPath('toctoc_comments') .
		...
		$prvcommentHTML.= '<div onclick="previewct(\'\', \'' . $tmpcid . '\',0);" id="tx-tc-cts-div-ct-prv-' . $tmpcid . '"  class="tx-tc-ct-prv" style="display:none;" >';
		$prvcommentHTML.= '<img id="tx-tc-cts-img-prv-ct-' . $tmpcid . '" src="' . '/' . t3lib_extMgm::siteRelPath('toctoc_comments') .
		...
		$outhtml = $pObj->cObj->substituteMarkerArray($template, $markers);
		if ((intval($piVars['level']) != 0)) {
	 ...
	 * @return	void
	 */
	protected function form_updatePostVarsWithFeUserData($conf, &$pObj, $piVars, $feuserid, $fromAjax, $userpic, $cid, $output_cid) {
		global $TSFE;
				...
				foreach($rows as $key) {
					...
					if ((!$piVars['imagetag'])  || $reloadpivars) {
						...
						if (!$_SESSION['commentListCount']) {
							...
							$img['params'] = 'style="' . $userimagestyle . ' display:none; margin:0;" id="tx-tc-uimg-' . $output_cid . '" align="left"';
						}
						if ($fromAjax){
	...
	public function updateComment($conf,$pObj,$ctid,$content,$pid) {
		...
		$contenttext = $this->makeemoji($contenttext,$conf);
		...
		if (($action=='approve') ||  ($action=='info')) {
			if (t3lib_div::validEmail($toEmail) && t3lib_div::validEmail($fromEmail)) {
				...
				if ($conf['HTMLEmail']) {
					...
					$contentformail = $this->makeemoji($contentformail,$conf);
	...
	*/
	 ...
	 * @param	string		$content: content
	 * @param	object		$pObj: parent object
	 * @param	[type]		$purge: if set to yes bb are dropped
	 * @return	string		HTML
	 */
	...
	function makeiLikeText($conf, $pObj, $cmd, $ref, $fromAjax, $myrating = array(),$mylikeval,$mydis='',$template,$cid, $commentspics,$extpreffortext) {
		...
		. '" src="' . '/' . t3lib_extMgm::siteRelPath('toctoc_comments') . 'res/css/themes/' . $conf['theme.']['selectedTheme'] . '/img/'  . $mylikepic . '" />';
		$retarr=array();
		$retarr[0]=$mylike;
		$retarr[1]=$mylikehtml;
				...
				if (count($rowsattmmm) >0) {
					if ($conf_cachetime > $rowsattmmm[0]['tstamp']) {
						...
						if ($rowsattmmm[0]['attachmentvariant']<3) {
							...
							$picstr='<img style="display:block; max-height: ' . $conf['attachments.']['webpagePreviewHeight'] . 'px;" id="vidimg' . $cid . 'index1" src="' . $rowsattmmm[0]['photo_main'] . '" class="tx-tc-pvs-vid-img" />';
							...
							$desriptionweb = $this->makeemoji($desriptionweb,$conf);
		...
		if (count($rowsatt)>0) {
				...
				if (trim($rowsatt[0]['photos_etc']!='')) {
					...
					if ($rowsatt[0]['attachmentvariant']==1) {
						...
						if (count($rrpic)>0) {
							...
							//pdf no pvpic
							$picsrcstr= '<img title="' . $rowsatt[0]['title'] . '" style="display: block;"  ' . $palign . ' src="' . $plinkhomepage . t3lib_extMgm::siteRelPath('toctoc_comments') .  'res/css/themes/' . $conf['theme.']['selectedTheme'] . '/img/adobepdf.png" ';
							$picsrcstr .= 'class="tx-tc-images-img-browse" />';
						}
						if (count($rrpic)>1) {
							//pic
							...
							//pdf
							$picstr = '<img title="' . $rowsatt[0]['title'] . '" style="display: block;" ' . $palign . ' src="' . $plinkhomepage . t3lib_extMgm::siteRelPath('toctoc_comments') . 'res/css/themes/' . $conf['theme.']['selectedTheme'] . '/img/adobepdf.png" ';
							$picstr .= 'class="tx-tc-images-img" />';
							$link='' . $plinkhomepage . $this->webpagepreviewsavefolder . $ttfeuserid . str_replace(' ', '_', $rowsatt[0]['systemurltext']);
						}
			...
			if (!$isforemailnotification) {
				if ($rowsatt[0]['attachmentvariant']==1) {
					...
					//$marginovl= 'margin: -' . (5+round(($conf['attachments.']['webpagePreviewHeight']/2),0)) . 'px 0 12px ' . (round(($conf['attachments.']['webpagePreviewHeight']/2),0)) . 'px;';

					$picstr='<img style="display:block; max-height: ' . $conf['attachments.']['webpagePreviewHeight'] . 'px;" id="vidimg' . $cid . 'index1" src="' .
					...
					'" src="' . '/' . t3lib_extMgm::siteRelPath('toctoc_comments') . 'res/css/themes/' . $conf['theme.']['selectedTheme'] . '/img/play_vid.png" />';
					...
					if (count($sourcearr) >=3) {
						...
						$templateattachmentHTML = $pObj->cObj->getSubpart($pObj->templateCode, '###SUBFORMATTACHMENTVIDHTML5###');
						$videohtml = '';
						...
						$templateattachmentHTML = $pObj->cObj->getSubpart($pObj->templateCode, '###SUBFORMATTACHMENTVIDFLASH###');
						$html5flag ='';
					}
					$videoembedhtml=  $pObj->cObj->substituteMarkerArray($templateattachmentHTML, array(
							'###VIDEOHTML###' => $videohtml,
							'###TITLE###' => $titlesitevid,
							'###IMAGE###' => $picstr,
							'###IMAGESRC###' => $picsrcstr,
					...
					$templateattachmentHTML = $pObj->cObj->getSubpart($pObj->templateCode, '###SUBFORMATTACHMENTVID###');
					$attachmentHTML =  $pObj->cObj->substituteMarkerArray($templateattachmentHTML, array(
							'###TITLE###' => $titlesitevid,
							'###IMAGE###' => $picstr,
							'###IMAGESRC###' => $picsrcstr,
				...
				if ($rowsatt[0]['attachmentvariant']==1) {
					...
					$templateattachmentHTML = $pObj->cObj->getSubpart($pObj->templateCode, '###SUBFORMATTACHMENTVIDMAIL###');
					$attachmentHTML =  $pObj->cObj->substituteMarkerArray($templateattachmentHTML, array(
							'###TITLE###' => $rowsatt[0]['title'],
							'###IMAGE###' => $picstr,
							'###DESC###' => $desriptionweb,
	...
	public function handleeID($uid, $conf,$pObj,$messagetodisplay,$refreshurl) {
		...
		if ($refreshurl !='') {
			...
			if ($refresh!='') {
				...
				$waittext.='<img style="display:block;' . $this->firstCSSmargin . '" align="left" src="' . t3lib_extMgm::siteRelPath('toctoc_comments') . 'res/css/themes/' . $conf['theme.']['selectedTheme'] . '/img/workingslides.gif" width="16" ';
		...
		foreach ($rows as $row) {
			...
			if ($mmtable== 'fe_users') {
				...
				if ($ownershipok==1) {
					if ($row['parentuid'] !=0) {
						...
						$posbeforeidh = strrpos($hlsctprow['external_ref'], '_')+1;
						$refIDh = substr($hlsctprow['external_ref'], $posbeforeidh);
			...
			if ($ownershipok==1) {
				...
				if ($prefix == 'pages_') {
					...
					$exticon= t3lib_extMgm::siteRelPath('toctoc_comments') . 'res/css/themes/' . $conf['theme.']['selectedTheme'] . '/img/usericon.gif">';
				} else {
				...
				$conf['advanced.']['useEmoji']=0;
				$commenttext = $this->makeemoji($commenttext,$conf);
