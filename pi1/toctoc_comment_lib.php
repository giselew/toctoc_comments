 ...
 *
 *
 *  198: class toctoc_comment_lib  extends tslib_pibase
 *  258:     public function maincomments($ref, $conf = null, $fromAjax = false, $pid=0, $feuserid = 0, $cmd = '', &$pObj, $piVars = array(),
		$cid = 0, $sessioncapdata = '', $userpic='', $commentspics = array(), $check='', $AjaxData = '', $ajaxextref= '', $tctreestate = null,$commentreplyid=0)
 *  413:     protected function checkExternalUid($conf,$pObj)
 *
 *              SECTION: Commenting functions
 *  441:     public function comments($conf,&$pObj,$fromAjax,$feuserid,$pid)
 * 1455:     public function comments_getComments(&$rows,$conf,$pObj,$feuserid,$fromAjax,$pid,$levelhlt=0)
 * 2148:     protected function comments_getComments_getRatings(&$row,$conf,$pObj,$feuserid, $fromAjax)
 * 2167:     protected function comments_getComments_getEmail($email)
 * 2181:     protected function isCommentingClosed($conf,$pObj)
 * 2227:     protected function commentingClosed($pObj,$fromAjax)
 * 2247:     protected function comments_getCommentsBrowser($rpp,$startpoint,$totalrows,$pObj,$fromAjax, $conf)
 * 2363:     protected function build_AJAXImages ($conf,$usergenderexistsstr='')
 * 2501:     public function comments_getComments_fe_user($params, $conf, $pObj,$commentid,$fromAjax)
 * 2588:     protected function fixFeUserPic ($conf, $pic)
 * 2607:     protected function addleadingspace($text)
 * 2628:     protected function check_comment_ownership ($rowexternal_ref,$feuserid)
 * 2650:     public function previewcomment($data, $pObj, $conf, $fromAjax=true)
 *
 *              SECTION: functions for images cache (AJAX)
 * 2678:     protected function setAJAXimage($image,$feuserid)
 * 2689:     protected function getAJAXimage($feuserid,$commentid)
 * 2720:     protected function setAJAXimageCache($image,$imageoriginal,$feuserid)
 * 2731:     protected function getAJAXimageCache($commentuserimageout)
 *
 *              SECTION: Form functions
 * 2760:     public function form($conf, &$pObj, $piVars,$fromAjax,$pid, $ifeuserid=0, $userpic, $commentid=0, $replylevel=0, $fromcomments = false, $cidfromajaxcomments = 0)
 * 3410:     protected function form_updatePostVarsWithFeUserData($conf, &$pObj, $piVars, $feuserid, $fromAjax, $userpic, $cid, $output_cid)
 * 3706:     protected function form_getCaptcha($pObj, $conf,$fromAjax)
 * 3754:     protected function form_wrapError($field, $pObj, $conf)
 *
 *              SECTION: Comments page-browser functions
 * 3787:     protected function processBrowserSubmission($conf, $pObj, $piVars, $fromAjax, $cid, $cmd,$pid, $ref)
 *
 *              SECTION: Inserting and Updating Comments functions
 * 3868:     public function processSubmission($conf,$pObj,$piVars,$fromAjax,$feuserid,$pid,$lang)
 * 4329:     public function updateComment($conf,$pObj,$ctid,$content,$pid)
 * 4427:     protected function processSubmission_checkTypicalSpam($pObj, $conf, $piVars, $lang, $fromAjax)
 * 4500:     protected function processSubmission_validate($piVars, $conf, $pObj,$fromAjax)
 * 4569:     public function sendNotificationEmail($uid, $points, $action, $conf, $pObj, $fromAjax, $piVars, $pid,$fetoctocusertoinsert,$attachment_id=0 ,$attachment_subid=0, $optinemail='')
 * 4922:     protected function emailOptIn($conf, $optinemail, $optin_ip)
 * 4995:     protected function checkCOI($conf,$email,$checkip=true)
 *
 *              SECTION: language handling functions
 * 5031:     public function fixLL(&$conf)
 * 5052:     protected function fixLL_internal($LL, &$ll, $prefix = '')
 *
 *              SECTION: mostly formatting functions
 * 5075:     protected function createLinks($text, $conf)
 * 5097:     protected function applyStdWrap($text, $stdWrapName, $conf = null, $pObj = null)
 * 5116:     protected function checkCustomFunctionCodes($code, $pObj)
 * 5138:     protected function isNoCacheUrl($url)
 * 5159:     protected function substituteMarkersAndSubparts($template, array $markers, array $subparts, $pObj)
 *
 *              SECTION: session and cache functions
 * 5182:     public function resetSessionVars($resetcontext, $alsoajaxvar = true)
 * 5229:     public function getClearCacheIds($conf, $pid = 0)
 * 5262:     public function initCaches()
 *
 *              SECTION: Smilie functions
 * 5288:     public function parseSmilieArray($data)
 * 5305:     public function replaceSmilies($content,$conf)
 * 5322:     protected function checkbbcrop($content,$commentCropLength)
 * 5354:     protected function replaceBBs($content,$conf,$purge=false)
 *
 *              SECTION: functions for the AJAx interface with jQuery/JavaScript
 * 5399:     protected function getAjaxJSData($feuserid, $pid, $languagecode, $conf, $pObj, $cid, $fromAjax, $dataonly=false)
 * 5439:     protected function getAjaxData($feuserid, $pid, $languagecode, $conf, $cid)
 * 5483:     protected function getAjaxDataAttachments($conf,$fromAjax,$pObj)
 * 5504:     protected function getAjaxJSDataCommentImgs($cid, $pObj,$fromAjax)
 * 5540:     protected function getAjaxJSDataComments($cid,$pObj,$fromAjax)
 * 5578:     protected function getAjaxDataComments($pObj, $cid)
 * 5598:     protected function getAjaxDataImgs()
 *
 *              SECTION: URL- and IP-related functions
 * 5617:     public function getCurrentIp()
 * 5631:     public function hasValidItemUrl($piVars)
 * 5654:     public function getDefaultConfig($pluginkey='')
 * 5667:     protected function ae_detect_ie()
 *
 *              SECTION: Rating functions
 * 5695:     public function getRatingDisplay($ref, $conf = null, $fromAjax = 0, $pid=0, $returnasarray = false, $feuserid = 0, $cmd = 'vote',
			$pObj = null, $cid, $fromcomments,$commentspics = array())
 * 5741:     public function isVoted($ref,$conf,$pObj)
 * 5759:     protected function getBarWidth($rating,$conf)
 * 5772:     protected function getRatingInfo($ref,$pObj, $feuserid=-1,$conf)
 * 5875:     protected function generateRatingContent($ref, $template, $conf, $fromAjax, $pid, $returnasarray, $feuserid, $pObj,$cmd, $cid,$fromcomments, $commentspics)
 * 6465:     function makeiLikeText($conf, $pObj, $cmd, $ref, $fromAjax, $myrating = array(),$mylikeval,$mydis='',$template,$cid, $commentspics,$extpreffortext)
 *
 *              SECTION: TYPO3 workarounds
 * 6812:     public function enableFields($tableName,$pObj)
 * 6832:     public function pi_getLLWrap($pObj, $llkey, $fromAjax)
 * 6850:     protected function getcheck ($ref, $i, $ajaxData, $ratingscheck)
 * 6871:     protected function formatDate($date, $pObj, $fromAjax, $conf)
 * 6991:     protected function getPageURL($fromAjax = false,$pid = 0)
 * 7017:     protected function getCleanHTML ($html)
 *
 *              SECTION: Usercards
 * 7050:     public function getUserCard($basedimgstr,$basedtoctocuid,$conf,$pObj,$commentid)
 *
 *              SECTION: Mail functions
 * 7366:     protected function getMailTo($mailAddress, $linktxt = '', $initP = '?')
 * 7409:     protected function encryptEmail($string,$back=0)
 * 7445:     protected function encryptCharcode($n,$start,$end,$offset)
 * 7467:     function send_mail ($toEMail,$subject,$message,$html,$fromEMail,$fromName,$attachment='')
 * 7595:     protected function slashName ($name, $apostrophe='"')
 *
 *              SECTION: Commentator-Notification functions
 * 7619:     public function handleCommentatorNotifications($uid, $conf,$pObj, $fromeID = false, $pid=0,$fromAjax=1)
 *
 *              SECTION: Administrator comment response functions
 * 7800:     protected function getCommentsReponse(&$params, &$pObj, $conf)
 *
 *              SECTION: Website preview functions
 * 7844:     public function cleanupfup($cmd, $cid, $data, $pObj, $fromAjax, $uploadedfile, $conf, $originalfilename)
 * 7864:     public function getwebpagepreview($cmd, $cid, $data, $pObj, $fromAjax, $websitepreviewid=0, $conf)
 * 7910:     protected function getpreviewinit($cid,$data, $pObj)
 * 7943:     protected function savewebpagepreviewtodb($pcid,$pcommentid,$userurl,$conf)
 * 8130:     protected function cleanupdbandfiles($conf,$uploadedfile='',$originalfilename='')
 * 8258:     protected function getwebpagecache($pcid,$pcommentid, $conf, $url='', $isbeforefetch = false)
 * 8369:     protected function read_dir($dir, $array = array())
 * 8397:     protected function commentShowWebpagepreview ($rowattachmentid,$rowattachment_subid,$conf,$pObj,$cid,$topwebsitepreview,$fromAjax, $row = array(), $isforemailnotification = false)
 * 8683:     protected function makeAttachementPicture($picturefilename,$conf,$descriptionbyuser,$originalfilename, $firstname, $lastname,$fetoctocusertoinsert)
 *
 *              SECTION: eID-Interface functions
 * 8852:     public function handleeID($uid, $conf,$pObj,$messagetodisplay,$refreshurl)
 *
 *              SECTION: Reply on comments functions
 * 8946:     protected function getCommentBoxDisplay($commentid,$conf,$level,$fromAjax,$triggeredlevel=0,$levelexpandoverride=0)
 * 9013:     protected function getCommentBoxChildrenDisplayIsCollapsed($commentschildrenids,$conf,$level,$fromAjax,$triggeredlevel=0,$levelexpandoverride=0)
 *
 *              SECTION: Fe-usergroup functions
 * 9074:     public function usersGroupmembers($pObj, $fromAjax, $conf, $communitybuddies = false)
 *
 *              SECTION: Recent comments functions
 * 9154:     public function getRecentComments($pObj, $conf,$feuserid)
 * 9215:     protected function comments_getRecentComments($rows,$conf,$pObj)
 * 9401:     protected function trimContent($text,$conf, $maxChars=0)
 * 9442:     protected function createRCLinks($text, $refID, $commentID, $prefix, $table, $externalprefix, $singlePid, $conf,$show_uid)
 *
 *              SECTION: Report comments functions
 * 9490:     protected function getCommentsReportLink($params, &$pObj, $conf ,$fromAjax,$pid)
 * 9525:     public function mainReport($content, $conf, $pObj, $piVars)
 * 9549:     protected function processReportForm(array &$errors,$conf, $pObj, $piVars)
 * 9677:     protected function showReportThanks($conf,$pObj)
 * 9690:     protected function showReportForm(array $errors,$conf,$pObj,$piVars)
 * 9775:     protected function getReportCaptcha($required, $error,$conf,$pObj)
 *
 *              SECTION: IP-blocking functions
 * 9819:     protected function IPBlockSpamCheck(&$params, &$pObj)
 * 9838:     protected function sendNotificationIPBlock($params, &$pObj)
 * 9852:     private function getIpAddr()
 * 9867:     private function checkTableBLs($ipaddr)
 * 9878:     private function checkLocalBL($ipaddr)
 * 9891:     private function checkStaticBL($ipaddr)
 * 9918:     function checkNetworkBLs($ipaddr)
 *
 * TOTAL FUNCTIONS: 106
...
class toctoc_comment_lib  extends tslib_pibase {
			...
			if (($_GET['toctoc_comments_pi1']['anchor']) || (intval($_SESSION['newcommentid']) > 0)) {
				if (($_SESSION['findanchor'] == '1') || (intval($_SESSION['newcommentid']) > 0)) {
					...
					if ((count($hlsanchorarr)>0) || (intval($_SESSION['newcommentid']) > 0)) {
						...
						///print $whereloc . $condfeusergroup . ' AND uid=' . $hlsctid . '<br>';
						///print_r($hlsctrow);
						///print '1<br><br>';
		...
		foreach ($rows as $row) {
			...
			if (strlen($row['content'])>$conf['commentCropLength']){
				...
				if (count($textcroppedrightarr)>1) {
					...
					if (count($bbterminatorarr)> 1) {
						//print $row['content'];exit;
					...
					if ($fromAjax) {
						...
						if (count($subcommentreplyformhtmlarr)>1) {
							...
							if (count($remainingthmlarr)>1) {
								...
								///print $remainingthmlarr[0] . ', ' . $_SESSION['commentListCount'];exit;
						...
						//$subcommentreply= str_replace('tx-tc-uimg-' . $row['uid'], 'tx-tc-uimg-' . '6g9' . $row['uid'] . '6g9', $subcommentreply);
					} else {
				...
				IF ($triggereduserid != $feuserid) {
					if ($conf['advanced.']['wallExtension'] != 0) {
						...
						if (count($rowsfeuser)>0) {
							...
							if (trim($firstname2 . '&nbsp;' . $lastname2)==$this->applyStdWrap(htmlspecialchars(''), 'firstName_stdWrap', $conf, $pObj) . '&nbsp;' . $this->applyStdWrap(htmlspecialchars(''), 'lastName_stdWrap', $conf, $pObj)) {
								if (array_key_exists('name',$rowsfeuser[0])) {
									//$lastname2=$rowsfeuser[0]['name'];
									$lastname2=$this->applyStdWrap(htmlspecialchars($rowsfeuser[0]['name']), 'lastName_stdWrap', $conf, $pObj);
				...
				if (count($_SESSION['AJAXimages'])!=0) {
					...
					foreach($rowsfeuserimages as $keyimage) {
						...
						if ($tmpimgstr==='') {
							...
							$img['params'] = 'style="' . $userimagestyle . '" class="'.$profileimgclass.'" id="tx-tc-cts-img-"'  ;   //style="margin: -6px 6px -6px -3px; padding: 0pt 0pt 0pt 0px; border-radius: 2px 2px 2px 2px; border: ' . $this->levelindentbordersize . ' #97b0ee;" align="left"';
	...
	*/
	 ...
	 * @param	[type]		$replylevel: ...
	 * @param	[type]		$fromcomments: ...
	 * @param	[type]		$cidfromajaxcomments: ...
				...
				*/
				...
				if ($info['t'] > 0) {
					...
					if ($isSpam < $cutOffPoint) {
						...
						//print $dataWhereparent . '<br>' . $this->insertwasnotconsistent;exit;
		...
		for ($i=0;$i<count($this->BBs);$i=$i+2) {
			...
			if (count($startbbarr) > count($endbbarr)) {
				if (strlen($startbbarr[count($startbbarr)-1])<$bbproximity) {
					...
					//print_r($bbbrigearr); print '<br>' . strlen($this->BBs[$i][1]) . '<br>' ;print_r($startbbarr); print '<br>' . count($endbbarr) . '<br>' ;print_r($endbbarr); exit;
				}
			}
	...
	public function handleeID($uid, $conf,$pObj,$messagetodisplay,$refreshurl) {
		...
		if ($refreshurl !='') {
			...
			if ($refresh!='') {
				...
				$waittext.='<img style="display:block;' . $this->firstCSSmargin . '" align="left" src="' . t3lib_extMgm::siteRelPath('toctoc_comments') . 'res/img/workingslides.gif" width="16" ';
 ...
 */
	protected function getCommentBoxDisplay($commentid,$conf,$level,$fromAjax,$triggeredlevel=0,$levelexpandoverride=0) {
		...
		if ((intval($levelexpandoverride)==1) && ($triggeredlevel>0) && ($triggeredlevel>$conf['advanced.']['userCommentResponseLevelExpanded'])){
			//print 'Expand vis. ' . $commentid . ', level: ' . $level . ', triggeredlevel: ' . $triggeredlevel . ', levelexpandoverride: ' . $levelexpandoverride . "\n" ;
		} else {
			$triggeredlevel=$conf['advanced.']['userCommentResponseLevelExpanded'];
			//print 'NO expand vis. ' . $commentid . ', level: ' . $level . ', triggeredlevel: ' . $triggeredlevel . ', levelexpandoverride: ' . $levelexpandoverride . "\n" ;
		}
