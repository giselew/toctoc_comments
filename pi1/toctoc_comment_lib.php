 ...
 *
 *
 *  204: class toctoc_comment_lib  extends tslib_pibase
 *  264:     public function maincomments($ref, $conf = null, $fromAjax = false, $pid=0, $feuserid = 0, $cmd = '', &$pObj, $piVars = array(),
		$cid = 0, $sessioncapdata = '', $userpic='', $commentspics = array(), $check='', $AjaxData = '', $ajaxextref= '', $tctreestate = null,$commentreplyid=0)
 *  427:     protected function checkExternalUid($conf,$pObj)
 *
 *              SECTION: Commenting functions
 *  455:     public function comments($conf,&$pObj,$fromAjax,$feuserid,$pid)
 * 1455:     protected function makeemoji($text, $conf)
 * 1550:     public function comments_getComments(&$rows,$conf,$pObj,$feuserid,$fromAjax,$pid,$levelhlt=0)
 * 2235:     protected function comments_getComments_getRatings(&$row,$conf,$pObj,$feuserid, $fromAjax)
 * 2254:     protected function comments_getComments_getEmail($email)
 * 2268:     protected function isCommentingClosed($conf,$pObj)
 * 2315:     protected function commentingClosed($pObj,$fromAjax,$conf)
 * 2336:     protected function comments_getCommentsBrowser($rpp,$startpoint,$totalrows,$pObj,$fromAjax, $conf)
 * 2454:     protected function build_AJAXImages ($conf, $pObj, $usergenderexistsstr='', $fromAjax=false)
 * 2619:     public function comments_getComments_fe_user($params, $conf, $pObj,$commentid,$fromAjax, $commentusername)
 * 2716:     protected function fixFeUserPic ($conf, $pic)
 * 2735:     protected function addleadingspace($text)
 * 2756:     protected function check_comment_ownership ($rowexternal_ref,$feuserid)
 * 2778:     public function previewcomment($data, $pObj, $conf, $fromAjax=true)
 *
 *              SECTION: functions for images cache (AJAX)
 * 2808:     protected function setAJAXimage($image,$feuserid)
 * 2819:     protected function getAJAXimage($feuserid,$commentid)
 * 2850:     protected function setAJAXimageCache($image,$imageoriginal,$feuserid)
 * 2861:     protected function getAJAXimageCache($commentuserimageout)
 *
 *              SECTION: Form functions
 * 2890:     public function form($conf, &$pObj, $piVars,$fromAjax,$pid, $ifeuserid=0, $userpic, $commentid=0, $replylevel=0, $fromcomments = false, $cidfromajaxcomments = 0)
 ...
 * 3952:     public function processSubmission($conf,$pObj,$piVars,$fromAjax,$feuserid,$pid,$lang)
 * 4390:     public function updateComment($conf,$pObj,$ctid,$content,$pid, $plugincacheid)
 * 4466:     protected function processSubmission_checkTypicalSpam($pObj, $conf, $piVars, $lang, $fromAjax)
 * 4539:     protected function processSubmission_validate($piVars, $conf, $pObj,$fromAjax)
 * 4609:     public function sendNotificationEmail($uid, $plugincacheid, $points, $action, $conf, $pObj, $fromAjax, $piVars, $pid,$fetoctocusertoinsert,$attachment_id=0 ,$attachment_subid=0, $optinemail='')
 * 4987:     protected function emailOptIn($conf, $optinemail, $optin_ip)
 * 5055:     protected function checkCOI($conf,$email,$checkip=true)
 *
 *              SECTION: language handling functions
 * 5088:     public function fixLL(&$conf)
 * 5109:     protected function fixLL_internal($LL, &$ll, $prefix = '')
 *
 *              SECTION: mostly formatting functions
 * 5132:     protected function createLinks($text, $conf)
 * 5154:     protected function applyStdWrap($text, $stdWrapName, $conf = null, $pObj = null)
 * 5173:     protected function checkCustomFunctionCodes($code, $pObj)
 * 5195:     protected function isNoCacheUrl($url)
 * 5216:     protected function substituteMarkersAndSubparts($template, array $markers, array $subparts, $pObj)
 *
 *              SECTION: session and cache functions
 * 5239:     public function resetSessionVars($resetcontext, $alsoajaxvar = true)
 * 5294:     public function getClearCacheIds($conf, $pid = 0, $repectsession = true)
 * 5353:     protected function getClearCacheExternal_ref_uids($conf, $external_ref_uid = '', $repectsession = true)
 * 5377:     public function setPluginCacheControlTstamp ($external_ref_uid_list)
 * 5440:     public function initCaches()
 * 5491:     protected function clearPagesCaches($conf,$pid,$plugincacheid)
 *
 *              SECTION: Smilie functions
 * 5520:     public function parseSmilieArray($data)
 * 5537:     public function replaceSmilies($content,$conf)
 * 5554:     protected function checkbbcrop($content,$commentCropLength)
 * 5580:     protected function replaceBBs($content,$conf,$purge=false)
 *
 *              SECTION: functions for the AJAx interface with jQuery/JavaScript
 * 5622:     protected function getAjaxJSData($feuserid, $pid, $languagecode, $conf, $pObj, $cid, $fromAjax, $dataonly=false,$externalref)
 * 5663:     protected function getAjaxData($feuserid, $pid, $languagecode, $conf, $cid,$ref)
 * 5706:     protected function getAjaxDataAttachments($conf,$fromAjax,$pObj)
 * 5727:     protected function getAjaxJSDataCommentImgs($cid, $pObj,$fromAjax)
 * 5763:     protected function getAjaxJSDataComments($cid,$pObj,$fromAjax)
 * 5801:     protected function getAjaxDataComments($pObj, $cid)
 * 5817:     protected function getAjaxDataImgs()
 *
 *              SECTION: URL- and IP-related functions
 * 5833:     public function getCurrentIp()
 * 5847:     public function hasValidItemUrl($piVars)
 * 5868:     public function getDefaultConfig($pluginkey='')
 * 5881:     protected function ae_detect_ie()
 *
 *              SECTION: Rating functions
 * 5909:     public function getRatingDisplay($ref, $conf = null, $fromAjax = 0, $pid=0, $returnasarray = false, $feuserid = 0, $cmd = 'vote',
			$pObj = null, $cid, $fromcomments,$commentspics = array())
 * 5955:     public function isVoted($ref,$conf,$pObj)
 * 5973:     protected function getBarWidth($rating,$conf)
 * 5986:     protected function getRatingInfo($ref,$pObj, $feuserid=-1,$conf)
 * 6090:     protected function generateRatingContent($ref, $template, $conf, $fromAjax, $pid, $returnasarray, $feuserid, $pObj,$cmd, $cid,$fromcomments, $commentspics)
 * 6666:     function makeiLikeText($conf, $pObj, $cmd, $ref, $fromAjax, $myrating = array(),$mylikeval,$mydis='',$template,$cid, $commentspics,$extpreffortext)
 *
 *              SECTION: TYPO3 workarounds
 * 7010:     public function enableFields($tableName,$pObj)
 * 7030:     public function pi_getLLWrap($pObj, $llkey, $fromAjax)
 * 7048:     protected function getcheck ($ref, $i, $ajaxData, $ratingscheck)
 * 7069:     protected function formatDate($date, $pObj, $fromAjax, $conf)
 * 7195:     protected function getPageURL($fromAjax = false,$pid = 0)
 * 7217:     protected function locationHeaderUrlsubDir($withleadingslash = true)
 * 7239:     protected function getCleanHTML ($html)
 *
 *              SECTION: Usercards
 * 7272:     public function getUserCard($basedimgstr,$basedtoctocuid,$conf,$pObj,$commentid)
 *
 *              SECTION: Mail functions
 * 7568:     protected function getMailTo($mailAddress, $linktxt = '', $initP = '?')
 * 7611:     protected function encryptEmail($string,$back=0)
 * 7647:     protected function encryptCharcode($n,$start,$end,$offset)
 * 7669:     function send_mail ($toEMail,$subject,$message,$html,$fromEMail,$fromName,$attachment='')
 * 7794:     protected function slashName ($name, $apostrophe='"')
 *
 *              SECTION: Commentator-Notification functions
 * 7818:     public function handleCommentatorNotifications($uid, $conf,$pObj, $fromeID = false, $pid=0,$fromAjax=1)
 *
 *              SECTION: Administrator comment response functions
 * 8000:     protected function getCommentsReponse(&$params, &$pObj, $conf)
 *
 *              SECTION: Website preview functions
 * 8044:     public function cleanupfup($cmd, $cid, $data, $pObj, $fromAjax, $uploadedfile, $conf, $originalfilename)
 * 8064:     public function getwebpagepreview($cmd, $cid, $data, $pObj, $fromAjax, $websitepreviewid=0, $conf)
 * 8110:     protected function getpreviewinit($cid,$data, $pObj)
 * 8148:     protected function savewebpagepreviewtodb($pcid,$pcommentid,$userurl,$conf, $pObj)
 * 8364:     protected function cleanupdbandfiles($conf,$uploadedfile='',$originalfilename='')
 * 8496:     protected function getwebpagecache($pcid,$pcommentid, $conf, $url='', $isbeforefetch = false, $pObj)
 * 8640:     protected function read_dir($dir, $array = array())
 * 8668:     protected function commentShowWebpagepreview ($rowattachmentid,$rowattachment_subid,$conf,$pObj,$cid,$topwebsitepreview,$fromAjax, $row = array(), $isforemailnotification = false)
 * 9089:     protected function makeAttachementPicture($picturefilename,$conf,$descriptionbyuser,$originalfilename, $firstname, $lastname,$fetoctocusertoinsert)
 *
 *              SECTION: eID-Interface functions
 * 9258:     public function handleeID($uid, $conf,$pObj,$messagetodisplay,$refreshurl)
 *
 *              SECTION: Reply on comments functions
 * 9355:     protected function getCommentBoxDisplay($commentid,$conf,$level,$fromAjax,$triggeredlevel=0,$levelexpandoverride=0)
 * 9420:     protected function getCommentBoxChildrenDisplayIsCollapsed($commentschildrenids,$conf,$level,$fromAjax,$triggeredlevel=0,$levelexpandoverride=0)
 *
 *              SECTION: Fe-usergroup functions
 * 9481:     public function usersGroupmembers($pObj, $fromAjax, $conf, $communitybuddies = false)
 * 9548:     protected function isUserOnline($feuser_uid)
 *
 *              SECTION: Recent comments functions
 * 9585:     public function getRecentComments($pObj, $conf,$feuserid)
 * 9646:     protected function comments_getRecentComments($rows,$conf,$pObj)
 * 9926:     protected function trimContent($text,$conf, $maxChars=0, $dospecialChars=true)
 * 9964:     protected function createRCLinks($text, $refID, $commentID, $prefix, $table, $externalprefix, $singlePid, $conf,$show_uid,$okrowsi)
 *
 *              SECTION: Report comments functions
 * 10024:     protected function getCommentsReportLink($params, &$pObj, $conf ,$fromAjax,$pid)
 * 10068:     public function mainReport($content, $conf, $pObj, $piVars)
 * 10092:     protected function processReportForm(array &$errors,$conf, $pObj, $piVars)
 * 10228:     protected function showReportThanks($conf,$pObj)
 * 10241:     protected function showReportForm(array $errors,$conf,$pObj,$piVars)
 * 10323:     protected function getReportCaptcha($required, $error,$conf,$pObj)
 *
 *              SECTION: IP-blocking functions
 * 10367:     protected function IPBlockSpamCheck(&$params, &$pObj)
 * 10384:     protected function sendNotificationIPBlock($params, &$pObj)
 * 10398:     private function getIpAddr()
 * 10413:     private function checkTableBLs($ipaddr)
 * 10424:     private function checkLocalBL($ipaddr)
 * 10437:     private function checkStaticBL($ipaddr)
 * 10464:     function checkNetworkBLs($ipaddr)
 *
 * TOTAL FUNCTIONS: 112
...
class toctoc_comment_lib  extends tslib_pibase {
	...
	*/
	...
	public function updateComment($conf,$pObj,$ctid,$content,$pid, $plugincacheid) {
		...
		// 04052013 $this->clearPagesCaches($conf,$pid,'');
		// 04052013 $this->setPluginCacheControlTstamp($plugincacheid);
	...
	protected function getClearCacheExternal_ref_uids($conf, $external_ref_uid = '', $repectsession = true) {
		...
		foreach($clearCachePluginsarr as $memexternal_ref_uid) {
			$_SESSION['mcp' . $memexternal_ref_uid]['L' . $_SESSION['activelang'] . 'U' . $_SESSION['feuserid']]=array();
		}
	...
	public function handleCommentatorNotifications($uid, $conf,$pObj, $fromeID = false, $pid=0,$fromAjax=1) {
		...
		if ($fromAjax==1) {
			...
			$templateCode = $pObj->cObj->fileResource($templatefilepath);
 ...
 */
	...
	protected function createRCLinks($text, $refID, $commentID, $prefix, $table, $externalprefix, $singlePid, $conf,$show_uid,$okrowsi) {
		...
		if ($conf['recentcomments.']['linkComments'] == 1) {
        	...
        	$text = str_replace('href="','href="javascript:recentct(' . $okrowsi . ', ' . $singlePid . ', \'',$text);
