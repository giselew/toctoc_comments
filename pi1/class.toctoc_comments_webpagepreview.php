 ...
 *
 *
 *   68: class toctoc_comments_webpagepreview
 *  106:     public function main($inputurl,$inputcommentid,$inputcid,$lang,$inconf = array(), $pObj = null, $pObjParent=null)
 *  311:     protected function timepoint($startpoint, $datainfo, $showtotal = false)
 *  336:     protected function check_pvs_url($value)
 *  355:     protected function file_pvs_get_contents_curl($urltofetch,$ext, $savepathfilename = '')
 *  505:     public function saveAndResize($filename, $new_width, $new_height, $pathAndFilename, $ext)
 *  812:     public function previewsite ($pObj=null, $pObjParent=null)
 * 1277:     public function pvs_fetch_images($strextraction,&$strouthtml, $iscss, $cssfile='')
 * 1431:     protected function pvs_fetch_css($strextraction,&$strouthtml, $iscss, $cssfile='')
 * 1533:     protected function checklogopattern($strtest)
 * 1559:     protected function checkimagepattern($strtest)
 *
 * TOTAL FUNCTIONS: 10
...
// 		}
	...
	protected function file_pvs_get_contents_curl($urltofetch,$ext, $savepathfilename = '') {
		...
		if (($ext=='html') || ($ext== 'css')) {
        		...
        		curl_setopt($ch, CURLOPT_TIMEOUT_MS, $this->conf['attachments.']['webpagePreviewCurlTimeout']);
        		curl_setopt($ch, 156, intval($this->conf['attachments.']['webpagePreviewCurlTimeout']));
	...
	public function saveAndResize($filename, $new_width, $new_height, $pathAndFilename, $ext){
		if (($this->selectedpics < $this->conf['attachments.']['webpagePreviewNumberOfImages']) || ($this->logofound == false)) {
			...
			if (DIRECTORY_SEPARATOR == '\\') {
				// windows
				$txdirname= str_replace('/','\\',$_SERVER['DOCUMENT_ROOT'] .$path_parts['dirname']);
			} else {
				$txdirname=$_SERVER['DOCUMENT_ROOT'] .$path_parts['dirname'];
			}
			$idstr=session_id();
			if ($idstr==false) {
	...
	public function previewsite ($pObj=null, $pObjParent=null) {
		...
		if ($html) {
			...
			$docutf8declared=false;
			if (strpos(strtoupper($html), 'CHARSET=UTF-8') >0 ) {
				$docutf8declared=true;
			}
